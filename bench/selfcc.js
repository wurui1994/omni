...
//   进程内编那 20 份：冷 597ms、热 403ms   ——   tcc（20 个进程）215ms
//   一趟一个进程：我们 3.8s / tcc 215ms = 17x  ← **这一栏不是编译速度**
//
// 中间那一栏才是编译速度：**冷 2.8x、热 1.9x**。最后那一栏里 20 × ~180ms 是 node + CLI
// 的起步（占了七成），tcc 那侧起步 ~2ms —— 先前只量了它，于是把差距夸大了六七倍。
// `buildSelf` 是**在一个进程里**连着编的，所以中间那一栏才是它的口径。
//
// 压过两轮（第一百三十九片，每一步都用「进程内、反复取最快」判，改完 `tests/c` 297/0）：
//   572ms -> 413ms（-28%）。头四处在前端的词法/预处理，第五处在 native 那一步：
...
//   切全局那两处 `bytes.push` 换成先分配再按下标写 —— 在噪声里，留着是因为它更直白。
//
// 大档（整份编译器那一份 14.4M 的 C，一个翻译单元）现在 `c obj` **5.6s** 出 65M 的 `.o`；
// 那一档里预处理占得少、`tccgen` 占得多，所以上面这几刀在它身上只有个位数的效果。
//
