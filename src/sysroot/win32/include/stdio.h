/* `<stdio.h>` —— win32 的那一份（两个 arch 共用）（交叉编译用；形状沿用 Linux 那一套，见 ../README.md）。
 *
...
int fgetc(FILE *f);
/* `fgets` 回的是**指针**（实现在 `libc/file.c`）—— 不声明就按 `int` 收，
 * LLP64 上高 32 位当场丢掉（量到过 `implicit declaration of function 'fgets'`）。 */
