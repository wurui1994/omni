 ...
 * 次序就是规则 —— 认错人的账（矩阵那几格）大半出在这儿：
 *
 *   1. `errorcode` / `abstract` / `virtual` 这几个**只对函数有意思**的词写在函数体里：真身是
 *      "体里写了个函数原型"，与那个词无关 —— **先收下**往下走（第二百六十二刀 M-006、
 *      矩阵 `fn-body × method-abstract` M-007），由"声明符上带括号"那一句接。写在**没有形参表**
 *      的量上时才报"只能写在类的方法上"；
 *   2. 说明符那一句就没成时：把那几个名字**记下来**（第二百三十五刀）—— 否则后面每处用到它们
 *      又各报一句"未声明的变量"，一件事记成好几笔；
 *   3. `int property* p` 是一格**属性指针**（取/存两个函数 + 那个对象）：这一层没有那格类型，
 *      **不在这儿拦就会被悄悄降成普通指针**（P-003）；
 *   4. `:=`（引用初始化）不收；
 *   5. **声明符尾巴上那对括号**：类或结构体的变量时那是**构造实参**而不是形参表
 *      （第一百〇三刀 + 第一百二十九刀），凑不齐就记 S-003；别的类型挂不了构造实参 —— 明说，
 *      不然会被悄悄丢掉；
 *   6. `T v = T(实参…)` 与 `T v(实参…)` 是**同一件事**（判据是**同一个类型**：右边那个名字解出来
 *      正是左边声明的那一格）；不同类型的 `T(x)` 是构造式转换，另一笔账；
 *   7. `static` 是另一回事：程序启动时就分配好、初值**只跑一次**（第二十六刀，见
 *      `emit-global.js` 的 `staticLocalLines`）；
 *   8. 数组：`int a[3];` 就是一段长度 3 的零内存；`int a[];` 不合法（长度只能从花括号初值数出来）；
 *      `int b[3] = a;` 是**抄一份**（第二十一刀，与结构体同一档）。抄的时候**源头先钉住**——
 *      要在目标那一格之前发，`int b[3] = b;` 里右边那个 `b` 指的是外层那个。
 */
export const LOCAL_DECL_ORDER = [
  { name: 'fn-words', why: '函数专用的词写在体里 → 真身是函数原型，先收下（M-006 / M-007）' },
  { name: 'bad-specs', why: '说明符没成 → 把名字记下来，别把一件事记成好几笔' },
  { name: 'prop-ptr', why: '`int property* p` 是属性指针 → 拦住，别悄悄降成普通指针（P-003）' },
  { name: 'ref-init', why: '`:=` 引用初始化不收' },
  { name: 'ctor-args', why: '声明符尾巴上那对括号 = 构造实参（只有类与结构体收得下）' },
  { name: 'ctor-call-init', why: '`T v = T(…)` 与 `T v(…)` 同一件事（判据：同一个类型）' },
  { name: 'static', why: '`static` → 模块级那一格 + 只跑一次的闸门' },
  { name: 'array', why: '数组：长度必须知道；`= a` 是抄一份，**源头先钉住**' },
];
