...
/* fma —— 交集的**第二条例外**（ADR-0014 第十五节）。`Math.*` 里没有它，所以这一份是
   **手写**的，不像上面那些转手宿主。权威是 C 的 `omni_r_fma`（libm，一条 fmadd），
   这一份要与它逐字节对上；能这么要求是因为 fma 是**精确运算**（IEEE-754 5.4.1 只舍一次）。

   做法：Dekker 拆分求准确积（p + e == a*b），再 two-sum 把 p+c 的尾巴 t 收回来，
   最后 `s + (t + e)` 只舍一次。
   边界写在明处：`SPLIT*a` 在 |a| 接近 realMax 时会溢出，次正规上 e 也会损失 ——
   这两档与硬件 fma 不一致。要它的场合（几何界、Bezier 求值）都在正常量级里。 */
const $r_fma = (a, b, c) => {
  const p = a * b;
