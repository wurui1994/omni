...
export function multiShape(vals, env, ctx) {
  ...
  /* **记录也收**：记录在方言里就是**一格指针**（见 `shapeType`），摆进多值那格结构体里
     ...
     **数组也收**（2026-09-22）：数组在方言里是**句柄**（一格字），摆进结构体里与记录同理，
     而"里头改了外头看得见"正是 go 的语义（切片返回的是同一块底层数组）。
     判据：`tests/go/cases/41-multi-slice.go`。pt 的
     `Node.Partition(size, axis, point) (left, right []Shape)` 就是这一格。
     **字典仍旧不收**：方言的结构体字段不收 `(dict K V)`（`sexpr/lower.js` 那张清单里
     没有它）—— 量出来是 `字段 m6.v1：…这里是 dict<int,int>`。要收得先给方言加那一格。 */
  for (const t of types) {
    if (!isScalar(t) && !isRecType(t, ctx) && elemType(t) === null) {
      ctx.gap(`多值里有一格不是标量、记录或数组（量到的是 ${t}）`);
    }
  }
