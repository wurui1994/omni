...
// 第六批循环的早退（+1）、第七批表示转换（+1）、第八批切片（+1）、第九批 map 那四格（+4）、
// **第二十九批断言（+1）** —— 现在 **28 格**。
//
...
export const NODES = new Map([
  ...
  /**
   * `map-keys` —— **按键遍历那一格**（第三十批）。
   *
   * 三门语言都在同一句话上卡住过（`for k in m` / `for k := range m` / `for k in table`），
   * 尺子上是 16 份。**加的是这一格，不是一格新的循环**：列表的 for-each 在这一层
   * 本来就没有节点 —— 它由映射落成现成的 `counted`（region + bind + loop + `prim len`
   * + `index-get`，见 `fromtree.js` 的 `counted`）。map 于是同一条路：先要一格
   * **键的列表**，再走那一格 counted。这样"遍历"这件事全语言只有一份降级代码。
   *
   * 为什么不做成 `map-iter`（一格带游标的节点）：那要在图上引入"迭代器"这个概念 ——
   * 一格有状态的值 + 两个操作（next / done），而它的寿命与被遍历的表纠缠。
   * 一格**列表**没有这些问题，而且它已经是这层里的一等值。代价明写：**键要拷一份**
   * （O(n) 空间），而遍历中改表不会反映到那份拷贝上 —— 那恰好是几门语言都说"别这么做"
   * 的事，所以这个代价同时是一条纪律。
   *
   * **次序 = 插入序**，六条腿都是（这条是判据，不是实现细节）：
   *   * interp / js 用宿主的 `Map`，它天然保插入序；
   *   * wat 那张表是 `[cap][count][k0][v0]…` 按尾追加，扫一遍就是插入序；
   *   * c 那侧的 dict 同理；
   *   * 核心方言落 `(dkeys …)` -> 主语言的 `dict.keys()`，那一份也是插入序。
   * go 的规范说 map 的迭代次序是**随机**的 —— 也就是说"某个固定次序"是它的一个
   * 合法实现，我们不违反它；V 与 nim 各自的次序不由我们发明，所以取"我们的 dict
   * 本来就有的那个"。
   */
  N('map-keys', 'expr', [
    { name: 'obj', sem: SEM.value },
  ], {
    effects: ['reads'], lifetime: 'owns', family: 'map-new',
    doc: '一格 map 的键，**按插入序**排成一格列表 —— `for k in m` 那一族的降级靠它',
  }),
  ...
  // ---- 断言（1 格，第二十九批）：**"停下来"不是新的一栏** ------------------------
  //
  // 这一格是**账上算出来的**：`bench/tograph.js` 量到 V 自己的编译器里 578 份文件
  // 第一堵墙就是 `assert`（还没落成图的 1918 份里的 30%）—— 一门语言的一个语句形状
  // 挡着三成分母，那就该有一格节点，而不是靠映射糊过去。
  //
  // **为什么值一格节点而不是一格 `prim`**（这是唯一要辩的地方）：
  //   * **消息是它自己的一格端口**。`prim` 的实参是一串（`args`），分不出"哪个是条件、
  //     哪个是那句话" —— 而这两格的求值时机与用途都不同。
  //   * 它是**可以整格删掉的**（`-d:release` / `NDEBUG` 那一类开关）。"这一格能不能删"
  //     是**节点该声明的事**，不是名字表该猜的事：删一格 prim 得先知道那个内建有没有别的
  //     作用，而删一格 assert 只看它自己。
  //   * 反过来，它**没有**要新的效应栏：`may-early-exit` 就是"这儿可能不往下走了"、
  //     `writes` 就是那句话印出去 —— 两栏都是现成的。这一点正是"加一格节点"的代价上限。
  //
  // **语义（六条腿一个口径）**：条件成立什么都不做；不成立就**把一句话印在 print 那一格上、
  // 然后整个程序停下来**。"停下来"怎么落归后端（`effect` 那一问的答案），
  // 而**退出码不在图上** —— 图上没有那一格，所以判据比的是"那句话印出来了没有"。
  N('assert', 'stat', [
    { name: 'cond', sem: SEM.value },
    { name: 'msg', sem: SEM.value, optional: true },
  ], {
    effects: ['writes', 'may-early-exit'], outs: [],
    doc: 'V `assert cond` / `assert cond, msg`（578 份文件的第一堵墙）'
      + ' / mojo `assert cond[, msg]`（Python 形状）',
    // 规格里数出来**两门**：只有这两门的语法里 assert 是**一条产生式**。
    // 别的八门不在 spec 里，理由各不相同，都不是"还没接"：
    providers: {
      spec: ['vlang', 'mojo'],
      why: {},
    },
