...
const RT_OPS = new Map([
  ...
  // `(gfxframe PATH W H FB)`：把一帧交出去（runtime/omni_fmt.c 的 omni_gfx_frame）。
  ...
  /* `(gfxtex 槽 宽 高 层 格 数组)`：一张纹理交给设备（同一手：数组按指针过去）。 */
  ['gfx_tex.int', {
  ...
  ['gfx_def.string', {
    sym: 'omni_gfx_def', ret: 'double', params: ['[2 x i64]', '[2 x i64]', '[2 x i64]'],
  }],
