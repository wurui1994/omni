...
class Lower {  /** @param {import('../source/diag.js').Diagnostics} diags */
  ...
  module(program) {
    ...
    for (const g of groups) {
      ...
      this.topIdx = null;
      /* `isInit`：这一格装的是**模块级初始化**，不是普通函数。后端要认它 —— 插件那条腿上
       * 「给不属于自己的模块级变量赋初值」得包成"还没人初始化过才做"，而那件事从前只在入口里
       * 判（backend-c 的 initGuard 认 `inEntry`）。切开之后赋值都搬进了这些格子里，不标一笔
       * 就一格都不包：量出来是**插件把核心的语言表清空**，装到最后只剩两格语言。 */
      this.funcs.push({
        name: nm,
        mangled: nm,
        file: g.file,
