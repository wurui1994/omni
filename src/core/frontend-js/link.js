...
//     在 a9df4874 上就有 126 条）。现在**一片一片在还**，还到 **112 条**：
...
//     那两格建**图节点**的同名）· 再 6 格（都是"私有那一侧"最省的改法：`cliOpt` / `dirName` /
//     `jsAbiRet` / `libcPad` / `arStr` / `mirOpText`）。
//     **这几处的共同点是"同名却不同事"** —— `asList` 在 fromtree 里回 null、在 wat 后端里
//     回空数组，这种最该先改。
//     剩下的 112 条分三堆（数出来的）：**ext 那十一份映射 71 条**
