...
//     在 a9df4874 上就有 126 条）。现在**一片一片在还**，还到 **130 条**：
//     `graph/eval.js` 那 9 格（值上的运算 -> `valTruthy` / `valPick` / `valMap*` / `valSlice`
//     · 内部的 `one` / `Env`）与 `glr/ebnf.js` 那 3 格（`ebnfGrammarName` /
//     `quoteGrammarStr` / `EbnfRx` —— 与 `yacc.js` 同名却**不同规矩**，这种最该改）。
//     剩下的 130 条分三堆（数出来的）：**ext 那十一份映射 71 条**
...
//     走完自己就没了，现在改名是给移动靶子上漆）· 剩下 45 条是真正独立同名的。
