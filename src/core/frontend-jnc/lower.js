...
//   1. `JNC_RULES=1` 才走规则那条路（`src/core/lang/jnc.js:29`），默认仍是这一份 ——
//      翻默认的门槛不是语料，是 `tests/jnc/bad/` 那 140 份边界：规则那条路上还有
//      53 份"居然通过了"、79 份"拒的理由不同"（量法与数见 ADR-0031 §8 那一段）；
//   2. 那 140 份边界的**话术出处**就是这一份 —— 规则那一侧的墙一族一族照它搬
//      （`MOD_NOPE` 那张表是第一族）。
//
// 两堆清零那一天翻默认，再过一个宽限期删这一份。在那之前：**改这一份只为修错，不为加料**。
//
// ## 这一份存在的理由与 asy 那一份相同
//
