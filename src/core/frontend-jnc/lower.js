...
//
// ## @deprecated —— 这一份是**待弃用的回退**（第二百五十六刀起）
//
// 规则化那条降级（`src/lang/jnc/*`，ADR-0030 §3 / ADR-0031 §8）到第二百五十六刀
// 在**整份语料上与这一份等效**：199 份降得下来 199（100%）、逐字节比 `.expected`
// 一致 199 / 不一致 0、账 0 条。所以这一份不再收新特性 —— 新的一律落在规则那条路上。
//
// 它还留着，两个理由，都是**眼下还成立的事实**：
//   1. `JNC_RULES=0` 是回退闸门（`src/core/lang/jnc.js:29`）—— 默认已经翻到规则那条路
//      （第二百五十八刀）。留一个宽限期是为了"出事能一秒退回去"，不是为了两条并跑；
//   2. `tests/jnc/bad/` 那 140 份边界的话术出自这一份 —— 规则那一侧一族一族重画
//      （名单 `tests/jnc/bad/xfail.txt`，只许缩短）。对齐的靶子是 **jancy 的源码**，
//      不是这一份：这一份的话哪儿说得比 jancy 含混，就按 jancy 的原话改规则那一侧。
//
// 宽限期过了删这一份。在那之前：**改这一份只为修错，不为加料**。
//
// ## 这一份存在的理由与 asy 那一份相同
//
...
class JncLower {
  constructor(diags, opts) {
    ...
    /* 基类那一层的名字表（第一百三十七刀）：类 / 结构体的全名 -> { ns, 源码里写的基类名字 }。
       在 aggHoist 那一遍记（typeDecl 太晚，见那处注），解成全名留到 basePath。 */
    this.aggBases = new Map();
    /** basePath 自己要查名解基类名字 —— 一道闸门，别让它绕回来。 */
    this.inBase = false;
  ...
  * scopePath() {
  ...
  /**
   * `INH` 那一段：每一层上按声明顺序 BFS 走基类（第一百三十七刀）。
   *
   * `class D0: B0 { X m_v; }` 里那个 `X` 可能是基类的成员 typedef —— jancy 那边类**同时是
   * 一层命名空间**，而派生类那一层是接在基类那一层上的（`ClassType` 派生自 `Namespace`，
   * 查名沿 `m_baseTypeList` 上溯）。基类的名字解成全名要在**这时候**做（aggHoist 那一遍类
   * 还没登记上），所以这儿会回头查名 —— `inBase` 是那道闸门，免得绕回来。
   */
  * basePath() {
  ...
  unionMembers(ag, cls, owner, gidx, min = 2, bs = null) {
    ...
    for (const m0 of this.flat(ag.items[4])) {
      ...
      /* 带名字的 union 的体里也有**方法**（第二百五十三刀）：`io.SocketAddress` 的
         `isEqual` / `parse` / `getString` 就写在那儿（io_SocketAddress.jnc:440-465），而它们的
         体在宿主那边（jnc_io_SocketAddress.cpp 的 `JNC_BEGIN_TYPE_FUNCTION_MAP`）。jancy 那边
         `UnionType` 就是从 `StructType` 派下来的，所以"union 能有方法"与结构体是同一件事。
         这一遍于是要与结构体那一遍一样开着 `errorcode`（`bool errorcode parse(string_t) thin;`）。 */
      ...
      for (const d of this.flat(m.items[2])) {
        ...
        /* 声明符上带括号的是**方法原型**，不是字段（第二百五十三刀）：先前它悄悄进了字段表
           （`bool isEqual(…)` 成了一格 bool 字段 —— 静默的错布局），登记走的是结构体那一遍
           抽出来的同一份 `protoRegister`（宿主面：`(ccall SocketAddress_isEqual self …)`）。 */
        if (info.formals !== null) {
          this.protoRegister(owner, info);
          continue;
        }
        const t = info.type;
  ...
  /**
   * 一格**只有原型**的方法登记（第一百四十七 / 一百六十四 / 一百六十九刀那一套）。
   * 从结构体 / 类的成员那一遍里抽出来 —— 带名字的 union 那一遍（第二百五十三刀）要用同一份：
   * `io.SocketAddress` 的 `isEqual` / `parse` / `getString` 就写在一格 union 的体里
   * （io_SocketAddress.jnc:440-465），而它们的体在宿主那边（jnc_io_SocketAddress.cpp 的
   * `JNC_BEGIN_TYPE_FUNCTION_MAP`）。
   */
  protoRegister(name, info) {
