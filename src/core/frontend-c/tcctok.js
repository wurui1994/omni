...
const PP_NAMES = [
  ...
  // 的那几个：`aligned` / `packed`（改布局）、`weak`（改符号绑定，第一百〇四片）、
  // `alias`（一个名字发两条符号，第一百〇五片）与 `visibility`（改 `st_other`，
  // 第一百〇六片），各带 gcc 的下划线拼法。别的属性名仍然是普通标识符（`section`、
  // `format`…），`parseAttrs` 在 default 那一支把它们的参数括号平衡掉 ——
  // 与 tcc 的 `skip_param` 同一支。
...
/* `__attribute__((visibility("hidden")))`（第一百〇六片，`tcctok.h:118-119` 的前一对）：
 * 落在 ELF 符号的 `st_other` 上（STV_DEFAULT 0 / INTERNAL 1 / HIDDEN 2 / PROTECTED 3）。 */
export const TOK_VISIBILITY1 = fixed('visibility');
export const TOK_VISIBILITY2 = fixed('__visibility__');
