 ...
 *
 * 两条路，都没走：
 *   1. **把入口拆开**：每个模块一格 `omni_init_<模块>`，入口按链接序挨个调 ——
 *      5085 条摊到 119 个模块上是每格 40 来条。模块级的名字本来就已经是全局
 *      （所以函数体引得到它们），所以搬的只是"初始化那几句"。这一条与 JS 的模块语义
 *      也更像，是推荐的那条；代价是所有 JS 前端产物的形状都变，一堆快照要跟着动。
