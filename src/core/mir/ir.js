 ...
 *
 * 两条路：
 *   1. **把入口拆开**：每个模块（按源文件的连续段）一格 `omni_init_N`，入口按链接序挨个调 ——
 *      那份编译器切成 87 段，最大一段三万个 AST 节点上下，都在上界以内。模块级的名字本来
 *      就已经是全局（所以函数体引得到它们），所以搬的只是"初始化那几句"。
 *
 *      **试过，退回来了**（提交与 revert 都在 git 历史里）：这么切之后 `tests/mir` 的
 *      `lower/cli.js` 确实绿了，别的轴一处快照都没动，可 **`omni bootstrap` 那一步红了** ——
 *      建出来的编译器 12 格插件全都 dlopen 成功，却只有 `jnc / wat` 两门语言登记上，于是
 *      编 `cli.js` 报「不认识这种扩展名」。也就是说**插件的注册是从模块级初始化那几句里来的**，
 *      而它们一被搬进 `omni_init_N` 就没跑（或没被留住）。清过缓存重来一样红；退回之后
 *      `bootstrap` 立刻回到 103/0。下一次动它之前先把"插件那条路上模块初始化是谁在跑"
 *      弄清楚（`backend-c/emit.js` 的 `omni_plugin_init` 只调 `mod.entry()`，可疑的是
 *      摇树或 `once_` 那一格与新形状的配合）。
