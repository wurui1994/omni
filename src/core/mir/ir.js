 ...
 *
 * 两条路里走了第一条（**已落**，见 frontend-js/lower.js 组装入口那一段）：
 *   1. **把入口拆开**：每个模块（按源文件的连续段）一格 `omni_init_N`，入口按链接序挨个调 ——
 *      那份编译器切成 87 段，最大一段三万个 AST 节点上下，都在上界以内。模块级的名字本来
 *      就已经是全局（所以函数体引得到它们），所以搬的只是"初始化那几句"。这一条与 JS 的
 *      模块语义也更像。落完 `tests/mir` 的 `lower/cli.js` 就绿了，别的轴一处快照都没动。
 *   2. **把 ref 加宽**（16 位 -> 32 位）：MIR 的字节、摘要哈希、四个后端一起改，
 *      而换来的只是"一个更大的上界"，没解决"入口无限长"这件事 —— 所以没走。
 *
 * C 与 JS 那两条腿本来就不受影响（它们不经 MIR）。**下一堵墙不在这儿**：
 * `emit-llvm src/core/cli.js` 现在过了这一关，停在「llvm 后端目前不支持 dyn」——
 * 那是 ADR-0014 第一阶段"只降标量"的边界，是另一件事。 */
