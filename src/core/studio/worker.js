 ...
 *   3. **枪不由我领，预算照旧算**。`OMNI_CLI_NO_SHOT=1` 只关掉"到点给本进程一枪"那一格
 *      （本进程是池子的常驻工人，被打死就得重开）；超时由池子数着、到点杀工人。
 *      **预算本身留着** —— 从前这儿写的是 `OMNI_TIMEOUT=0`，可那个环境变量会跟着孩子走：
 *      `spawnSync` 没了时限、生成出来的程序里那格 SIGALRM 与外部看门狗也一起关掉。
 *      量出来的后果（2026-09-26）：一个 GUI 例子在 Studio 上跑了 28 分钟还在，
 *      父进程早就没了（孤儿，只能手动 `kill -9`）。三层保护一格都不能少。
 */
