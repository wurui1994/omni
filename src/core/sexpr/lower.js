...
const RMATH = new Map([
  ...
  ['nextafter', 2],
  /* 第二条例外（ADR-0014 第十五节）：fma 在 C99 math.h 里有、Math.* 里没有。收它的理由
   * 与 nextafter 同类：参考实现（asy）是 C++ 编译出来的，a*b + c*d 这类式子在 arm64 上
   * 被收缩成一条 fmadd（一次舍入），要与那份字节对上就得有单次舍入的 fma。
   * 权威是 C 的 libm（omni_r_fma）；JS 那侧手写（Dekker + two-sum），能要求逐字节是
   * 因为 fma 是精确运算（IEEE-754 5.4.1）。 */
  ['fma', 3],
]);
...
class CoreLowerer {
  ...
  expr(n) {
    ...
    //      指针（`#gfx <种类> <路径> <宽> <高>`）—— 几百万次 putpixel 不该变成几百万行输出。
