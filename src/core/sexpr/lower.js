...
const RMATH = new Map([
  ...
  ['nextafter', 2],
  /* **第二条例外**（ADR-0014 第十五节）：`fma` 在 C99 math.h 里有、`Math.*` 里没有。
   * 收它的理由与 nextafter 同类：参考实现（asy）是 C++ 编译出来的，`a*b + c*d` 这类
   * 式子在 arm64 上被收缩成一条 `fmadd`（一次舍入），要与那份字节对上就得有单次舍入的
   * fma —— 分两步算差最后一位，而「一次舍入」用别的算符做不出来。
   * 权威是 C 的 libm（omni_r_fma）；JS 那侧手写（Dekker 拆分 + two-sum），
   * 能要求逐字节是因为 fma 是**精确运算**（IEEE-754 5.4.1）。 */
  ['fma', 3],
]);
