// src/core/lower/int.js —— **公共降级器的整数规则**（ADR-0044，搬自 src/lang/common/int.js）
//
// 整型提升、常用算术转换、"算完就掩"、无符号那一格换读法 —— 这四条是 C / C++ / jancy / Zig
// 共同的规矩，C 前端那一侧同样在做（`frontend-c/ctype.js` 的 `VT_*` 位与 `cconst.js` 的
// `asIntN`）。所以它的家在公共这一层（ADR-0031 轴 B），jnc 那边只留一格转口。
//
// 出处是旧降级的 `wrapTo` / `wrapVal` / `realOf`（`frontend-jnc/lower.js:780-811`）：
// 尺子上量出来 46 格函数体全卡在它上面（`(ret (bin "+" (var x) (var y)))` 对
// `(ret (bin "-" (bin "^" (bin "&" … ) …) …))`）。
//
// **为什么现在要掩**：方言只有**一格** int（64 位有符号），所以四种位宽靠"算完回卷"体现：
...
//
// **这一整摊是暂时的**：方言长出带宽度的整数之后（ADR-0031 §8.1），每一处 `wrapTo` 都塌成
// 一格 `trunc` / `sext`，`intConvCode` 那四条"要不要发字"的判断整块消失 —— 塌的时候只改这一份，
// 两门语言一起跟着变。这就是把家搬到公共层的理由。
...
export function realOf(code, w, u) {
  return sx.op(u === true && w >= 64 ? 'torealu' : 'toreal', code);
}
...
export function intUnary(op, a) {
  ...
  if (op === '-') {
    return { code: wrapTo(sx.un('-', a.code), rt.w ?? 32, rt.u === true), type: rt };
  }
  if (op === '~') {
    const code = sx.bin('^', a.code, sx.int(-1));
