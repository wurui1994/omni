// Omni stage0 — 宿主里**不问操作系统**的那一半（封闭 ABI 的纯计算部分）
//
// `native.js` 是封闭 ABI 的 node 那份实现（ADR-0011 决策 2 与 17），52 个导出里有 15 个
// **一行 node 都不碰**：32 位整数运算、dynamic 的标签、real 的六种排版、间接 eval、
// 按名字调一条 op、函数值那两格。它们进 ABI 的理由不是"要问操作系统"，而是
// **方言表达不出**（`Math.imul` / `eval` / `instanceof Map` 都不在自举的子集里）。
//
// 单独摊成这一份的理由只有一条：**浏览器那条腿（`host/browser.js`）要用同一份**。
// 手抄第二份 `fmtFixed` 等于给"同一个 double 打印出同一串字符"多开一条会分叉的路 ——
// 而那正是 `native.js` 头注里说的那件事。于是这儿是**一份实现，两条腿共用**。
//
// 这份文件**不在 ABI 表上**：`frontend-js/link.js` 认的是"从 `core/host/native.js`
// 导入的名字"，所以引用方照旧写 `./host/native.js`，`native.js` 把这几格转手出去。
// 换句话说，这一刀对编译器本体与降级那一路是**零影响**。
//
// callJsOp 靠下面这两个 import 把 prelude 当自己的实现跑起来。
