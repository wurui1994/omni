...
`Object.hasOwn` 护栏。** 第三条的教训是闸门的**范围**：它是把幂等从 `stage0/src` 扩到
...
3. ✅ JS 前端第一版（`stage0/src/frontend-js/`：词法器 + 语法分析器 + 生成器），
   `stage0/src` 全部文件解析通过，未覆盖的语法一律报错
...
6. JS AST → OIR 的降级，然后打通 C0 → C1 → C2，验不动点。
   **这一步还没开始**，而且拦路虎不在前端：编译器源码要用闭包 / 函数值、异质记录对象、
   异常，以及一批宿主库设施，Omni 目前都没有（清单在 `docs/js-bootstrap-subset.md`）。
