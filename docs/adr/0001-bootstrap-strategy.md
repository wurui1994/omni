...
2. **字符串拼接不能退化成 O(n²)**。编译器最热的模式是 `out.push(piece)` 然后 `join`；
   如果前端把它降级成一串 `omni_str_cat`，每次都重新分配加拷贝，代价是平方级。
   需要一个 builder（可变字节缓冲），这是自举前必须补的一件事。
3. **分配用 arena**。编译器是批处理进程：读源码、产出 C、退出。整个生命周期只需要
   "分配、永不单独释放、退出时整体丢掉"。这比先上 ARC 简单得多，而且更快。
   ARC（ADR-0007）留给"用 Omni 写长期运行的程序"那一天。
4. `-O2`。不做 LTO（tcc 没有）。
...
2. JS 前端第一版：覆盖上面量出来的子集，先能解析 `stage0/src` 全部文件
3. `tests/js-roundtrip/`：幂等 + 语义一致两条断言
4. 字符串 builder（第 4 节第 2 项）与 arena（第 3 项）—— 自举前必须有
