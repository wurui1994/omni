...
> **一段绕路的记录**（2026-09-20）：第一版落地时接口记录发的是**值语义的结构体**
> （`byval`），不是类 —— 那时后端把引用语义的记录发成 `(ptr rN)`，而方言的
> `(arr 元素)` 不收 `(ptr rN)`，于是 `[]Shape` 落不下去。`40fb337b` 把引用语义的
> 记录改成发**方言的类**之后那条约束没了，接口记录也跟着回到本文定的形状。
> 回到类的收益是**两处**：`(arr If_Shape)` 与 **`map[string]If_Shape`**（方言的
> `(dict K V)` 的 V 收类、不收值语义的结构体）—— 后者是 pt 的 `textures` 那一格。
> 这么改**语义上是中性的**：接口值在装箱之后没人往里写字段，所以"抄一份"与
> "共用一个句柄"观察不出差别（go 那侧能观察出差别的 `==` 我们本来就不支持）。
