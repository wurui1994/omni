...
**real 上的数学函数**（核心方言的 `(rmath "NAME" A [B])`，ADR-0014 决策 1）：白名单只有
`sqrt` / `fabs` / `floor` / `ceil` / `round` / `pow` / `fmod`，判据是"五条腿逐字节相同"这条
铁律（`tests/sexpr/cases/01-core.sx` 钉着）—— 宿主的 libm 与 V8 不是同一份实现，量过
`atan`/`tan`/`log`/`cos` 在最后一位（1 ULP）
就分叉，所以超越函数（exp/log/trig）一个都不收，见到就报错。收进来的那七个是量过相同的：
`sqrt` 由 IEEE-754 规定唯一正确，`fabs`/`floor`/`ceil`/`round`/`fmod` 是精确运算，`pow` 用
位精确字面量跑了 80 组随机输入 0 差异。
- `round` 是 **C 的离零舍入**：`round(-2.5) == -3`。JS 的 `Math.round` 会给 `-2`（它是
  向正无穷舍入），所以 JS 那两条腿自己绕过去，不是直接转发。
- `fmod` 的符号跟**被除数**一致：`fmod(-7.5, 2) == -1.5`，`fmod(7.5, -2) == 1.5`。这是 C 的
  余数语义，不是数学上的模。
- 参数与结果都是 `real`；要 int 结果就在外面套 `int(...)`（`floor` 之类**不**自己回 int，
  那是各语言前端的约定，不是这一层的）。
