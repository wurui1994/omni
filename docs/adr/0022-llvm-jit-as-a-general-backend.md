...
- **J6 会话进编译器进程**：新增 ABI（`jit_open/jit_add/jit_map/jit_lookup/jit_call_i`），
  ...
  **1 与 3 已落，而且是真跑出来的**（`tests/llvm/run.js` 第 10 节 `session-aot`）：
  三批会话产物各编一份 `.ll`，加一个只管按顺序调 `omni_chunk_1/2/3` 的小驱动，
  一起链成一个可执行文件 —— 印出 `100 / 101 / 202`，也就是"第二批改第一批的变量、
  第三批还读得到"。这一节不碰任何宿主 ABI，正是上面那句"倒过来"的兑现。
  还剩 2（跨批**调函数**）与 4（撤一批）。
