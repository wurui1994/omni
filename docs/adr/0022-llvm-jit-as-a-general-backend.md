...
- **J6 会话进编译器进程**：新增 ABI（`jit_open/jit_add/jit_map/jit_lookup/jit_call_i`），
  ...
  **J7 之后这一格的理由要重写一遍**（量出来的）：那 220ms 是 J7 之前的数字。现在宿主
  一次冷启动 —— 进程 + LLVM 初始化 + 摆一份缓存好的对象码 + 调 main —— 在一份最小的 IR
  上是 **10ms**（`int main(void){return 3;}`，对象码命中；生成那一次是 810ms，那正是 J7
  存下来的东西）。全程 `run-jit` 的 140ms 里几乎全是 **node 启动加编译器自己那一遍**，
  与 JIT 无关。所以：
  - **"省掉 spawn"这条理由只值 ≤10ms**，为它加一条"按 ptr 间接调用"的封闭 ABI（决策 5
    说过那是单独一节的论证）不划算；
  - **真正还站得住的理由是 REPL**：一格**会话**（同一个 JITDylib 上反复 add/lookup、
  ...
  **REPL 要什么（挨个试出来的，不是想出来的）**。REPL 的引擎接口只有两句
  ...
  1. **会话的顶层变量现在不是全局**。`(let base int (int 100))` 在 REPL 里降成的是
     ...
     **已落**：会话里的顶层 `(let …)` 提成一格模块级变量，语句只剩赋值；MIR 上多了
     `globalLink`（`'def'` = 这里定义、符号给后面几批看，`'ref'` = 别人家定义、
     这里只声明），LLVM 那条腿于是发 `@g_base = global i64` 与
     `@g_base = external global i64`。"声明哪几条"按**这一批提到过**的算
     （降级器的 `gUsed`）—— 全发出来的话每批的字节数会随会话长度涨，那正是 REPL
     增量要钉住的东西。整程序那条路一个字节都没变（`shared` 是假的、`externGlobals`
     是空的）。
  2. **MIR 的 CALL 是模块内的函数下标 —— 闭世界**。第二批调第一批定义的 `f1`：
     ...
     **已落**：MIR 的函数上多了一位 `decl`（"正文在别的产物里，**调用约定还是我们
     自己的**"）—— 与 `extern` 的差别正在这里，那一条是 C 那边的符号、调用点按 C 的 ABI
     摆实参、签名从调用处收，而 `decl` 的形参与返回值都在 MIR 上，调用点与模块内的调用
     一模一样，后端只是发 `declare` 而不发 `define`。声明哪几条按**这一批调过**的算
     （降级器的 `fnCalled`）。
  3. **每一批都发一个 `define i32 @main`**（还带 `omni_host_init`/`omni_run_entry`
     ...
     **已落**：`emitLlvm(mir, {repl: true})` 就是这一格，差别只有"不发那个包装"。
  ...
  **1、2、3 都已落，而且是真跑出来的**（`tests/llvm/run.js` 第 10 节 `session-aot`）：
  四批会话产物各编一份 `.ll`，加一个只管按顺序调 `omni_chunk_1..4` 的小驱动，一起链成
  一个可执行文件 —— 印出 `100 / 101 / 202 / 102`，也就是"第二批改第一批的变量、第三批
  还读得到、第四批还调得到第一批定义的函数"。这一节不碰任何宿主 ABI，正是上面那句
  "倒过来"的兑现。

  **JIT 那条腿上也验过了**（`tests/jit/run.js` 的 4g `session`）：宿主多一个 `--add
  FILE.ll`，把好几份 IR 摆进**同一个 JITDylib**，`--call` 按顺序叫每一批的入口 ——
  同样这四份 IR，答案与 AOT 那条腿逐字节相同。它逼出宿主那侧两处必须改的地方：
  - 宿主符号的定义与"表里没有的"那一遍扫描要**跨整批做一次**。一格名字只能定义一次，
    而 `omni_print_int` 这种每批的 IR 里都有一句声明 —— 分批调就是重复定义；
  - 扫描那一遍要认出"落点在这一批**别的模块**里"（`omni_jit_defined_in`），否则第二批的
    `@g_base`/`@s_f1` 会得到一句假的 `unresolved`，而 ORC 自己一查就找到了。

  还有一条边界顺手钉住：`--objcache` 的键是**一份** IR 的内容（J7 里由调用方算），
  与 `--add` 一起用等于让几份 IR 抢同一个文件 —— 明着报错，不留静悄悄错的组合。

  **还剩 4（撤一批）**，而那一格只有在真的常驻会话里才有意义（进程一退什么都没了）。

  动 4 之前先把一件事量准：**今天没有一条腿允许在会话里重新定义一个函数**。核心方言那侧
  第二批再写一遍 `(fn f1 …)` 在降级时报 `'f1' 重复定义`；omni 那侧走检查器，报的是
  `redefinition of 'f' with the same parameter types`，而且**那一批不留痕迹** —— 后面
  再调 `g(10)`、`f(10)` 印的还是旧那份的答案（量出来两句都是 11）。所以这不是"两条腿
  对不上"，而是一格**没做的功能**：`InterpSession.install` 那句"同名重定义按后来者算"
  说的是引擎层容得下，前端压根没放它过去。

  也因此 4 的题面是**重定义**，而"撤一批"只是它的一种落法：
  - 语义得先定。要"旧代码也看见新的身体"（JS 那侧全局改名就是这个效果）就只能**加一层
    间接**：会话里定义的函数，调用点走一格函数指针全局，定义它的那一批往里存新地址。
    MIR 上不必发明东西 —— `CALLI` 与取函数地址那两条已经在了（ADR-0017 第六刀第十三片），
    这条路子还有个好处：JIT 那侧根本不需要 `ResourceTracker`，重定义就是往同一格全局里
    换一个指针，AOT 那条腿也照样成立。
  - 要"新代码用新的、旧代码用旧的"（快照语义）则每次定义换一个符号名就够，但那与
    "REPL 里改一个函数再试一次"的直觉相反 —— 前一批的调用点会永远停在旧身体上。
  - 只有想省掉那层间接、又要"旧代码看新的"时，才需要 `ResourceTracker` 把旧那批撤掉重摆；
    而一批里还定义着别的东西（它的 chunk 入口、它的全局），整批撤掉太粗 —— 这正是
    "撤一批"作为**手段**并不合身的地方。

  推荐第一条（间接，只作用在**会话**产物上，整程序那条路一个字节不变）。不管选哪条，
  前端与后端要一起改：落成"前端允许了、JIT 报重复符号"的半格是最坏的结果。

  **已落，走的就是第一条**（`src/core/sexpr/lower.js`）：会话里的每个函数配一格函数指针
  全局 `fp_<名字>`，定义它的那一批在自己入口里存一个 `(fnref …)`；`(call …)` 在会话里
  发的是"读那格全局 + `CallFn`"。于是重新定义只是往同一格全局里换一个指针 —— 旧代码
  跟着换身体，JIT 那侧不需要 `ResourceTracker`，AOT 那条腿也照样成立。两代的正文各有
  自己的符号（`s_f1` / `s_f1__2`），适配器与那格单件也跟着带代号。签名变了要报错
  （前几批的调用点是按旧签名查过的）；`fnGen` 进了会话的快照，失败的一批不许把代号往前推。

  落这一格时踩到两个坑，都在别处：
  - 零初始化那几句紧跟在入口最前面，`fp_f1 = NullFn` 会把刚存进去的适配器冲掉 ——
    量出来是一句 `call of a null function value`。所以那几句赋值要**插在零初始化之后**。
  - `from_oir` 从前按**下标**取闭包记录（`closures[src.closureId]`），而闭包 id 是降级器里
    全局递增的、会话的一批 delta 只带这一批的记录 —— 第二批里 id 2 的记录下标是 0。
    现在 MIR 的闭包记录带上 id、按 id 查（没有 id 的前端还是按下标，逐字节不变）。

  判据（两条腿上都真跑出来）：`tests/llvm/run.js` 第 10 节与 `tests/jit/run.js` 的 4g 都扩到
  **五批** —— `100 / 101 / 202 / 11 / 1000`。最后那两行是关键：第四批调第一批定义的 `g1`
  得 11，第五批把 `f1` 换成 `*100` 之后，**第一批里那句 `g1` 的正文**也走到新身体上，得 1000。

  顺带一条要说清的：这一格**顶掉了第二件事在会话里的用处** —— 跨批调函数现在走函数指针
  全局，不再发按符号名的 `declare`，所以 `MirFunc.decl` 在 sx 会话这条路上暂时没有用户了
  （它仍是"直接跨模块调用"该有的机制，`sig` 那条分文件的路要它）。
- **J7 对象码缓存 —— 已落**（`--objcache PATH`）。ORC 是惰性物化的，真正花时间的是"查地址"
  ...
  **全程也量了一遍**（`node src/cli.js run-jit tests/cases/01_basics.omni`，含 node 启动 +
  前端 + 后端 + 宿主）：miss 0.20s、hit 0.14s。所以 J7 在真实那条路上切下了大约 60ms，
  剩下的 140ms 基本是 **node 启动加编译器自己那一遍** —— 那正是 J6（会话进编译器进程）
  要对的东西。这条数据也说明 J6 排在 J7 后面是对的：先把能纯函数化的那一半存起来，
  再去动进程模型。
