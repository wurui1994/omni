...
- **J6 会话进编译器进程**：新增 ABI（`jit_open/jit_add/jit_map/jit_lookup/jit_call_i`），
  ...
  **还剩 4（撤一批）**，而那一格只有在真的常驻会话里才有意义（进程一退什么都没了）。
  ...
  动 4 之前先记一条量出来的事实：**核心方言的会话现在根本不许重新定义一个函数** ——
  第二批再写一遍 `(fn f1 …)` 在降级时就报 `'f1' 重复定义`（解释器那侧倒是写着"同名重定义
  按后来者算"，两边对不上）。而"改一个函数再试一次"是 REPL 里最常做的事。所以 4 的题面
  比"撤一批"大一圈，是**重定义**：前端要允许后来者盖掉前者，JIT 那侧同一个 JITDylib 里
  两份 `s_f1` 是重复符号 —— 得靠 `ResourceTracker` 把旧那批撤掉（或者每批一格 JITDylib、
  按链搜索）。这两件事要一起定，不然会做出一个"前端允许了但 JIT 报重复符号"的半格。
