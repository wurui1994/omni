...
剩下的一副是 **c67**：账在我们编 `c67-gen.c` 这一步，最小的复现是
`long long f(long long a){return a<<3;}` —— 32 位目标上的 64 位移位走运行时的 `__ashldi3`，
那次调用的参数落位上差一个 c67 寻址模式位（我们发 mode 0、clang 那份发 mode 1）。
先记在账上，找那个构造是下一片的事。
