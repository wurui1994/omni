...
10. **arm64 代码生成器 + 内存里执行**（`omni run x.c` 在没有 cc 的机器上跑起来）——**前四片已落地**（整数与结构化控制流；调用与递归；浮点与它的调用约定；线性内存。每个值一个栈位、内存基址钉在 x28，用 `.incbin` + clang 链接后在真机上**跑起来**验的），见下面的第九刀第四到七片节。
...
**线性内存。**`MLOAD`/`MSTORE` 的十五种宽度符号落成六条 arm64 存取。
`tests/arm64/from-mir.js` 到 61 条。

### 一、基址钉在 x28 上

MIR 的线性内存地址是**从 0 起的字节偏移**，而真机上那片字节在哪要到运行期才知道。
三条路：

1. 每次访问都从一个全局符号里读基址 —— 多一次访存，而且每处都欠链接器一笔重定位；
2. 把基址当隐含形参层层传 —— 改掉所有函数的签名，与外部 C 函数也就对不上了；
3. **钉一个寄存器**。

选 3，钉 **x28**。这不是我们的发明：wasm 的几个引擎都把内存基址钉在一个寄存器上。
x28 是**被调用者保存**的，于是两件事同时成立 —— 模块里的代码从不写它（不必在序言里
存一份），调用外部 C 函数回来之后它还在（ABI 保证）。

「进模块的代码之前把 x28 设好」是**驱动**的事（第 10 步「在内存里执行」的那一半）。
测里用一段十三条指令的手写蹦床代劳，它同时也是这条约定的说明书：

```
_omni_thunk:                 // (fn, base, a, b)
  stp x29, x30, [sp, #-32]!
  mov x29, sp
  str x28, [sp, #16]         // 调用者的 x28 要还回去
  mov x28, x1                // 内存基址
  ...
  blr x9
  ldr x28, [sp, #16]
...
`MSIZE`/`MGROW`（没有运行期，谈不上扩容 —— 这两条要等驱动那一半）、越界检查
（wasm 的语义是运行期错误；现在越界就是真越界，与 C 一样）、指针那一族（`PLOAD` 等，
不过 C 前端根本不发它们：C 的一切都落在线性内存上）。

到这里，**C 前端发得出来的 MIR 里，除了外部符号的调用，arm64 后端全认了**。
`&x`、`memcpy`、影子栈、struct 的字段、数组的下标 —— 都是 `MLOAD`/`MSTORE` 加一条
`add`，这一片全落地了。
