...
6. **路径 B 的一遍过降级器**：声明、类型、表达式（值栈 `vtop` 的等价物）、语句、
   ...
   **前九片已落地**（第一片 `int`/`void` + 表达式 + 语句 + 自定义函数；第二片整型的宽度
...
- **`goto`**（`gen-bad/goto` 钉着）：嵌套 block 那一招吃得下 switch，是因为它的目标都在
  **同一个方向**（往外跳）。goto 可以往回跳、可以跳进别人的作用域 —— 那要一套
  「循环 + 状态机」的改写（relooper 那一路），是独立的一片。
