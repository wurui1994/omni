...
11. **x86_64 同两步**，然后 **Mach-O / ELF 写出**，然后**内联汇编**——Mach-O 的目标文件写出**已落地第一片**（`MH_OBJECT`、`__TEXT,__text`、`ARM64_RELOC_BRANCH26`；`CCALL` 落成 `bl <符号>`，clang 能把我们的 `.o` 与 libc 链起来跑），见下面的第九刀第八片节。
