...
200 份语料**降得下来 200（100%）、行为一致 200 / 不一致 0**，账 0 条。**默认那一格已经翻到
规则这条路**（第二百五十八刀，`JNC_RULES=0` 退回旧那条）—— 旧降级 `src/core/frontend-jnc/lower.js`
转 `@deprecated`：不再收新特性，只当回退。`tests/jnc` 224 passed / 0 failed / 128 xfail
（xfail = 旧降级画的那些墙在规则这条路上还没重画，名单 `tests/jnc/bad/xfail.txt`，只许缩短）。
前一轮接上的几族
...
- **反应器**（第二百五十六刀，82-reactor.jnc / 162-oneventlist.jnc，出处 `Reactor.cpp`）：
  `reactor r { … }` 里**每一句是一格反应函数** `$r<k>`（`onevent` 那几句是 `$e<k>`），
  外头包一格 `(if $on …)`；`r.start()` 是"绑一遍 + 把每一格跑一遍"（绑的是**静态超集**，
  绑一次就够 —— 按 `bindingof` 那格多播 `+= $r<k>$clo` 的闭包壳），`stop()` 把 `$on` 落下来、
  `restart()` 是停了再起。**四种写法一张表**：类里声明+类外带体、类里就带体、顶层带体、
  只声明没有体（那是一句错）。逐运行重收依赖（jancy 真做的那件事）明说不收。

**等效之后先量的是"墙"，不是"新特性"**（第二百五十七刀起）。语料 199/199 只说明**该降的降对了**，
它一句也没说"不该降的拦住了"。把 `tests/jnc/bad/` 那 140 份边界拿规则那条路量一遍（量法：
`JNC_RULES=1 omni run bad/<名>.jnc`，看退出码与那句话里有没有 `.expected` 那一串），结果分两堆：
- **53 份"居然通过了"** —— 边界没有墙。里头 4 份是**丢一个词就静默给错答案**：`threadlocal`
  变普通局部量、`async` 的返回类型从 `std.Promise*` 变 `int`、`C1 weak*` 永远不为 null、
  `disposable` 的 `dispose` 一次不调。根由是修饰词这一族先前**读到才算**（表里没有的词
  不说一句地丢），所以这一刀落的是一张 `MOD_NOPE` 表 + 整棵树走一遍的闸门
  （`src/lang/jnc/types.js` 的表、`lower.js` 的 `gateMods`）：一处拦住、漏不了，
  话与旧降级那一侧逐字同一句。余下 49 份要一族一族分：真接上了的**升进 cases/**（带
  `.expected`），没接上的补墙；
- **79 份"拒的理由不同"** —— 拒是拒了，可说的是规则那条路自己那句账，不是那条边界的话。

**所以默认那一格翻了**（第二百五十八刀）：`JNC_RULES=0` 才退回旧那条。翻的依据不是"两堆清零"
（那是迂腐的一致），是**规则这条路已经严格更强**：整份语料等效、`bad/` 那两堆里"居然通过"的
一多半其实是它**多收了的特性**（位域的类、`threadlocal`、枚举那几族…）。所以尺子跟着换了口径：
`bad/` 那 140 份里对不上的记成 **xfail**（名单 `tests/jnc/bad/xfail.txt`，一行一个理由，
**只许缩短** —— 拦对了 run.js 会当场叫你删掉那一行）。一族一族往下走：真接上了的**升进
`cases/`**（第一格是 `cases/200-threadlocal.jnc`），该拦的把话按 **jancy 的原话**重画。

**对齐的靶子是 jancy 的源码，不是旧降级**。旧降级 `src/core/frontend-jnc/lower.js` 已经
...
`(ccall Owner_… self …)`，ADR-0022 的 J4b/J4d），规则那条路上还没接。接上那一族就把那个
参数删掉。

**真语料那把尺子**（`node tests/lib/jnc-rules-sweep.js --jancy`，662 份 `.jnc`）：
自家那 200 来份是挑出来的最小例，"还差哪些特性"只能从真语料上问。今天 662 份里
**降得下来 151（22.8%）**、账 205 条，榜首那几族是：跨文件/宿主面的被调（688）、
字段的基类型（513）、聚合体（330）、作用域图（207+73）、返回类型（179）、`async`（35）。
`--libs` 那一档（把标准库也放进 import 搜索路径）今天是 0/662 —— 那是"连库一起降得下来"的
收尾数，它要等库自己那几族（async / scheduler / dynamic throw）接上。
