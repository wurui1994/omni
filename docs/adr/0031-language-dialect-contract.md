...
199 份语料**降得下来 199（100%）、行为一致 199 / 不一致 0**，账 0 条 —— 到这一格，
规则化那条路子与旧降级**在整份语料上等效**（旧降级 `src/core/frontend-jnc/lower.js` 因此转"待弃用"）。
前一轮接上的几族
...
**所以默认那一格还没翻**（`JNC_RULES=1` 才走规则那条路）：翻默认的门槛不是语料，是这两堆
清零。旧降级 `src/core/frontend-jnc/lower.js` 此刻的身份是**待弃用的回退**（`@deprecated`），
它仍是 `bad/` 那 140 份边界话术的出处。
