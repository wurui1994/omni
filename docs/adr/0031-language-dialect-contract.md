...
199 份语料**降得下来 84（42.2%）、行为一致 84 / 不一致 0**，账 68 种。这一轮接上的几族
...
作用域图 23、认不出基类型 17（其中 8 是泛型）、跨文件的被调 17、基类上的同名方法 10。
剩下的大族按"下一刀"排：**重载挑一格**（函数 8 + 方法 6，旧降级发 `q$o1` / `q$o2`）、
位域、虚派发、反应器与事件、`bindable` 生成的取存、泛型。
