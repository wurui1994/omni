...
199 份语料**降得下来 79（39.7%）、行为一致 79 / 不一致 0**，账 67 种。这一轮接上的几族
（各带出处）：静态字段与成员属性、`basetype.…`、合成出来的构造（基类 / 字段初值 / 内嵌对象）、
聚合体按值抄一份与 `operator :=`、花括号初值按格子写、点串的类型名、`countof`、
`'a'` 是一个整数、`a.g` 是命名空间里那一格、静态方法、完整声明式的属性。

**账不许转手**：外面那几层（printf / 赋值 / 调用 / 取字段…）先前一律补一条"这一格还拼不出来"，
把真原因盖住了 —— 现在里头记过就闭嘴（`acctSeen`）。于是尺子上的前几堆是真原因：
作用域图 19、认不出基类型 17、跨文件的被调 14、局部量声明 12。剩下的大族：
`autoget` / `bindable` 生成的取存、位域、重载挑一格、虚派发、反应器与事件。
