...
- `未声明的变量` `37 -> 69`（**+32**）—— 墙后面那一堵：算符的体现在真降了，于是体里的 `m_p`
  （26 处）与 `p0`（8 处）撞上了**另一件还没长出来的事** ——`IteratorBase<P>` 那格
  `P readonly m_p;` 的 `readonly` 走的是"指针后面的修饰符"那条路，字段从来没登记上。

一句话：这一刀把 `postfix` 那道门打开了，门后是 `readonly` 字段那道门。总数涨的 16 条全是
**本来就欠着、先前被挡在门外看不见**的账 —— 记在这儿，下一刀的选题就有出处了。
