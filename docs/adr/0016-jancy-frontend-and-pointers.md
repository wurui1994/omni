...
"那份拷贝"要一格临时内存 —— 那是**一条语句**，而这一层早就有落点：第五十八刀给 errorcode 传播开的
`ecOut`（`ecScope` 在每条 `EC_HOIST` 语句前开一格）。所以这一刀一行新机器都没造，只是把那条现成的
落点用上了：

```
(let $vt1 (ptr jnc$variant) (pnew (ptr jnc$variant) (int 1)))
(pstore (var $vt1) (call jnc$var$i (int 7)))
(expr (ccall Sink_put (var s) (int 1) (var $vt1)))
```

惰性位置上落点是关着的（`ecLazy`：`&&` 的右边、`? :` 的两支）—— 那时明说不收，理由与第五十八刀
一字不差：在那儿插一句就把求值顺序改了。
...
- **判据只到 `.sx` 那一层**：`tests/llvm/run.js` 第 9 节里宿主的体还没有读 variant 的那一格 ——
  要真跑一遍得让 `host.c` 按上面那个布局读（`$t` / `$n` / `$r` / `$s`），那一步这一刀没做，
  所以"宿主真读得对"这件事**尚未证**。记在这儿，别当成已证。
