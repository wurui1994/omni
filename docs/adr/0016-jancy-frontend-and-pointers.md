...
### 第一百八十六刀：同名那几条里"只有原型"的那些 —— 挑到宿主面那张表上去

第一百四十四 / 一百五十刀记的那笔账：只有原型的重载没进候选，于是调用点数出来的个数与类型
"本来就不全"，报的话像"你写错了实参个数"。第一百八十三刀把那些原型的**签名**收进了宿主面那张表
（`hostSigs`），所以这一刀能把话说完整：**合得上的那一条不是"不在表里"，是在宿主面那张表里**。

改法两处：`protoHostRetry`（按 `Owner$method` 去 `hostSigs` 里再挑一次，走
`hostMethodCall`）＋ 单条签名那一处的个数不对时先试它一次。

**两条为什么是安全的**（都是量出来的，不是想当然）：

1. 走到那儿说明**带体的那几条里没有一条收这么多个实参**（`pickOverload` 先按个数筛过），所以
   宿主面那张表里收这么多个的那一条必然没有对应的定义 —— 不会把"该调的定义"换成 `(ccall …)`。
2. 那时候**一个实参都还没降**（下面那个循环才降），所以不会把谁降两遍。`self` 也是调用方
   **已经求过值**的那一格，照原样包成值传下去 —— 不再走 `baseVal`，免得把 `f().m(x)` 的左边求两遍。
   `protoHostRetry` 回 `undefined` 表示"那张表里没有"、回 null 表示"试过了、那一处已经报过"，
   所以不会出现两句诊断。

判据在 `tests/llvm/run.js` 第 9 节，形状正是那面墙钉的那一种：`Plain` 上 `mix` 写两条 ——
一条带体（一个实参，这一层自己发）、一条只有原型（两个实参，体在宿主）。`q.mix(3, 4)` 走宿主那条
（`a*10+b` -> 34）、`q.mix(5)` 走带体那条（`a+100` -> 105），印 `mix 34 105`。**"挑对了没有"
就在这两个数上**。`bad/protoonly-arity` 退役。

- 腿：`node tests/jnc/run.js` 281/0、`node tests/llvm/run.js` 38/0。
- 逐份那张榜：lowered `136 -> 138`（+2）、clean `206 -> 208`（+2）；
  **对 `4121 -> 4127`（+6，往反方向走了）** —— 又是墙往里挪：这几处过去之后，后面的东西才被问到。
- 一起编那张榜：`70 -> 68`；那一行 `13 -> 10`。
- **剩下的 10 条照实记**：它们落在另外三处出错口 —— `pickOverload` 里按个数筛没剩（8256）、
  按类型排没一条合得上（8284），以及单条签名那一处**实参类型**不对（12924）。前两处的难点不是
  安全性（那儿也没降过实参，`cheapTy` 不降），是 `pickOverload` **回的是一个名字**、不是一格值，
  所以"改挑宿主面那条"得让它先能安静地回一句"没挑着"（不然会出现"一句诊断 + 一次成功降级"）。
  第三处的难点是那时前几个实参已经降过，重来会把 `ecOut` 那几行发两遍。这三处各要一小格机器，
  单独一刀去做。

### 第一百八十七刀：`pickOverload` 也能挑到宿主面去 —— 一格哨兵

上一刀留下的三处里的**前两处**（`pickOverload` 里按个数筛没剩、按类型排没一条合得上）。
拦路的既不是安全性也不是机器（那儿也没降过实参 —— `pickOverload` 只用 `cheapTy` 问类型），
是**返回值那一格放不下这个意思**：它回的是"挑中了哪个名字"（一个字符串），而这儿要说的是
"带体的都不合、可宿主面那张表里有"。

所以给它一格哨兵：`const HOST_PICK = '\u0000host'`（不可能与真名字撞），并且**只有把 `hostOk`
开着的调用方拿得到它**（另一个调用方 —— 构造那一处 —— 照旧拿原来那句诊断）。拿到哨兵的那一处
去 `protoHostRetry` 再挑一次；那儿回 `undefined`（表里真没有）才落回 `protoSiblingNope`。

- 腿：`node tests/jnc/run.js` 281/0。
- 一起编那张榜：`68 -> 64`；`原型 … 同名那几条里收这么多个实参的就是它` 那一行从 10 掉出前三。
- 逐份那张榜：clean `208 -> 210`（+2）、lowered `138` 没动、
  **对 `4127 -> 4186`（+59，往反方向走了）**。**这一次的原因查清楚了、而且很具体**：
  `string_t` 的 `m_p` 那一行 **46 -> 129** —— 也就是说多出来的全是它。更多的调用点过去之后，
  它们的实参里那些 `s.m_p` 才被问到，而那一格是第一百七十七刀**明说不收**的墙。
  换句话说：+59 不是新债，是把一笔旧债从"还没走到"变成"走到了"。
- **由此定下下一格**：`m_p` 这一行现在是逐份榜上最大的一条 `N`（129 处 / 一起编榜上 7 处）。
  它要的是**一段真字节**这个说法（这一层的 `char*` 指的是方言的整数格），那是方言那一侧的事
  —— 与 `main` 的退出码、函数值过宿主面同一类：**方言那一层缺一格**。这一格值得单独一刀，
  而且要先把"方言里加一格字节指针"这件事量清楚（ADR-0026 那条 string 落进内存的账是它的邻居）。

### 第一百八十八刀：第三处出错口 —— 实参**类型**不对时也去宿主面挑一次

第一百八十六刀留下的最后一处（单条签名那一处，出错口是实参类型）。这一处的难点先前记的是
"那时前几个实参已经降过，重来会把 `ecOut` 那几行发两遍"—— 修法就一行：**降实参之前记一格水位**
（`ecOut.length`），要改挑宿主面那条时先退回去。`decls` 那一侧不用退：那儿的东西都按名字去重
（`varBox` 那一族）。

安全性与前两处同一条：走到这儿说明带体的那几条里挑出来的**最合**的那一条也收不下这个实参
（`pickOverload` 先按类型排过），所以不会把"该调的定义"换成 `(ccall …)`。

**踩了一个坑、当场量出来**：第一版把 `args[i]`（其中一个实参那格节点）当成"这一句调用"传给了
`protoHostRetry` —— `hostMethodCall` 要从它身上取实参表，于是 `n.items[2]` 是 undefined，
`tests/llvm` 那一节当场崩（`this.flat(n.items[2])`）。腿是最后一道闸门：改成传 `n`。

判据在 `tests/llvm/run.js` 第 9 节，形状与退役那面墙（`bad/protoonly-argtype`）一样：`Plain` 上
`note` 两条**同元、只差类型** —— 带体的收整数（`n+1`）、只有原型的收 `string_t`（体在宿主，
回 `strlen(s)*100`）。`q.note("hi")` 与 `q.note(7)` 印 `note 200 8`。顺带这一格还证了
**字符串过 C_ABI 是那格胖指针的第 0 个字**（宿主那边 `strlen` 得出来）。

- 腿：`node tests/jnc/run.js` 280/0、`node tests/llvm/run.js` 38/0。
- 两张榜**一个数都没动**（逐份 138 / 210 / 4186，一起编 64 / 22）。照实记：语料里没有一处正好
  落在这第三处出错口上（退役那面墙注释里提到的 `std.String.copy` 那一族在 `std_String.jnc`，
  那份文件在这两条量法里都够不着）。这一刀是**把界补齐**，不是为了推数字 —— 前两处兑了、
  第三处留着的话，同一笔账就还剩一半，下一个人读到那句诊断又要重新查一遍。

### 第一百八十九刀：**一刀量法** —— 把 jancy 自己那几个扩展库的源码放进 `-I`

一起编那张榜上剩下的两行大头（`没有这个类型` 14、`没有这个函数` 13）钻下去，全是**同一件事**：

```
6 没有这个类型：'std.Buffer'      5 没有这个函数：'sys.getPreciseTimestamp'
3 没有这个类型：'std.Guid'        3 没有这个函数：'std.setError'
2 没有这个类型：'std.Error'       2 没有这个函数：'sys.getTimestamp'
1 没有这个类型：'std.StringHashTable'   1 没有这个函数：'sys.sleep'
```

这些名字的定义**就躺在 jancy 仓库里**：`src/jnc_ext/jnc_std/jnc/std_Buffer.jnc` 那一族与
`jnc_sys/jnc/`。jancy 那边它们由扩展库随身带、并且自动 import（`SOURCE_FILE_TABLE`，
第一百七十六刀量过那一段）。**我们的尺子先前不给这两个目录**，于是一整批
`import "std_Buffer.jnc"`（91 处）、`import "std_HashTable.jnc"`（98 处）与
`没有这个类型：'std.*'` 全是**量法的噪音**——榜首那几行是假账。

改的是 `tests/lib/jnc-sweep.js` 一处：`EXT_INCS` 收 `jnc_std` / `jnc_sys` 那两个 `jnc/` 目录，
逐份与 `--group` 两条路都带上（CLI 本来就收重复的 `-I`）。

**基线整个挪了一格，所以这一趟的数与上一趟不可比** —— 照实记下新的一档：

- lowered `138`、clean `210`（两个都**没动** —— 没有任何文件因此变好或变坏）；
  `(文件, 拦路项)` 对 `4186 -> 4724`（+538）。
- 涨的那几行**全是先前被 import 挡住、这才看得见的真东西**：
  - `不写 try 调 errorcode 的 '…'` **11 -> 158**（errorcode 那一族的下一大块）；
  - `std.HashTable 没有 construct` **1 -> 103**（`E`——我们答错了，std 那几个类的 construct）；
  - `'…' 的第 1 个实参要 void*，这里是 char*` **3 -> 99**（`E`——jancy 收 `char*` 隐式转 `void*`，
    我们不收）；
  - `string_t` 的 `m_p` `129 -> 148`、`同元重载第 1 个实参的类型要先降一遍` `85 -> 115`。
- 掉的：`没有这个类型` `315 -> 287`、`未声明的变量` `213 -> 205`，两条 import 那一行整行消失。
- 一起编那张榜：`没有这个类型` `14 -> 7`；总数 `64 -> 66`、理由 `22 -> 25`（同一条：看得见的
  真东西多了）。

**这一刀的价值不在数字，在选题**：它把三条**新的、具体的**下一格摆到了台面上，而且两条是 `E`
（我们答错了，不是"还不收"）——
...
### 第一百九十九刀：扩展库那几份隐式 import

上一段（第一百九十四到一百九十八刀）把那三份声明修得单编全绿，这一刀把它们**接进来**。

jancy 的扩展库除了那张源码表（`JNC_LIB_SOURCE_FILE`）还有一张**导入表**：

```cpp
JNC_LIB_IMPORT("std_globals.jnc")     // jnc_std_StdLib.cpp:930
JNC_LIB_IMPORT("std_Error.jnc")       // 同上 :931
JNC_LIB_IMPORT("sys_globals.jnc")     // jnc_sys_SysLib.cpp:218
```

那几份声明对**每个模块**都是"一开张就在"的，谁都不用写那条 import —— `memcpy` / `strlen` /
`atoi` / `std.setError` / `sys.getTimestamp` 那几族全在里头。第一百七十六刀其实已经认下了这件事
（那一刀把 std_globals 里的 CRT 那一族**抄进了前端**，理由写的就是"每个模块都隐式 import 了
那一份声明"）—— 这一刀把"抄一份"换成"真去 import 那一份"。

这一层没有"装扩展库"那一步（扩展库走的是 `import "libfoo.dylib"`，声明照旧写在源码里），
所以判据只有一条：**那几份声明找得着吗**。找得着（第一百八十九刀把 jancy 那几个扩展库的 `jnc`
目录放进了 `-I`）就照 jancy 那样隐式 import；找不着就什么都不做。落点是 `run` 里 `impDrain`
之前一句 `libImports(tree)`，往那张现成的 worklist 上推三条 —— 传递依赖、循环、查重全是现成的
（第六十刀那一套）。自己就是那三份之一时不会重复摊：`impSeen` 一开始就装着这个 unit。

判据在 `tests/jnc/cases/157-libimport.jnc`（C 双胞胎 `/tmp/c164.c`）：那一份里**一条 import
都没写**，却调得着 `libTwice` / `std.libTriple` / `sys.libTick` —— 它们在
`cases/libimp/std_globals.jnc` 与 `cases/libimp/sys_globals.jnc` 里，那个目录由 `.args` 的 `-I` 给。
`std_Error.jnc` 故意**不摆** —— 那一格是空跑，证的是"找不着就什么都不做"（别的用例的 `-I` 里
三份都没有，于是整个 tests/jnc 那一侧一个字都不变）。

- 腿：`node tests/jnc/run.js` 284/0（新增 `cases/157-libimport`）、`node tests/llvm/run.js` 38/0。
- 逐份那张榜：lowered `145 -> 148`（+3）、`(文件, 拦路项)` 对 `4481 -> 4436`（−45）。
  三行大的一起往下走：`没有这个函数：'…'` `228 -> 161`（−67）、`没有这个类型：'…'` `287 -> 257`
  （−30）、`未声明的变量 '…'` `205 -> 198`（−7）。
- 一起编那张榜：`62 -> 45`（−17）、理由 `23` 没动；`没有这个函数` `15 -> 2`、
  `没有这个类型` `7 -> 2`。
- **两处往回走，记成账**：
  - clean（没有"还不收"的文件）`217 -> 215`（−2）。那三份声明接进来之后，先前**够不着**的
    那些调用点现在够得着了，于是有两份文件从"只有普通错"变成"还有一条还不收"。墙往里挪了一格，
    数字往回走一格 —— 与第一百八十七刀那次同一类。
  - `'…' 那几条同元的原型：第 1 个实参的类型这一层还得先降一遍才知道` `33 -> 134`（+101）。
    这一行是 `strtol` / `strtof` / `strtod` / `atoi` 那一族：std_globals 里它们各有**两条同元
    原型**（收 `string_t` 的与收 `char const*` 的），第一百九十四刀已经能按类型挑了 ——
    挑不出来的是"实参的类型这一层还得先降一遍才知道"那一种（第八十刀那条：绝不猜）。
    先前这一行小，是因为那些声明**根本没接进来**（报的是"没有这个函数"）。所以这 +101 是
    从上面那 −67 里搬过来的一部分：话从"没有这个名字"换成了"有这个名字，可挑不出是哪一条"
    —— 后一句是**对的那句**。
  - 一起编那张榜上 `string_t` 的 `m_p` `9 -> 10`（+1），同一个原因（够得着的地方多了）。

### 第二百刀：同元重载里问得出类型的那三格实参

上一刀把那三份声明接进来之后，榜上最大的一族变成了"同元重载 / 同元原型：第 N 个实参的类型
这一层还得先降一遍才知道"（134 + 115 + 86 + 85 处）。第八十刀那条规矩没动 —— 挑哪一条按
`argCost` 排、问不出实参类型的**明说不收**（绝不猜）—— 这一刀添的是"问得出来"的那几格。
挑哪三种是**量出来的**，那一族拦着的实参就是它们：

- `indent.append(' ', 4)`（HidUtils.jnc:19）—— **字符字面量**。`charLit` 落成的是一格整数
  （那几个字节拼成的数），所以与 `65` 同一档。
- `insert(-1, string)`（std_String.jnc:96/103/110）—— **一元的负号**。`-` / `+` / `~` 不改类型
  （整数还是整数、实数还是实数），`!` 回 bool；别的（`*` 解引用、`&` 取地址、`++` 那一族）
  各有自己的形状，照旧回 null。
- `std.setError($"attempt to access out-of-bounds offset $offset")`（同上:162）——
  **格式化字面量**。`$"…"` 产出的是一格**动态**的字符串（literals.rst:62），所以类型是 string、
  而不算"字面量那一档"（那一格只对整数之间那条有意思，见 argCost）。

三格都守着 `cheapTy` 原来那两条：**一个字都不发**（不碰 expr / err / nope，只查表），
而且只在问得准时才回。

判据在 `tests/jnc/cases/158-overloadlit.jnc`（C 双胞胎 `/tmp/c165.c`）：顶层三条同元重载
（`long` / `string_t` / `bool`）与类里两条同元方法，分别用 `' '`、`-3`、`$"ab$x"`、`!false`
挑出来。C 那边没有重载，所以那几条各起一个名字 —— 挑哪一条是这一层的事，双胞胎钉住的是
**算出来的数**（`' '` 就是 32，所以 `pick(' ')` 是 320）。

- 腿：`node tests/jnc/run.js` 285/0（新增 `cases/158-overloadlit`）、`node tests/llvm/run.js` 38/0。
- 逐份那张榜：`(文件, 拦路项)` 对 `4436 -> 4283`（−153）；lowered `148`、clean `215` 没动
  （那一族多是"某一份文件里的一条"，拦路项少一条不等于整份文件收得下）。那四行从榜前十五
  **整族掉下去**了。
...
### 第二百〇八刀：顶层同名一族里"一条带体、一条只有原型"——话说准，界钉住

一起编那张榜上剩的三条 `E` 都指着同一处（`ias.jnc:30/36/66`）：

```jnc
size_t errorcode transmit(void const* p, size_t size);      // :30 只有原型
size_t errorcode transmit(string_t text) {                  // :35 带体
    return transmit(text.m_p, text.m_length);
}
```

先前报的是"原型 'transmit' 与它那个定义的签名对不上"——**那句话是错的**：jancy 的重载只按
**形参**分，这两条是**两个函数**，只有原型的那一条体在宿主那边。类里那一格第一百八十六刀已经
收了；顶层这一格收不了，理由不在源码上，在**方言那一层**：

- 类里：带体的那一条方言名 `Owner$m`、宿主那一条的 C 符号 `Owner_m` —— 两个字符串，不撞。
- 顶层：带体的那一条方言名就是 `transmit`，而宿主那一条的方言名**就是那个 C 符号**
  （`(cabi transmit …)`，`impDecls` 那一处起这一层的 cabi 名一直等于 C 符号）——
  一个模块里同一个名字放不下两格。

所以这一刀不改行为，改的是**那句话**（分成两句）与**墙**：

- 形参不一样 -> `还不收`，说的是"同名那一族里的另一条（体在宿主），而顶层这一格两边在方言里
  会撞同一个名字"。墙钉在新的 `tests/jnc/bad/proto-hostovl.jnc`。
- 形参一模一样、只有返回类型不同 -> 这才是**真错**（jancy 那句 "conflicting return types"，
  重载不看返回类型）。老那道墙 `bad/proto-mismatch.jnc` 改成这个形状 —— 它先前那个形状
  （`int twice(int)` / `int twice(double)`）**在 jancy 里是合法的**，注里那句"jancy 那边这也是错的"
  是错的，一并改掉。

- 腿：`node tests/jnc/run.js` 290/0（新增 `bad/proto-hostovl`，改了 `bad/proto-mismatch`）、
  `node tests/llvm/run.js` 38/0。
- 逐份那张榜：clean `223 -> 222`（**−1**）—— 那一格从 `E` 变成了 `N`，于是那份文件从
  "只有普通错"变成"还有一条还不收"。数字往回走一格、话变准了：这是第一百八十七刀那类账
  （墙往里挪一格）的又一次。lowered `155`、对 `4147` 没动；一起编那张榜 `40`、理由 `20` 没动
  （那三条 `E` 换成了一条 `N` 加两条 `E`，条数没变）。
...
榜上三行：`表达式里的 post-inc` 13、`pre-inc` 6、`post-dec` 1（20 份文件）。原样是
`m_reportFieldEncoderArray[encoderIdx++].encode(m_reportBuffer.m_p, bitOffset)` 那一族。

方言里 `++` 是**语句**（不取值），所以这一格要的正是 **EC_HOIST 那条现成的路**（第五十八刀为
errorcode 的传播开的那一条，第一百七十几刀带宽度的格式化字面量也在用）：把值落成一格临时、
自增那一句插到**这条语句之前**。这一刀的全部内容就是**次序**：

- 后缀（`i++`）—— 先落临时（旧值）、再自增；
- 前缀（`++j`）—— 先自增、再落临时（新值）。

自增那一句本身一个字都不用新写：直接调 `exprStmt(n, this.ecPad.length)`（语句位置上那一支
早就把整数回卷、指针步进、real 三种都分好了）。插不进语句的位置照旧说清 —— 与 errorcode
那一处、格式化字面量那一处是同一句话。

算符重载那一族（第一百三十一刀）**这一刀不收**，理由写清：它们是**两个函数**，
`postfix operator ++` 回旧值、前缀那个回新值（stdt_Iterator.jnc:44），取值这一半要按调的是
哪个函数来定，与内建那三种"自己再读一遍"不是一条路。

判据是手写的 C 双胞胎 `/tmp/c179.c`，用例 `tests/jnc/cases/173-incexpr.jnc`。
**这两份都刻意每条语句只放一处副作用** —— C 的实参求值次序是未指定的，
`printf("%d %d", i++, i)` 这种写法不是尺子（第一百九十六刀那一趟栽过一次，那条账就在上面）。
所以写成 `int a = i++;` 再印，印 `5 6` / `6 6` / `9 2`。

- 腿：`node tests/jnc/run.js` 316/0（新增 `cases/173-incexpr`）、`node tests/llvm/run.js` 38/0。
- 逐份那张榜：那三行**整行都没了**，lowered `159 -> 160`、clean `230 -> 232`（+2）、
  `(文件, 拦路项)` 对 `4001 -> 3986`（−15）。新冒出来的是 EC_HOIST 那一族的两行小尾巴
  （`这个位置上的 pre-inc` 2、`post-dec` 1）—— 那是真的"这儿插不进语句"，与 errorcode 那 20 份
  同一笔账。
- 一起编那张榜：`39`、理由 `19` 没动。

### 第二百二十二刀：把要插的语句插进**惰性**那两格（`? :` 的两支、`&&` / `||` 的右边）

榜上 `这个位置上的 errorcode 调用 '…'` 20 份。临时给那句诊断带上"为什么插不进去"量了一遍：
**22 处全是惰性位置**（18 处 `? :`、2 处 `&&`、2 处在更里头）。原样是
`return m_sessionConnectionState ? true : open();`（SerialTapSession / JLinkRtt 那一族）与
`return setDeviceVidPid(vid, pid) && openDevice();`。

先前这两格把传播的落点**关掉**（`ecLazy`）：传播那两句只插得到"这条语句之前"，插到那儿就成了
"无条件先调一遍"，短路语义与求值顺序一起被改掉 —— 所以第五十八刀当场说不收，而不是悄悄发一段
跑法不一样的代码。

落法（两格用**同一套**）：给结果开**一格内存**（`(pnew (ptr T) (int 1))` —— 与第九刀把局部量
提进内存、第二十四刀把取过地址的全局提进内存是同一件事，方言一个字没动），把整条式子摊成一句
`(if …)`，**每一支要插的语句放进它自己那半**，表达式本身回 `(pload 格)`：

```
(let $s1 (ptr bool) (pnew (ptr bool) (int 1)))
(if (var already)
  (do (pstore (var $s1) (bool true)))
  (do (let $e0 bool (call open (var x)))
      (if (un "!" (var $e0)) (do (ret (bool false))))     ; 传播就在这一半里
      (pstore (var $s1) (var $e0))))
(ret (pload (var $s1)))
```

惰性一点没丢。`&&` / `||` 那一格顺手从二元算符那条路里提成了自己的 `logicOp`：短路那一半先
写定（`&&` 写 false、`||` 写 true），再按左边那一问决定要不要跑右边。

**只有真需要时才走这条路**：两支都没插出语句就照旧发一句 `(sel …)` / `(bin "&&" …)` ——
一个字都不多发，所以先前能编的程序一个字节都不变（腿上 316 份用例一份没动过）。

`? :` 两支的类型对齐从 ternary 里抽成了 `selValue`（两条路共用同一套规矩，免得日后各自漂）。

判据是手写的 C 双胞胎 `/tmp/c180.c`（bool 的 errorcode 出错回 false，所以 `throw;` 就是
`return 0`、传播就是 `if (!r) return 0;`）。用例 `tests/jnc/cases/174-eclazy.jnc` 两格都走一遍，
**第 2 行印 1 而没有 open** —— 那正是短路还在。

**界往里挪了一格**：`bad/errorcode-lazy` 退了（它写的"要给表达式做一次真正的三地址化，不是这
一刀"这一刀兑掉了），换成 `bad/errorcode-loopcond.jnc` —— 剩下的那一种是**循环的条件**：
它每一圈都要重求一次，抬到语句之前只求一遍，那是另一个程序；要收得把循环本身改写
（`for (;;) { <那几句>; if (!条件) break; 体 }`），是自己一刀。

- 腿：`node tests/jnc/run.js` 317/0（新增 `cases/174-eclazy` 与 `bad/errorcode-loopcond`，
  退了 `bad/errorcode-lazy`）、`node tests/llvm/run.js` 38/0。
- 逐份那张榜：`这个位置上的 errorcode 调用` 那一行 20 份**整行没了**，
  顺手把 `这个位置上带宽度或补零的格式化字面量` 那一行（4 份）也带走了 —— 它走的是同一条
  `ecOut` 落点，惰性位置一开，它也插得进去了。
  lowered `160 -> 161`、clean `232 -> 234`（+2）、`(文件, 拦路项)` 对 `3986 -> 3953`（−33）。
  新冒出来两行 `E`（各 1 份：`'…' 两边不同型：左是 string，右是 int`、
  `jnc.Type 没有方法 '…'`）—— 那是先前被这一格盖着的真诊断。
- 一起编那张榜：`39`、理由 `19` 没动。

### 第二百二十三刀：类指针的数组（顺手把那句多拦了一种的话改准）

榜上这一族是好几行：`ui.Action*` 42、`ui.Icon*` 40、`ui.StatusPane*` 34、
`ui.InformationValue*` 10、`ui.CheckBox*` / `ui.BoolProperty*` / `sys.Thread*` 各 1 ——
ioninja 那一支的工具栏、图标、状态栏、信息面板全是这个形状。

门上那一问（declarator 的 array-suffix 那一支）只放整数 / 实数 / 布尔 / 枚举 / 结构体进来，
说的理由是"要方言能把多个字的值当元素搬（与 `&p` 同一格）"。那句话对 **fat 数据指针**（三个字，
ADR-0024）、`string` 与函数值是对的 —— **可对类指针不对**：这一层的类值就是一格地址
（第五十二刀），一个字，与 `int` 的元素同一格。所以那一问**多拦了一种**。

落法就是把这一种放进门里 —— `(blk (ptr C) 3)` 方言本来就收（数组的元素只要"一个字的值"）。
**一个字节的新机器都没造**，也没动方言。同一处那句话也改准了：现在写清"哪几种是多个字、
为什么"，以及"一个字的那几种与结构体都收"。

判据是手写的 C 双胞胎 `/tmp/c181.c`，用例 `tests/jnc/cases/175-clsptrarray.jnc`：局部的、
结构体字段里的、模块级的三处各一个，再加"没赋过值的那一格是 null"（方言的内存出来就是零）。
两边都印 `30 7 1`。

- 腿：`node tests/jnc/run.js` 318/0（新增 `cases/175-clsptrarray`）、
  `node tests/llvm/run.js` 38/0。
- 逐份那张榜：那一族**整族没了**（42 / 40 / 34 / 10 / 2 / 1 / 1 那七行），
  `(文件, 拦路项)` 对 `3953 -> 3906`（−47）。剩下的只有 `string 的数组` 14 与
  `char* 的数组` 2 —— 那两种是真的"多个字"（string 旁边挂着表、`char*` 是 fat 数据指针），
  换了新那句话之后**理由写在行里**了。
  lowered `161`、clean `234` 都没动 —— 那几十份后面还压着 `没有这个类型` 那一族口径账
  （它们本来就是 import 不着的那些模块里的类）。
- 一起编那张榜：`39`、理由 `19` 没动。

### 第二百二十四刀：`case` 的标签那句话认错了人

榜上 `case 的标签算不出来（要一个编译期整数常量）` 53 份。抽样量了一遍：**全是同一种** ——
`case EthernetTapLogRecordCode.Packet_ch1:`、`case HidDispatchCode.GetDeviceVid:`、
`case ModbusDispatchCode.GetProtocol:` 这样的**限定名**，那些枚举躺在别的模块里、
逐份编的时候 import 不着。

而"算不出来"这句话是**认错人**：这一层的 `constInt` 早就认枚举成员（`case Code.Ok:` 当场
量过走得通 —— exposedLit 与枚举体那两支）也认常量折叠（`constBin`，`case 1 + 2:`）。
所以这一格不是"算不出来"，是**那个名字本身找不着**，与 `没有这个类型` 是同一笔口径账
（一整批文件本来是一起编的）。

改法：标签是名字 / 限定名而 `constInt` 回 null 时，把那个名字印出来、说清"枚举成员与常量折叠
都认得，所以这一格是名字找不着"。墙 `bad/case-unknownenum.jnc`。

- 腿：`node tests/jnc/run.js` 319/0（新增那堵墙）、`node tests/llvm/run.js` 38/0。
- 逐份那张榜：那一行 53 份**整行没了**（换成按名字分的那些行 —— 名字各不相同，所以散得很开，
  一行最多几份）。`(文件, 拦路项)` 对 `3906` **没动**、lowered `161`、clean `234` 也没动 ——
  这一刀一个字节的代码生成都没改，改的只是话。**这是这一刀的全部收获**：先前那 53 份看着像
...
### 第二百二十六刀：无名形参（顺手兑掉一条"怕编名字"的界）

顺着上一刀那份导出样例往下读，榜上 `原型 '…' 没有带体的定义` 那一行里 `foo` 出现 6 次，
诊断的括号里写着**真正的原因**：`（形参表这一层没收下来）`。翻过去看：

```
void foo(int);            // jnc_sample_01_export_c/script.jnc:19-21
void foo(double);
void foo(char const*);
```

—— **无名形参**。jancy 收它（形参的名字只在体里有用，原型上根本用不着）。

树上它是 `(formal-anon specs ptrs)`：没有声明符，所以类型就是"说明符 + 那串 `*`"。落法是给它
编一个**源码里写不出来**的名字（`$a<序号>`，`$` 不在标识符里）；数组长度省不掉、类值当形参
那两条检查照旧（与有名字那一支同一句话）。

**这一刀兑掉的是 `bad/formal-unnamed` 那份注里的一句担心**：那儿写的是"编了之后这条声明就有了
一格源码里不存在的名字，往后'按名字查形参'的地方（默认值、重载的诊断）都会拿它去比，那是个静默
的错"。量了一遍：这一层**从来不按名字查形参** —— 默认值是按位置挂在每一格上的（`def`），重载的
诊断印的是位置（"第 1 个实参"），全文里对形参名字的唯一一问是 `p.name === 'this'`（而
`$a0` 永远不等于 `this`）。所以那份担心不成立，墙退了。

判据是手写的 C 双胞胎 `/tmp/c182.c`，用例 `tests/jnc/cases/176-anonformal.jnc`（印 42）。
**双胞胎那份注里写清了一处两边不一样**：C 里"定义"上省掉形参名要到 C23 才合法（原型上一直
合法），所以那边定义那一格照旧写上名字、只是不用它 —— 判据是**答案**（第一格被忽略、第二格照常
用），而答案跟形参有没有名字无关。

- 腿：`node tests/jnc/run.js` 319/0（新增 `cases/176-anonformal`，退了 `bad/formal-unnamed`）、
  `node tests/llvm/run.js` 38/0。
- 逐份那张榜：`无名形参` 4 份与 `原型 '…' 没有带体的定义（形参表这一层没收下来）` 2 份
  **两行整行都没了**，lowered `161 -> 162`、clean `235 -> 238`（+3）、
...
### 第二百三十二刀：嵌进来那一格的 construct 也在宿主那边

榜上"原型 '…'.construct 没有带体的定义 —— 造一格它就得调它"那一行 95 处。`new` 那一条路
第一百六十二刀就落了（`(ccall C_construct self …)`，签名取声明里那一份），所以先量一遍**它到底
拦在哪儿** —— 一起编那张榜上剩的两处指得很清楚：

```
ui.ToolBar m_toolBar;              test/ioninja/api/doc_PluginHost.jnc:26
ui.PropertyGrid m_propertyGrid;    同上:28
```

不是 `new`，是一格**嵌进来**的类字段（局部的类变量走同一条路）。这一层的类字段就是一格对象
（第五十二刀），声明它就等于造它、紧接着要调构造 —— 那句话由 `ctorCall` 发，而那儿只认这一层
自己那格 `(call C$construct …)`，没有宿主面那一支。缩到最小：

```
opaque class W { construct(); }
class Host { W m_w; }              -> 原型 'W.construct' 没有带体的定义
```

落法是把 `new` 那一支的三行搬过来：同一张 `hostCtors` / `hostCtorArgs`、同一个符号、同一份
声明里的签名，只有那格地址不同 —— 字段的地址（`(pfield …)`）而不是刚 `pnew` 出来的那一格。
一格新机器都没造。

判据摆在原生腿上：`tests/llvm/run.js` §9 添了 `class Owner { Plain m_p; }`，`new Owner` 之后
`ow.m_p.seeded()` 印 5（那个 5 是 `construct(long seed = 5)` 原型上的默认值，第一百九十一刀）。
独立的尺子是手写的 C 双胞胎 `/tmp/c188.c`（嵌进来那一格，构造在另一个翻译单元、传字段的地址），
也印 `emb 5`。

- 腿：`node tests/jnc/run.js` 321/0、`node tests/llvm/run.js` 38/0（§9 多一行 `emb 5`）。
- 逐份那张榜：那一行**整行没了**（95 处）。`(文件, 拦路项)` 对 `3744 -> 3650`（**−94**）、
...
### 第二百三十五刀：那格声明自己没成，别把回声也记成一笔

上一刀那条路往前再走一格。榜上 `未声明的变量 '…'`（192 处，E）先量了一遍名字，头几个是这样：

```
 169  m_bufferPropertySet     （类在没 import 的文件里 —— 逐份编那笔口径账）
 133  params
  54  a
  37  hdr
```
...
```
string_t getDeviceInfoString(UsbDeviceParams const* params) {   test/ioninja/plugins/UsbLog/UsbLogRepresenter.jnc:19
	…
	params.m_vendorId,                                          同上:41
	params.m_productId,                                         同上:42
```

`UsbDeviceParams` 这一份没 import（逐份口径），于是**那格形参自己就没成** —— `formalList`
在 `specs` 那一句上回 null，名字一格都没登记上。后面每一处用到它又各报一句"未声明的变量" ——
**认错人**（名字明明写着），而且把**一件事记成好几笔**（一个名字用了几处就几笔）。

落法与上一刀一样是"记一张表"：`localDecl` / `formalList` 在类型认不出来那一句上回 null 之前，
先把声明符上那几个名字记进 `badNames`（新添的 `badDeclNames` 只走语法树、一个字都不发），
查名那两处（表达式与左值各一处）问不着名字时先问它一句，问着了就指回那一句真话。

**记成普通错而不是"还不收"**：根上那一句（`没有这个类型`）本来就是普通错，这儿只是它的回声。
第一版写成 `nope`，量出来 clean `239 -> 235` —— 那 4 份文件本来一格"还不收"都没有，被这一句
硬推出去了，而它们并没有真的多欠什么。改回 `err` 之后 clean 回到 239。

这张表是**按名字**记的、不分作用域：同一份文件里另有一格同名的、真的没声明的变量会被这句话
盖住。记成账：这一格只在**已经报过错的文件里**才有内容（表是空的时候一句都不发），所以盖住的
范围至多是那一份已经失败的文件。

- 腿：`node tests/jnc/run.js` 325/0（新增 `bad/decl-cascade`）、`node tests/llvm/run.js` 38/0。
- 逐份那张榜：`未声明的变量 '…'` `192 -> 160`，新出来的那一行 110。
  `(文件, 拦路项)` 对 `3663 -> 3741`（**+78**，往错方向）—— 原因与第二百一十四 / 二百三十三刀
  同一种：**一行裂成两行**，而一份文件两行都算。裂开是有用的：剩下那 160 处是**真的**没声明
  （或者别的原因），信号纯了。lowered `162`、clean `239` 都没动。
  逐行比过一遍：只有这两行动。（量的时候差点记错一笔：`import "io_ssl.jncx"` 那一行看着从 9
...
### 第二百三十七刀：那个实参是哪一种，说出来

榜上剩的两大行是 `'…' 的同元重载：第 2 / 第 3 个实参的类型这一层还得先降一遍才知道`（86 + 85）。
第二百二十八刀那一遍已经把**到底是哪些实参**全语料量过一次了，答案只有两种，而两种都是
**已经记在这份 ADR 上的方言级的账**：

```
 268 + 6 + 4  sizeof(params) / countof(a) / dynamic-sizeof   —— 方言一格 8 字节那笔
        168   text.m_p / text.m_length                       —— string_t 的字节段那笔
```

那句笼统话于是欠着两件事：一是**没说清**（读的人还得自己去翻），二是**记错了行** —— 那一族并不是
"这一层还没做重载决议"，重载决议早就有（第八十刀），拦着的是那两笔账。按第一百四十九刀那条法，
这一行该指名。

落法是一格 `cheapWhy`（只看语法树、一个字都不发，与 `cheapTy` 那一族同一条规矩）：认 `sizeof` /
`countof` / `offsetof` 与它们的 `dynamic-` 三兄弟、以及"左边是 `string` 的取字段"；`&x` 与算式
里头递归问一遍（`&params`、`p + sizeof(hdr)` 那种）。问着了就把那半句换成指名的话，问不着照旧
说原来那句 —— 剩下那 14 + 15 处就是"别的原因"（第二百二十八刀量到的 `&x`、不认得的名字、
被调者形状那几种），信号从此是纯的。

墙两道：`bad/ovl-arg-sizeof`、`bad/ovl-arg-strbytes`。

- 腿：`node tests/jnc/run.js` 327/0（新增那两道墙）、`node tests/llvm/run.js` 38/0。
- 逐份那张榜：`第 2 个实参…` `86 -> 14`、`第 3 个实参…` `85 -> 15`、`第 4 个实参…` `1 -> 0`；
  新出来的三行 84 + 84 + 1。`(文件, 拦路项)` 对 `3732 -> 3758`（**+26**，往错方向）——
  与第二百一十四 / 二百三十三 / 二百三十五刀同一种：**一行裂成两行**（一份文件里第 2 个实参
  既有 `m_p` 那种、也有别的原因那种，两行都算）。lowered `163`、clean `240` 都没动。
  逐行比过一遍：只有这六行动。
- 一起编那张榜：`32`、理由 `14` 没动（那两行在 group 里各 2 处，换了名字、处数不变）。

### 第二百三十八刀：`%p` 那一格是刻意划的界，让诊断把它说出来

榜上 `printf 的转换 '…'`（15 对）量了一遍：**全是 `%p`**（全语料 59 处，别的转换符一处都没有）。

这一格的账**早就想清楚了**，写在 `bad/printf-conv-p` 那道墙的注上（第三十二刀）：

> `%e` / `%g` / `%u` 都是"还没长出来"，做掉就落地了；`%p` 是**这一层刻意不做**。这几条腿上
> 指针是**两套实现** —— run / interp 那几条是 arena 模拟（地址是块内偏移），run-c / run-llvm
> 是真指针。裸地址在这两边根本不是同一个数，所以"印出来"这件事没有一个各条腿都对的答案。
> 而"不许观测裸地址"这条纪律正是那两套实现能一直对得上的原因。

欠的只有一件事：**诊断没把它说出来**。榜上看见的是"printf 的转换 '%p'"，读起来像"还没做"——
把一条**刻意**的界记成了一笔欠账。这一刀把墙注上那句话搬进诊断里，并在话尾给出能观测的那几种
（`p == null` / `*p` / `p[i]` / `p - q`）。墙照旧在，`.expected` 与注跟着改。

- 腿：`node tests/jnc/run.js` 327/0、`node tests/llvm/run.js` 38/0。
- 逐份那张榜：`printf 的转换 '…'` 那一行**整行没了**（15），新出来的那一行 15。
  `(文件, 拦路项)` 对 `3758` 没动（15 换 15 —— 这一刀换的是账记在哪一行，不是拦路项的个数），
...
### 第二百四十一刀：静态字段带花括号初值，走的是同一条路

上一刀量出那句兜底话剩下的 21 处，全是这个形状：
...
那一支上；带花括号初值的那一种在语法上是**另一个 head**（`var-decl-curly`），于是落到了兜底话上
—— **欠着的**，不是划的界。顶层那一遍现成就有 `globalDeclCurly`（第二十四刀那条路：长度从花括号
里数出来、初值一格一格抄），类体这一遍 `this.ns` 已经是这个类（第一百二十四刀），交给它就完了。
元素类型收不收由**已经在那儿的门**说 —— `string 的数组` 那笔 ADR-0026 的账照旧自己报。

**非静态**的那一种（`string_t m_statusTextTable[] = { … }`，量到 4 处）留着：那要在构造里一格
一格填，是另一笔账 —— 那 4 处就是这一行剩下的全部。

判据是手写的 C 双胞胎 `/tmp/c192.c`（C 里静态成员摊成一格**带前缀的文件级数组**，正是这一层落的
形状：方言里那格叫 `C$m_table`），用例 `tests/jnc/cases/183-staticcurly.jnc`（类与结构体各一格），
两边都印 `42 20`。

- 腿：`node tests/jnc/run.js` 330/0（新增 `cases/183-staticcurly`）、`node tests/llvm/run.js` 38/0。
- 逐份那张榜：`类里除字段以外的成员` `15 -> 4`（剩下的 4 处就是非静态那一种）。
  lowered `163 -> 164`（**+1**）、clean `240 -> 241`（**+1**）、
  `(文件, 拦路项)` 对 `3737 -> 3735`（**−2**）。
  往错方向的几笔照旧写清 —— 都是**先前被那句兜底话盖着**的账露出来了：
  `string 的数组` `14 -> 22`（+8）、`char* 的数组` `2 -> 3`（+1）（那些表的元素本来就是
  string / char*，ADR-0026 那笔），以及新出来一格 `第 1 项越过了 ui$ListItem 的 0 个字段`（+1）
