...
`--group std`：`368 条 / 36 种` → `359 条 / 34 种`。少的那两种里认得出的一种是
`结构体 '…' 里的字段 '…' 按值套回了自己`（4 处整行没了，就是那句认错人的话）；另一种落在榜
**印不出来的那一段**里（这张榜只印前 25 行，34 种印不全）—— 说不出是哪一行就不写，
这也是账的一部分。`未声明的变量` `69 -> 35`（**−34**，上一刀涨的那 32 条连本带利还回来了）。
