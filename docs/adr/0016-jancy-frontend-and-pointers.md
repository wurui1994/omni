...
## 第一百五十六刀：`opaque class` 上的属性 —— 撤掉一格**冒充宿主状态**的存储

`opaque class Action { bool autoget property m_isCheckable; }`（ui_Action.jnc:27，ui 那一族几乎
每个类都是这个形状）。`opaque` 说的是"这个类的布局在宿主的 C/C++ 那边"（opaque.rst:15-29），
所以 `autoget` 那格存储也在那边。

先前这一层替它**合成**了一格 `…$m_value` 字段（进了那格结构体）、再让合成的取值器去读它。
于是：写那一侧报"存值器没有定义"，读那一侧**过了** —— 读出来的是我们自己那一格、不是宿主的
状态。一半写宿主、一半读自己，而且**不报一个字**。这比"还不收"坏得多，是这一轮里找到的第一个
**静默错答案**。

这一刀把那一格撤了：主人是 `opaque class` 的属性一格存储都不合成，读写两侧都指着宿主面。
真要收它得给属性那两个函数也发 `(ccall Owner_get_p self)` / `(ccall Owner_set_p self v)` ——
与第六十六刀 construct 那一支同一条路，是 ADR-0022 J4b 剩下的一格。

### 账：pairs `5579 -> 5692`（**+113**）—— 这 113 处里 58 处先前**一个字都不报**

- 逐份那张榜：lowered `121`（没动）、clean `192`（没动）、pairs `5579 -> 5692`。新长出来两行：
  `写属性 '…' —— 它的主人是 opaque class …` **103 处**、`读属性 …` **58 处**；
  老那两行只剩非 opaque 的余量（写 91 / 读 3）。
- `--group test/ioninja/api`：`156`（**没动**）、理由 `39 -> 40`。

**那 58 处是这一刀的全部意义**：它们先前不在榜上 —— 不是因为收了，是因为这一层悄悄给了个答案。
榜涨了 113，可"我们知道自己不会算错的地方"多了 58 处。于是把这条口径写下来，与"逐份那张榜是
选题的尺子"并列：

**榜上不动、甚至变好，也可能是因为在说谎** —— 一格"合成的存储"顶了宿主的状态，读那一侧就永远
不报错。所以凡是这一层**替用户合成**的东西（第一百四十八刀那个 `synth` 记号、autoget 的存储、
基类链上那些空构造），都要问一句：**这一格在 `opaque` / 原型那一侧还算不算数**。

`bad/opaque-prop.jnc` 钉这条界（读那一侧 —— 先前正是它不报）。
