...
### 第十六刀（查清，未改）：最后那一个"数值不同" —— `2.5E+0.1*N` 里的 `E` 是**东**，不是指数
...
**`2.5*E + 0.1*N` = (2.5, 0.1)**（`E`/`N` 是 plain 里的方向常量）：按 drawlabel 的公式
`scale0 = 2.5` -> `Align = (0.5, 0.02) - (0.5, 0.5) = (0, -0.48)`，与参考印的
`(0.000000,-0.480000)` 逐位对上。

我们印的 `(-0.5, 0)` 反推回去是 `align = (0, 0.25)` —— 也就是把 `2.5E+0` 当成了**一个浮点
字面量**（2.5），后面 `.1*N` 再乘上去：`2.5 * 0.1 * N = 0.25*N`。**词法那一格错了**：
`E` 后面跟的不是纯数字（这里是 `+0.1`，小数点之后还有东西）时，那个 `E` 不该被吃成指数。

改的是 asy 前端的数字词法（`frontend-asy` 的 lexer）：指数只在 `[eE][+-]?[0-9]+` **且后面
不再接 `.`** 时成立；不成立就只吃到小数部分，`E` 留给后面当标识符。判据现成：这一句
`label("6", z6, 2.5E+0.1*N)` 出来的对齐向量必须是 `(2.5, 0.1)`。
