...
也就是说形状对、位置居中、只差一个均匀倍数 —— 下一刀最该先做的是**把
`angle`/`zoom`/`m`/`M` 这四个数印出来**，拿手算的 `tan(0.5*angle)*|M.z|` 与
"要让 ink 铺满所需的半宽"（现在的 42%）对一下，看差的是 `angle` 的单位、
`|Zmax|` 该取哪一头、还是 glrender 在 `initDisplay` 之后又调过一次视口。
