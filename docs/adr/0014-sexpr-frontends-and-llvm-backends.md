...
## 下一刀（未做）：清单换成两个数组，不再走文本

**结论先说：这一刀不需要给方言加新 op。** 之前记的"要给方言加一格把大数组交给运行时"
已经过时了 —— blob 结构体在 03ec970 里就摊进 `omni.h` 了，C 那条腿上数组就是那个结构体的
指针，所以只要让 `_r3render` **多收两个数组**（`(r3render nums ops str)`），运行时那侧
直接读 `->items` 就行；口子与现在这条 `(r3render <字符串>)` 同一形状（frontend-asy/calls.js
里那十行）。

现在这条路的账（pdb，`-O0`，每趟 shipout）：
- asy 侧把清单拼出来：`anon124` 自用约 4s（含插桩，实测值更小）、`sn` 716 万次调用；
- 写 65.2MB 到 `/tmp`，再由 `r3_num`/`r3_word` 逐词读回来：解析 0.82s。

换成两个数组之后这三样**一起没了**：数直接落在 `real[]` 里（连 `_rhex` 与十六进制
都不需要了）、操作码与长度落在 `int[]` 里，运行时按下标走。估的收益：`-O0` 上每趟省
1.5~2s（5.87s 里的），`-O1` 上按比例约 0.5s（1.47s 里的）—— 也就是**三维那一档再快三成**。

要认的代价与要守的规矩：
1. **`OMNI_R3_DIR` 那个文本清单是现在唯一的可读量口**（`r3.scn` 能直接看、也能手写来单测
   `omni_r3.c`）。换成二进制之后得留一条"把两个数组转回文本"的调试路，不然出问题只能靠打印。
2. **判据不变才算数**：清单是我们与运行时之间的私有格式，数是精确传的，所以位图应当
   逐字节不变 —— 验收照老规矩，七个三维例子 + `twoSpheres`。
3. JS 那三条腿不受影响（`r3render` 在那边回空串、走 gs 那条老路）。
4. 顺带能去掉的：`sn`/`_rhex`/`x` 前缀那一套，以及 `r3_num` 里刚补的整词校验。
   **别急着删** —— 手写清单的调试路要留着它们。
