...
- **lmfit1**（方向相反：我们套 gsave、参考不套）—— 是**闸门条件**问题，不是浮点问题，
  下一刀从 `drawelement.h:296-342` + `psfile.h:295-330` 整段搬开合规则最有希望
- **laserlattice**（参考套、我们不套）—— 与 cardioid 同向；它的图是 `image/palette`，
  界那一族已经铺满，下一刀先按 lmfit1 的结论回头看它
