...
方向不对）。它上头有 `xaxis3(...,Arrow3)` / `yaxis3(...,Arrow3)`、
`dot("$(1,1)$",...)`、`arrow("$y=\sqrt{x}$",...)` —— 三维那边这些标签与箭头是
面片/billboard，很可能有一族我们只记了界的一部分或压根没记。下一刀先把
shellsqrtx01 的 m/M 印出来，对着"漏了哪一族"查，而不是继续在 bound 那条链上推。
