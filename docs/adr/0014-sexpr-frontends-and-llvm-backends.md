...
2. **非绘图部分的执行**，与真正的 `asy` 二进制逐字节比对：pair/triple、数组与切片、
   ...
   oracle —— 跟 `tests/oracle` 那条轴同一个口径。**还没做**，但口径已经量清楚了（下面）。
...
写下来是为了不含糊：现在成立的只有第 1 道。第 2、3、4 道一个都没开始，语法覆盖率
不等于「等效实现」。
...
- `bool` 输出**补到 5 个字符**：`true ` / `false`。这不是笔误，是 asy 的写法。
...
- `write` 的签名是 `write(string 前缀, T...)` 或 `write(T...)`：多个 T 之间是 **制表符**，
  前缀与第一个 T 之间**没有**分隔符（`write("x = ", 5)` → `x = 5`）。而 `write(5, "u")`
  在 asy 里直接报 `no matching function 'write(int, string)'` —— 这个错也是要复现的行为。

按这些量出来的口径，第 2 道门槛的第一刀（int / bool / string + 控制流 + 函数 + 单参
`write`）需要核心方言补两处，两处都是「C 系语言都要的」而不是为 asy 特设的：
`(brk)` / `(cont)`（OIR 早有 `Break` / `Continue`，只是方言没开口），以及一个
数值转字符串的节点（封闭 ABI 里 `js_str` 就是它，六条腿都有）。
