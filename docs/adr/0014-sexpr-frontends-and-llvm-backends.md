...
量口是现成的：`/usr/bin/time -l` 看峰值足迹，或者跑 BezierPatch 看还死不死。
再加一个最小复现：`draw((0,0,0)--(1,1,1),2bp+blue)` 一条**直**粗线，run-c 全程 2.89s、
峰值足迹 **432MB**（含 node 那个壳）—— 一条线就四百兆，BezierPatch 那两条画在面片上的
**曲**线（tube 的 Split 递归更深）就是 43GB。所以下一刀先量 tube 那一族的构造次数。
