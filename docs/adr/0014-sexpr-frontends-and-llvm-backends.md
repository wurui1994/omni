...
- 探针：`edge_trans` **8754 → 0**、`sph_trans1` 40932 → 27804、`sph_trans` 56154 → 31674；
  `flat_trans` 是唯一变坏的一格（0 → 1740，最大差 6，全在那条对角线上）——
  那说明填充规则还差一点（`a2` 是 `1-a0-a1` 算出来的，它的"恰好为 0"与真正的第三条
  边函数不是一回事）。下一刀先把这一处对齐，再看 `sph_trans1` 剩下的 27804。
