...
## 量：EPS 那一轴的现状 —— 一样 81，能碰的非数值差只剩 0 个

<!-- ADR-0014 轴现状-BEGIN -->

```
  一样 81   只有数值差 1   结构不同 86   没出图 1   超过 3000ms 20
  （没有参考、不计分 24；摇骰子、比不出来 7；189 个全部现跑，270s）
```
...
结构不同的 86 个里，**82 个的差异摘要里带"位图块数 1 vs 0"** —— 参考是 dvips 嵌进去的
位图（三维那一族走 `_image`/渲染），逐字节对不上是这一轴的边界，不是缺陷。
剩下的 **4 个**（cardioid / gamma / laserlattice / lmfit1）是同一个签名：

```
  cardioid     #4166 参考 gsave   vs 我们 newpath
  gamma        #3040 参考 gsave   vs 我们 newpath
  laserlattice #3767 参考 gsave   vs 我们 newpath
...
顺手把这 4 个里的 cardioid 拿 **run-c 那条腿**（链真 libm）跑了一遍：前 3067 个 token 与参考
相同，第一处差是 `1.89843058e-17` vs `2.46521741e-17` —— 一个近零坐标，正是前面
「FMA」那一节记的 clang `-ffp-contract=fast` 那一档；而那 82 个 token 的长度差（gsave 块）
仍然在。所以这 4 个例子要的是**两件**：FMA 的合并方式，与 `cos` 这一族的最后一位
（JS 那条腿上还多一层 V8 vs libm，见「哪份 libm」那一节）。

**结论**：这一轴上"能靠读源码对上的结构差"已经清零了 —— 剩下的是 82 个位图、
4 个末位浮点、1 个 textpath 的宽 bar、20 个还超 3s 的、1 个没出图。
