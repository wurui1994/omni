...
- `(brk)` / `(cont)`：OIR 早有 `Break` / `Continue`，方言一直没开口。实现只有
  ...
  > **后来长了层号**（ADR-0016 第四十刀）：`(brk N)` / `(cont N)` 是往外数第 N 层，
  > N 是 1..9，省掉就是 1。逼出这一格的是 jancy 的 `break2` / `continue2`。MIR 本来
  > 就按层号跳，所以那一头没动；改的是方言、HIR 的检查、树解释器的信号编码，加 JS 与 C
  > 两个结构化后端要发标签（C 里只能 `goto`）。
...
`OMNI_ASY_MODS=1` 仍然是开关，还没变成 `run` 的默认。（**第一百〇四刀翻了**：它成了默认，
`OMNI_ASY_MODS=0` 才回到"整份程序一份大 JS"那条。）
...
### 第一百〇四刀：三道缓存全是摆设 —— 一份产物按名字共用，就不能把"入口是谁"烙进正文

要求是一句话：**`OMNI_ASY_MODS` 任何时候都得是默认，包括缓存，包括 REPL**。
翻开来量，发现那条路上三样东西各自坏在一处，而且都是"日志里写着、没人细看"的那种。

**一、快路永远不命中。** `asyModsBuild` 往清单第二行写的是 `srcStamp()|main:<入口>`，
`asyModsFast` 拿 `srcStamp()` 去比 —— 于是同一个入口连跑两趟，第二趟照旧满编。
量出来 **5.9s**（命中时 0.22s），日志上每趟都有那句「asy mods 不命中 编译器自己变了」。
教训：**缓存的键只准写在一处**，两边各拼一遍就是这个结果。现在 `cs` 是一个变量，
两边都调它。

**二、库的产物跨不了入口。** 上面那个 `main:<入口>` 本来是为 TeX 那条路加的
（`_mainname()` 降成字面量，dvips 把 dvi 的名字写进产物正文），可它进的是 `cs` ——
**每一份**产物的印记都带着它。于是 01-arith 编完再跑 02-strings，日志写着
「新编 4 份、复用 0 份」，28 行的例子 **0.9s**。

改法分两步：
- `_mainname()` 不再降成字面量，而是一格 weak 全局 + 一个 getter，名字**由入口在
  main 的第一句里填**（`(set asy__mainname_v (str "01-arith"))`）。于是库与 weak
  两边的正文都与"入口是谁"无关。落这一步时踩了一脚：`(set …)` 加在 lower.js 拼
  `(main …)` 的那个 `body` 上，而模块路是从 `sections.main` **重拼**一遍的 ——
  单体路对、模块路 `_mainname()` 回空串。两条路都读的那个数组才是该改的地方。
- 印记里那格 `main:` 只发给**正文里真出现过那个名字**的单元（判据故意偏保守：
  恰好同名的库会白重编一次，但不会拿着别的例子的名字跑）。清单里跟着记一格，
  快路上核一遍 —— 库共用之后，"这一份是不是别的入口刚写下的"必须问得出来，
  与 `w|`（weak 那一份）是同一类问题。

量出来（同一份源码，交替跑两个入口）：新入口 0.9s -> **0.34s**（「新编 2 份、复用 2 份」）、
换回上一个入口 -> **0.33s**、同一入口重跑 **0.22s**（清单快路）。

**三、REPL 里那门语言少了一半。** `repl --lang asy` 的 AsySession 一直是 `load: null`，
`import settings;` 报的是"这条路上没有模块加载器"，隐式的 asy_builtins 也不在 ——
而 asy 自己的 REPL 是能 import 的。现在把 `run` 那一路的两样（模块加载器 `fe.loader`
与 prelude）接上，文件 IO 与解析缓存仍在 cli.js 那一侧注入。
代价是启动从 0.17s 变成 **0.67s**（库进来了），换来的是 `import settings;`、`sin(0.5)`
-> 0.479425538604203、`substr("hello",1,3)` -> `ell` 这些在 REPL 里真的能用。

`run` 那一路的默认跟着翻过来：`.asy` + 有 JS 引擎 + 不是 `--interp` 就走模块路，
`OMNI_ASY_MODS=0` 才回到"整份程序一份大 JS"那条（那条上还有第七十四刀的整份产物缓存）。
**原生二进制那一路还没有这一层**：模块路是 `spawn node` 跑 ESM 图，原生那边的对应物
（把链好的可执行文件按同一把印记缓存起来）还没做，记在这儿。

跑过的轴：`tests/asy/run.js` **259/0**（含 draw 那五个与真 `asy -f eps` 逐图对照）、
`tests/run.js` **96/0**（含 repl 四份会话 × 两个引擎）、`tests/asy/sweep.js`
（304 个例子干净 **299**、模块 0 条、3.6s）、`_mainname()` 四条路各印一遍
（模块路 / 单体路 / `--interp` / `run-c` 都是 `prefixchk`）。

### 第一百〇五刀：原生那一路一趟缓存都没有，而所有中间文件都躺在系统会清掉的地方

两件事，同一个根：**"跑一遍"到底把东西放在哪儿**。

**一、原生二进制上 `run` 每趟满编。** node 那一路早有三层缓存（模块产物、整份 JS、
AST），而自举出来的二进制一层都没有 —— 量出来 `run tests/asy/cases/03-quotes.asy`
每趟 **12s**（AST 读回来 3.4s + 前端 5.0s + 发 C 2.5s + clang 0.5s），
而这 12s 的输入一个字节都没变。

补的是一层**可执行文件缓存**（`.omni-cache/asy-exe/e-<路径哈希>.bin` + `.dep`）：
键与 JS 那份同一套（编译器印记 + ASYMPTOTE_DIR / OMNI_ASY_BUILTINS / cwd + 这一趟读过的
每个文件的改动时间与字节数），**多一格 cc 与它的 flags** —— 同一份源码用 clang 与用 tcc、
`-O0` 与 `-O2` 链出来的是两个可执行文件。三个决定：
- **在 compile 之前问**：大头全在前端，编完再问等于白编。
- **直接链进缓存那一格**（`clang -o .omni-cache/asy-exe/…bin`），不先链到别处再拷 ——
  少一次 8MB 的拷贝，也少一条"拷了一半"的路。清单最后写：链一半就没有清单，下一趟重来。
- `run-c` **不吃**这层缓存：那是测试轴上"明说要走 C 这条腿"的动词，缓存是给 `run` 的。

量出来（自举出来的二进制，03-quotes.asy）：冷 **4.8s** -> 热 **0.012s**。
钉过三种失效：改源码（重链，输出跟着变）、touch 库文件（不命中）、同一份源码（命中）。
已知的粗糙：这一层按**改动时间 + 字节数**判，不像 mods 那层按内容哈希 ——
`touch` 一下就重编一遍。等真的碍事再换。

**二、所有中间文件搬出 `/tmp`。** 从前 glr 表、运行时 `.o`、生成的 `.c`、链出来的
`a.out`、jit 宿主、13 条测试轴的工作目录，全都在 `mkdtemp(tmpdir())` 底下。三个毛病：
系统会清（EPS 参考那 192 份就是这么丢的，一趟 5 分钟的真 asy 重跑）；名字随机
（`/var/folders/x6/dw0k…/omni-l2pb0k/a.out.c`，出了问题连"上一趟编了什么"都翻不出来）；
同一台机器上两个 checkout 撞不撞全靠运气。

现在只有一个根：`cacheRoot()` = 仓库里的 `.omni-cache`（`OMNI_CACHE_DIR` 可搬走）。
底下 `glr/ rt/ jit/ incr/ asy-ast/ asy-js/ asy-mods/ asy-exe/ work/ test/`。
一次性的工作目录**不再用 mkdtemp**：名字由"用途 + 输入哈希"定，下一趟原地盖掉 ——
从前靠系统清 `/tmp` 才不攒垃圾，搬进仓库之后随机名就等于每跑一趟多一个目录。
代价写在明处：**并发跑同一个输入会撞**（那本来也在争同一格缓存）。测试轴那 13 处走
`tests/work.js` 的 `workDir(轴名)`：一条轴一格、进来先清空，跑完那一格就是这一趟留下的。

这里踩过一次，值得记：第一版把名字定成**只按用途**（`work/run-ll`、`work/run-jit`、
`work/run`），结果 asy 轴上 `cases/95-sig-batch` 的 `run-llvm` 印出了**另一个例子的输出** ——
一条轴上几十个例子是同时跑的，同一条腿共用一个 `a.out`，谁先链完谁的程序就被别人 exec 了。
所以"用途 + 输入哈希"里的**输入哈希不是可选的**：`run` / `run-llvm` / `run-jit` 三处
都按这一趟的源文件路径起名。

跑过的轴：`tests/run.js` 96/0、`tests/asy/run.js` 259/0（五条腿齐跑 426.7s，
就是这一轴抓出了上面那次撞车）、oir 607/0、glr 20/0、wat 12/0、sexpr 75/0、cabi 4/0、
mir 33/0、incr 8/0、js-exec 11/0、jnc 145/0、llvm 22/0、jit 22/0、gpu 17/0（1 跳过）、
bootstrap 60/0 不动点，外加原生二进制上那三种失效的手工核对。

### 第一百〇六刀：44 行源码发出 24200 行 C

上一刀把"编过的东西不再编"解决了，这一刀解决的是**编出来的东西本来就不该编**。

量出来的样子：`tests/asy/cases/01-arith.asy` 44 行，进 C 后端的 OIR 里有 **1066 个函数**，
发出来 **1127357 字节 / 24200 行 C**。这 44 行用得到的是 `write` 与几个算子，其余全是
...
`MakeClosure.make`、闭包描述子、类的方法表），逐种去认，以后加一种节点就漏一条边，
而漏一条边的后果是链接期未定义符号。所以反过来：把活函数的对象图里出现过的**所有字符串**
收一遍，谁的符号名（`f.mangled`）在里头就算被引到。保守 —— 一句字符串常量刚好等于某个
函数名就会把它留下 —— 但摇不错。根是入口加 `funcs` 之外那几格（structs/classes/enums/
containers/boxDeeps/closures/fnTypes）里提到的一切，方法表与闭包描述子就在那些格子里。

量出来：**1066 -> 142 个函数**，C 从 24200 行降到 **4031 行**，摇这一趟本身 **7ms**
...
踩到一件事，值得记：第一版的 `seen` 写成 `WeakSet` —— 这一趟走完就扔，弱引用一点用没有，
而**我们自己的 JS 前端还没有 WeakSet**。`tests/mir` 的 `lower/cli.js` 与 `tests/bootstrap`
当场把它抓出来（`FAIL lower/cli.js`）。改成 `Set` 就好了：这份代码要能被自己编译，
这条约束在写它的时候必须在脑子里。

跑过的轴：run 96/0、asy 259/0、oir 607/0、sexpr 75/0、cabi 4/0、mir 33/0、glr 20/0、
wat 12/0、incr 8/0、js-exec 11/0、jnc 145/0、llvm 22/0、jit 22/0、gpu 17/0（1 跳过）、
bootstrap 60/0 不动点。
...
## 落地：examples 那一轴的**时间**（先分类、再量、再缩）

用户的约束把这一轴的形状定死了：**一个例子 5s、整趟 120s**。原来那一份（`eps.js`）是串行
spawn、每例上限 3s，全量一趟好几分钟 —— 判据与预算都对不上。这一节记的是把时间压下来
的四步，以及每一步量到的数。

### 第一步：例子进自己的目录

`tests/asy/examples/`（229 份，其中 220 份 `.asy` + 9 份数据）。**必须是自己的一份**，
因为后面要改那几个跑不完的 —— 参考树只能读。改了之后 oracle 也从**这一份**生成，
所以两侧跑的仍是同一份源码，逐记号对照照样成立。

### 第二步：静态分类（零成本）与它的**预测力**

`tests/asy/triage.js static` 按源码特征（三维 / 曲面 / 等值线 / 位图 / 动画 / 幻灯 /
TeX / 外部工具 / 循环上界）打分分四档。然后用实测去对，结果是**这套先验很弱**：

```
档   出图  超5s  报错     实测超时率   我预测的
 D     27     9     4       0.23        0.9
 C     36    14     2       0.26        0.5
 B     28     5     3       0.14        0.15
 A     87     3     2       0.03        0.03
...
D/C 两档高估得厉害。原因很具体：`surface`/`three` 在真 asy 里是 C++ 跑的网格，便宜；
真正的重头是 `curvedlabel` / `functionshading` / `spectrum` / `poster` / `worksheet` /
`layers` / `mosaic` 这些**与三维无关**的。所以静态分类只留着当粗筛，**尺子是实测时间**。

### 第三步：并行量两侧 —— 顺带量出「并行度会冤枉例子」

真 asy（5s 上限、220 份）：**出图 178、超 5s 的 31、报错 11**，中位 634ms、最慢 4841ms。
也就是说 5s 预算下能进对照的只有 178 份；那 31 份要么缩规模、要么不计分。

我们这一侧的成本结构：

```
最小例子 draw((0,0)--(1,1))：首跑 1387ms，全热重跑 177ms
gamma.asy 拆开看：前端 35ms、拼产物 113ms、**跑生成的 JS 1502ms**
编译器自己加载 145ms，node 空转 123ms
```

单次成本几乎全是**冷编译 + 跑**，而产物缓存按源码内容做键，所以第一趟付编译、之后便宜。
于是门的形状是「预热一趟（不计分，与 oracle 同类）+ 计分趟并行跑热态」。

**一处必须记下来的量法陷阱**：这台机器 8 个逻辑核里只有 4 个是性能核。开 6 路并行时，
四个本来合格的例子被冤枉成超时：
...
所以 `triage.js` 的并行度默认压到 4（性能核数），不是「核数 - 2」。即便如此，边界附近
的例子在两趟之间还会来回跳（17 个 vs 19 个被打死，名单不同）——**5s 这条线在当前
0.5～2s 的基线上本来就没有余量**，这一条如实记着，它是下一刀（把基线压下去）的理由。

### 第四步：真正跑不完的只有两个，缩规模之后都进限

串行、30s 上限重量那七个"疑似超时"的，只有两个是真的：

```
例子                我们      真 asy    处置
Gouraudcontour     >30000ms   1050ms   不计分（`rand()` 播种，真 asy 自己两趟都不同）
splitpatch         >30000ms   2933ms   缩：递归深度 n=20 -> 8
interpolate1         5198ms   4459ms   缩：七处 graph 采样 n=500 -> 150（asy 自己也贴着线）
其余四个            1.9～4.2s          并行度冤枉的，单跑都在限内
```

缩完再量（两侧都跑缩小后的**同一份**源码）：

```
splitpatch      我们 1301ms   真 asy 1317ms   （从 >30s 下来，而且与 asy 打平）
interpolate1    我们 3001ms   真 asy 1517ms
```

### 量：C 腿 / LLVM 腿在这儿**帮不上忙**

“要不要换 C 或 LLVM 才能达标”这一问是量过才答的：

```
例子                默认 JS 腿    --backend c
interpolate1          3005ms       3763ms    C 更慢
SierpinskiGasket      5003ms       编不过（clang 拒了 C 后端发出来的代码）
```

两条理由：这些例子是**固定成本 + 中等计算**，不是纯计算，而 C 腿要**额外付一次 clang**；
而且它在这套语料上还编不过。结论是：现在的瓶颈不在"跑得快不快"，在**每个例子那半秒到
两秒的固定成本**（node 启动 + 编译器加载 + 再 spawn 一个 node 跑生成的 ESM）。
要动就动那儿 —— 一个进程里连着跑很多个例子，与 `glsl_host.c` 的 `--serve` 是同一条路子。
那是下一刀，不是换后端。
...
### 试过「一律按位置取名」——**退回来了**，但换到两条硬事实

`genSym` 有两套命名（计数器 / 位置派生），而它自己的注释说只有位置那一套跨趟安全。
于是试着把它改成默认。两趟都失败，各暴露一件事：

**第一趟：键里放 `unit.key`** —— 立刻撞名：

```
omni_weak.sx:3173: error: 捕获 'asy__self' 要 marker，这里是 autoscaleT
  (fldset (var this) Tinv (mkclo asy__m9f39b3e5_asy__anon_47598d80 (var this)))
```

一个**单元里内联着好几个源文件**（`plain_markers` 与 `plain_scaling` 都在 `plain` 这个
单元里），只用单元键的话两个文件里偏移相同的两处就是同一个名字。这也解释了原注释里
那条旧记录（`捕获 'asy__self' 要 …_picture，这里是 Label`）—— 同一个坑。

**第二趟：键里换成源文件路径** —— 撞名没了，换成**重复定义**：

```
plain__8c9b5de0.sx:4529: error: 'asy__m9f39b3e5_asy__anon_c6e2dc3e' 重复定义
  (cfn … () ((s string) (p pen) (tex bool) (bbox bool)) (arr path)
```

这一条**比原来的 bug 更有信息量**：同一个源码位置的匿名函数被**降了两遍**。计数器那一套
下这两遍各拿一个新号，所以看不出来；按位置取名就现形了。而"同一个节点降两遍"正是计数器
会漂的根因 —— 两遍之间号往前走了，而"替别的单元补生成"的那些引用点算的是另一批号。

所以正确的修法不是「换命名方案」，是**让同一个节点只降一遍**（或者按节点把闭包记忆化，
第二次要用时拿回同一个名字与同一份 `(cfn …)`）。那一格动的是 `mkClo` 与它的调用方
（`anonFn` / `localFunClo` / `recNew` 的函数值字段 / 默认实参包装），要连着
`tests/asy/run.js` 那 330 个用例一起验。

改动已 `git checkout` 退掉（`import graph3;` 现在照旧出图），这一节只留结论。

<!-- ADR-0014 位置命名试过退回来-END -->

## 量：我们这一侧慢在哪儿 —— **不是启动、不是缓存，是 int 的表示**

用户指出一件对的事：我一直拿真 asy 的 634ms 中位当参考，而**asy 自己并不快**（C++ 的
树遍历解释器 + 每次排标签都 shell 出去跑 LaTeX），拿它当性能标准是错的尺子。
于是把我们这一侧从头量了一遍。

### 第一刀：把 1471ms 拆开，97% 在例子自己那段

`gamma.asy`（真 asy 952ms，我们 1428ms）在生成的入口里逐个 `omni_init_*` 计时：

```
init gamma           1428ms   ← 97%
init asy_builtins       3ms
init plain              1ms
其余 15 个模块          0ms
加载 + 解析 8.9MB JS   ~40ms
```

也就是说**启动、模块加载、库初始化加起来不到 3%**。我先前"瓶颈是每例 0.5～2s 的固定
成本"那个结论只对热态的小例子成立，一到真画图的例子就完全不成立 —— 记下来，别再重复。

### 第二刀：CPU profile 指到数组访问器那一族

```
14.2%  solvesection      asy_builtins.js:1470   路径求解（三对角）
11.3%  $aget             omni_rt.js:254
 5.3%  $aset
 4.5%  (GC)
 4.2%  $anew
 4.0%  $alen
 2.1%  $acopy / $vcopy
 （9.1% 的 spawnSync 是 profile 自己的开销，不算）
```

`$aget/$aset/$alen/$anew/$acopy` 一族 **27%**，比最热的算法函数还多一倍。

### 第三刀：根因不是"多跳一层函数"，是 **int 在 JS 侧是 BigInt**

`omni_rt.js` 里那段注释早就把这一条记下来了（"真正的大头是 int 在 JS 侧是 BigInt"），
这次连着量了一把 —— 顺带**修正它记的倍数**（那是旧 node 上的数，node 26 上不再成立）：

```
数组下标 a[i]（i 是 number）            177ms / 3e7 次
数组下标 a[Number(bigint)]              344ms          -> 1.9 倍
int 加法带回绕 BigInt.asIntN(64,x+1n)   219ms / 3e7 次
int 加法带回绕 (y+1)|0                   37ms          -> 5.9 倍
```

所以两件事都要做，但**顺序反了就白干**：

1. **表示先改**：`int` 不再一律用 BigInt。这与 ADR-0013 第三刀给**解释器**做的是同一件事
   （i32 换 number，靠 `i32Op`/`i32ToU`/`i32Wrap` 三条宿主 op，量到 15 倍），只是 JS
   后端这一侧还没做。算术上是 5.9 倍，这是最大的一块。
2. **访问器再内联**：`$alen` 只是读一个属性却是一次函数调用（4%）；`$aget`/`$aset` 的
   null 检查与边界检查在方言那一层大多是可证的。内联之后 V8 才看得见下标运算本身。

`$anew`/`$acopy` 造出来的垃圾（4.2% + 2.1%，连着 4.5% GC）是第三块：`solvesection`
这类每次都新建临时数组，可以复用缓冲。

**这一节只记量出来的数，改动没做** —— 改的是 `omni_rt.js` 与 `backend-js/emit.js`，
所有语言前端（sexpr / c / jnc / glsl / asy）共用这一份，要连着五腿一致那几条轴一起验。

<!-- ADR-0014 我们慢在哪儿-END -->

### 第四刀：**"内联访问器"这一条量下来是假的** —— 把上一节的第 2 条划掉

上一节把「内联 `$alen`/`$aget`」列成第二块。**先量再改，量出来它不成立**（3e7 次，node 26）：

```
调 $aget(a, i)，下标是 BigInt        1179ms
把 $aget 的正文手写在原地，仍是 BigInt 1310ms   ← 反而更慢
调 $aget(a, i)，下标是 number         119ms
手写在原地，下标是 number             113ms
```

V8 早就把那几个小函数内联掉了，手工展开只是把它的活抢了一半。**一次函数调用不是成本，
BigInt 才是**：同一个循环换成 number 下标就是 10 倍。profile 里 `$aget` 那 11.3% 不是
"调用开销"，是它体内那句 `Number(i)` 与调用方那些 BigInt 运算。

再把成本拆细一层（2e7 次）：

```
全 number 的循环                        20ms
只把计数器换成 BigInt（自增 + 比较）    357ms
只把 Number(bigint) 那一次转换留下      178ms
全 BigInt（计数器 + 取模 + 下标转换）   526ms
```

也就是说**没有便宜的半步**：BigInt 只要出现在热循环里，无论是算术还是边界转换，都要付。
`int` 的表示是唯一的入口。

**改法定下来了（还没做）**：`int` 在 JS 侧收成**规范化的 number|BigInt** ——
`|v| <= 2^53-1` 时是 number，否则是 BigInt。规范化那一条是关键：两种表示的取值范围不重叠，
于是 `===` 仍然对（`1` 与 `1n` 不会同时存在）、Map 的键仍然唯一、`String()` 仍然一样。
`+ - *` 的快路上算完查一次 `Math.abs(r) < 2^53`（IEEE 是正确舍入的，真值 ≥ 2^53+1 时
算出来必然 ≥ 2^53，所以这一查是**充分**的），不过关就回 BigInt 重算一遍；`/` 与 `%`
在安全整数上本来就精确（真商与整数的距离 ≥ 1/|b|，而 |b|·q > 2^53 才可能被舍掉）。

代价与风险写在明处，这是它还没落地的原因：`dynamic` 那一族**拿 `typeof v === "bigint"`
当 int 的标签**（prelude 的 `$dtype`/`$dynEq`/`$dbool`/`$dynBit`），所以装箱边界上必须
转回 BigInt —— 与 ADR-0013 第二刀那条 `CVT_SEXT` 装箱是同一类账（那边混着 i64 的程序
反而慢了 1.4 倍）。还有 `getBigInt64`/`setBigInt64` 那一族（指针、线性内存、缓冲）、
`$str_base`、`tests/oracle/int64_wrap`（Python 当尺子，要求逐位 64 位回绕）。

<!-- ADR-0014 内联访问器是假的-END -->

### 第五刀：**一样 24 -> 66** —— 差的不是画法，是 `_mainname()` 在库命中缓存时丢了

上一轮那 51 份"只有数值差"里，**42 份整个只差一个词**：参考写 `(<例子名>_.dvi)`，
我们写 `(t_.dvi)`。那个词是 dvips 写进 EPS **正文**的（不是注释），所以它算一处差。

根因不在 TeX 那一段，在**登记时机**：`_mainname()` 的那格 weak 全局与 getter 由
`calls.js` 按"谁调它谁登记"生成，而调它的是 `asy_builtins.asy`（outprefix 那一句）。
库命中产物缓存的那一趟**根本不降 asy_builtins**，于是 `arrGen` 里没这一格，
`lower.js` 里那句

```js
if (this.arrGen.has('asy__mainname')) main.unshift('(set asy__mainname_v (str "<例子名>"))');
```

就不发 —— 名字留空，`asy_builtins.asy:6903` 那道 `if (pre == "") pre = "t"` 兜住，
前缀退成 `t`。判据是同一趟里 **equilateral 对、Pythagoras 错**：差的是"这一趟有没有
重编到库"，与例子本身无关。

改法：登记**无条件**（全局 + getter 在发 weak 之前补上），那句 `(set …)` 也无条件发。
代价是每个 asy 程序多一格没人读的全局。量出来的（`OMNI_EPS_T=5000`，全量 196 份参考）：
...
### 第六刀：那 86 份"结构不同"里 **78 份的参考是位图** —— 口径本身不对等

这一条是这一轮最要紧的发现，也是先前把"3D 保真"排在第一位的那个判断的**更正**。
拆开参考自己：

```
.omni-cache/epsref 共 196 份
  含 `/ImageType 1 … image`（正文是一张位图）  101 份
  纯矢量                                        95 份
```

真 asy 出 EPS 时**三维那一族是渲染成位图再嵌进去的**（`settings.render` 默认 -1 =
自动，这台机器有 OpenGL，于是走 glrender 那条路）。量出来的样子（unitoctant）：
`156 x 147` bp 的框里嵌一张 `624 x 588` 的 `/DeviceRGB` 图（正好 4 倍超采样），
外面套 `ASCII85Decode + FlateDecode`。我们这一侧 `settings.render = 0`（没有 OpenGL），
走的是**矢量**那条路 —— 把每一块 patch 投影成填充路径。

于是按"数 + 运算符"的流去比，两边根本不是同一种产物：

```
旧结论里"结构不同" 86 份
  参考是位图的      78 份   <- 这一格比的是"一张图" vs "几万条路径"，永远不会一样
  参考是矢量的       8 份   <- 这才是真该修的：advection alignedaxis cardioid cards
                              interpolate1 limit phase（外加 colorplanes）
```

所以"3D 结构不同 85 份"这句话是**口径错**，不是我们把三维画错了。两条出路，都要一个
带 z 缓冲的三角形光栅器：

1. **把 asy 那条路补上**：三维场景渲染成位图 -> zlib -> ASCII85 -> `/ImageType 1`。
   零件手上有（`src/jit/png.c` 里已经有 deflate，ASCII85 是十几行），这条走通之后
   同一条 token 对照就重新成立，而且很可能**顺带解决超时**（几万条路径的 EPS 本来就慢）。
2. **换一条判据**：位图参考那 101 份改成"两边各自光栅化后比像素"。

<!-- ADR-0014 一样24到66与位图口径-END -->

### 第七刀：产物缓存不一致的**根因抓到了** —— 记录名依赖"这一趟谁先被降"

改完上一格之后重跑，7 份例子从"结构不同"掉成"没出图"，错法都是同一族：

```
omni_weak.sx:1505:5: error: 类 segment 没有字段 'A' —— 有的是 active / a / b / c / edge
      (fldset (var this) A (call asy__new_point))
```

清掉 `.omni-cache/asy-mods` 再跑，**同一份源码、同一个例子，好了**（连跑两趟都好）。
所以这不是我上一格改出来的错，是它**动了 weak 那一份的正文、于是掀开了一个一直在的坑**。

坑在 `frontend-asy/lower.js` 的 `recUniq`：

```js
recUniq(nm) {
  if (this.unit.tpl === null && !this.records.has(nm)) return nm;   // <- 这一句
  let t = `${this.unit.pfx}${nm}`;
  ...
}
```

**"这一趟还没人叫这个名字"就把裸名字给它。** 于是同一个库的类名取决于*程序里谁先被降*：

- `contour.asy` 里有 `private struct segment { bool active; pair a, b; int c, edge; }`
- `geometry.asy` 里有 `struct segment { point A, B; … }`

谁先降，谁拿到裸名 `segment`，另一个才拿 `asy__m<哈希>_segment`。而生成的构造函数名字是
`asy__new_<类名>` —— 于是 `asy__new_segment` 这个名字**在不同程序里指的是不同的类**，
而它落在**共用的 weak 那一份**里。一边命中缓存、一边重编，两份就对不上了。

这与「genSym 一律按位置取名」那次退回来的教训是同一条：**产物里的名字只许由这一份自己的
身份决定**，不许由"这一趟的程序长什么样"决定。上一轮记的那 48 份"没出图 = 缓存不一致"，
根因就是这一格（当时只查到"全量重建能修"，没查到为什么）。

改法：记录名一律带自己单元的前缀（主文件那一格前缀是空的，天然唯一），
同名冲突的兜底 `asy__sh<k>_` 也带上前缀。代价是全部产物名字变一次（缓存重建一次），
换来的是"一个库一份产物"这句话真的成立。

<!-- ADR-0014 记录名的根因-END -->

### 第八刀：两种"给类名加前缀"的改法**都退回来了**，先把噪声源掐掉

上一格查清了根因（记录名依赖"这一趟谁先被降"）。改它试了**两种**，各量一次，都退回来了：

```
                                asy 轴     EPS 轴（一样 / 只有数值差 / 结构不同）
不改（顺序依赖还在）            259 / 0        66 / 9 / 86
(1) 一律带单元前缀              258 / 1        16 / 34 / 113
(2) 裸名只给主文件 + 内建面      -  / 1        18 / 34 / 112
```

两种都要跟着补"拿源码名当真名用"的地方，量出来四处（每一处都有确定的形状）：
`recCtorOf`（嵌套 struct 不在 recVis 里 —— `plain_picture.asy:496` 的
`nodes3.push(node3(d))` 报"内建函数 'node3'"）、`decls.js` 的 `asyDeclTyName`
（`plain_filldraw.asy:132` 的 `filltype(filltype.Fill, …)` 报"带点的名字或算符名"）、
`statQual`（点号左边是源码名、`statOf` 查真名）、`exprs.js` 里写死的 `cycleToken` / `code`
（`graph3.asy:1571` 的 `join(g,cycle)` 丢了那格隐式转换）。四处都补上之后**两种顺序都出图、
两趟字节数一模一样**（41079 / 136161），顺序依赖确实没了 —— 但 EPS 那一轴从 66 掉到 18。

所以还有一批按**库类型的名字**做的判断没找齐（`.aif` 接口索引里存的是名字、跨单元的
cast 表也按类型字符串配）。这不是"再补一处"能收的，是一次真正的重构。**两次失败之后
按纪律停手**：退回、把这一格的账记在这里，别把主线指标压在一个没查完的改动上。

**先掐掉噪声源**（这一格留下了）：`tests/asy/eps.js` 在"全量 + `OMNI_EPS_FRESH=1`"这一档
开跑前把 `.omni-cache/asy-mods` 清掉。理由是量出来的：上一个例子留下的产物会让下一个
例子编不过（先 Pythagoras 后 impact -> `类 segment 没有字段 'A'`），一趟全量里这么冤枉掉
7 个例子（"结构不同"变"没出图"）。根因没改，但**报出来的数不能是脏缓存上跑出来的**。

下一刀该往哪儿走（记在明处，别再从"改名字"这一头进）：**把命名的上下文并进产物的印记** ——
一份产物如果没拿到它源码里那个名字（被别人先占了），它就不是"那个库的规范产物"，
不该覆盖也不该被另一个上下文的程序复用。判据在降之前就知道（这一趟这个名字被占了没有），
...
### 第十一刀：量清"超 5s"那 18 个 —— **15 个的参考是位图，而我们在吐几十倍大的矢量**

把 18 个逐个单跑（90s 上限，串行，不并行免得互相冤枉），连着出图的字节数与参考对比：

```
名字                    参考是   参考字节    我们字节     倍数   我们用时
AiryDisk                位图      152057   10055972      x66   17054ms
Klein                   位图      650825          0        -   >90s（打死）
SierpinskiSponge        位图      382671   14122993      x37   39890ms
cheese                  位图      316569    2767684       x9   28238ms
curvedlabel3            位图       99885    4529251      x45   20107ms
filesurface             位图     1132188    2569883       x2    6549ms
gamma3                  位图      769656    4875464       x6    7306ms
genusthree              位图       77548          0        -   >90s（打死）
genustwo                位图      116065          0        -   >90s（打死）
hyperboloidsilhouette   矢量       31679      31648       x1   12047ms
magnetic                位图       99543    4090838      x41    6309ms
pdb                     位图      349662      79385      x0    52298ms
pipeintersection        位图      171614   12501647      x73   16294ms
sinc                    位图      199568     201434       x1    9384ms
soccerball              位图      256551    4022760      x16   10149ms
spheresilhouette        矢量       30956      30967       x1   17052ms
trefoilknot             位图      536907   14902226      x28   （见上一行族）
worldmap                矢量     1951545    1951507       x1
```

两条结论，都不是猜的：

1. **18 个里 15 个的参考是位图**（第六刀那一格的直接后果）。真 asy 把三维渲染成一张
   624x588 的图嵌进去；我们 `settings.render = 0` 走矢量，于是把几万块 patch 投影成填充
   路径 —— 输出比参考大 **2 到 73 倍**，时间就花在生成与写这十几 MB 的 PostScript 上。
   也就是说这一档的"慢"与"结构不同"是**同一件事**：口径不对等。
2. **矢量参考那 3 个（hyperboloidsilhouette / spheresilhouette / worldmap）我们的字节数与
   参考几乎一样**（x1，差几十字节）—— 它们画得对，只是慢。这三个才是"我们这一侧的性能"
   该盯的样本，前面 15 个不是。

所以下一刀的次序定了：**先补三维的光栅 shipout**（渲染成位图 -> zlib -> ASCII85 ->
`/ImageType 1`），它同时收掉两件事 —— 那 15 个的口径与它们的时间（时间从"patch 数"变成
"像素数"，而像素数是固定的 624x588）。零件手上有：`src/jit/png.c` 里已经有 deflate，
ASCII85 十几行，三角形光栅 + z 缓冲是 ADR-0019 那条软光栅路上的东西。
`int` 换表示那一刀（第三刀/第四刀量的 5.9 倍）留给"矢量那三个"与标签那一路，
它不是这 15 个的解。
...
### 第十二刀：位图那一格**落地了** —— `_image` 出真的 `/ImageType 1`，gs 认，像素能比了

按上面那张施工图做完了，`abort("_image 还没做")` 换成真的输出：

- ASCII85 编码（`asy__a85grp` / `asy__emitpixels`）：4 字节一组 5 个字符、全零那组缩成 `z`、
  末组补零只发 n+1 个字符、结尾 `~>`。85 个字符靠一张常量表 + `substr` 取，不需要 `chr`。
- 那块字典（`asy__emitimg`）照参考逐字发，**只少 `/FlateDecode` 那一行** ——
  `/ASCII85Decode` 单独就是合法 EPS。
- `drawop` 多一格 `kind == 5` 与 `pen[][] img`；界那一段不用改（`opbox` 的 else 支就是
  `pathbox(o.g)`，而 `wide` 只对 `kind == 0` 成立）；`transform * frame` 里像素不跟着变、
  只变那四个角。
- 有效变换是 `t * matrix(initial, final)` —— 照 `runpicture.in:395-411` 三个重载与
  `drawimage.h:26` 的界（`t*(0,0)` 与 `t*(1,1)`）定的，也就是说 `initial`/`final` 只用来
  **造那个变换**。四个角都算（不只两个），于是旋转过的图跟着白捡。
- 值 -> 色板那一格照 `psfile.cc:604` 抄：`step=(色板格数-1)/(max-min)`、下标 `(int)((v-min)*step)`
  截断夹住；空色板按 min..max 铺灰。三个重载（`real[][]+palette` / `pen[][]` / `pen F(int,int)`）
  都落在同一格上。

**验（这一格第一次有了像素这把尺子）**：laserlattice.asy 从"没出图 —— abort"变成出图
1093394 字节、**4 块图**（参考也是 4 块）；`gs -sDEVICE=ppmraw -r36 -dEPSCrop` 两边都渲染
得出来（也就是说这块 PostScript 是合法的），270x270 逐像素比：

```
              平均绝对差   |差<=8 的占比
原样             45.03         63.5%
上下翻转         54.21         58.1%     <- 比原样差，说明行序是对的
左右翻转         93.76         42.9%
```

所以**摆放与朝向是对的**（翻转都更差），剩下的 45 是**内容**的差 —— 大头该在色板那一格
（laserlattice 走的是 `palette.asy` 的 `pdata` 那条路，与我们照 psfile.cc 抄的那条口径可能
不是同一处）与标签那一路。这一条从"看不见"变成了"一个能追的数"，往下就按这个数走。

顺带：这也是三维光栅 shipout 要用的那半 —— 那边只差"把 patch 光栅化成这块像素"。

<!-- ADR-0014 位图那一格落地-END -->

### 第十三刀：那 45 的像素差追到底 —— 两个都是"照抄没抄对"，收到**逐字节相同**

上一刀留下的数是"平均绝对差 45.03、完全相同 45.8%"。按这个数往下追，工具是自己写的
十行解码器（ASCII85 -> `zlib.inflateSync` -> 像素字节），**不经过 gs**，于是比的是像素本身：

先排除了三件事（每一件都量过，不是猜的）：

- **不是朝向**：原样 45.8% 相同，上下翻转 31.8%、左右翻转 26.1%、转置 12.0% —— 原样最好。
- **不是色板**：`BWRainbow()` 两边都是 32761 格，头五格的 rgb 逐字相同。
- **不是数据**：把例子里的 `pot()` 单独跑一遍，`min=0 max=1`、抽样值逐字相同。
- **不是块序**：四块图两两配，对角线最好（45.8/45.6/48.6/40.2 对其余 13~17）。

真凶两个，都是"照参考抄"抄漏了一处：

**（1）ASCII85 的 85 个字符那张表多了一个字符。** 写的是 `"…[\\]^_`…"` —— 而 **asy 的字符串
字面量里 `\` 不是转义引导**：`"[\\]"` 的 length 是 3（真 asy 与我们都印 3，量过）。于是表长
86，`\` 之后每个下标偏一位。这个错法很能骗人：尺寸、朝向、摆放、块序全对，逐字节还有
45.8% 相同（平坦的地方偏一位仍是同一个颜色），只有颜色变化处冒出**色板里根本没有的颜色**
（像素 0 我们 `(11,246,144)`，反查色板 `undefined`；参考 `(11,0,11)` 是第 225 格）。
改成一个反斜杠：平均差 45.03 -> **0.156**，`<=8` 的占比 100%。

**（2）0..1 -> 一个字节那一步。** 我按 `round(v*255)` 写，而 asy 是 `pen.h:143` 的
`byte()`：**截断 `v*256`、夹到 255**。剩下那 0.15 全是 ±1，换成这一条之后：

```
laserlattice 四块图（256x256x3 = 196608 字节/块），解出来逐字节比：
  第 0 块  完全相同 100.00%   最大差 0
  第 1 块  完全相同 100.00%   最大差 0
  第 2 块  完全相同 100.00%   最大差 0
  第 3 块  完全相同 100.00%   最大差 0
```

**四块图与真 asy 逐字节相同。** 顺带把色板下标那一格也对齐了（`psfile.cc:611` 是
`(size_t)((val-min)*step+0.5)`，四舍五入不是截断 —— 单独改它只动了 0.3 个百分点，
但它是对的，留着）。

这一格的意义不只是 laserlattice：三维那 101 份位图参考要的**编码这一半已经逐字节对上了**，
剩下的只有"把 patch 光栅化成那块像素"。

<!-- ADR-0014 像素差追到底-END -->

### 第十四刀：位图那一档的**判据换了** —— 不比那段字节，比解出来的像素

`_image` 落地之后 laserlattice 仍被判"结构不同"，而它的四块图与参考**逐字节相同** ——
判据本身不对：参考那侧多一层 `/FlateDecode`，我们只发 `/ASCII85Decode`，那一段字节两边
必然不同。按 token 比等于把"图对不对"永远判成"结构不同"。

`tests/asy/eps.js` 里加了三件（十几行，`node:zlib` 现成）：

- `pullImages(text)`：把 `image` 到 `~>` 之间的数据段摘出来（`/ASCII85Decode` -> 需要时
  `inflateSync`），正文里换成一行 `%IMGDATA`，并且把**编码那两行**（`/DataSource …` 与
  `/FlateDecode`）也摘掉 —— 滤镜是编码细节。`Width` / `Height` / `BitsPerComponent` /
  `Decode` / `ImageMatrix` 与外面那句 `concat` 照旧参与比较，那些是真的几何。
- `cmpImages`：逐块比块数、字节数、逐字节；不一样就说清"第几块、几个字节不同、最大差多少、
  首处在第几个字节"。
- 判定：矢量那半边一样且位图也一样才算"一样"；矢量一样而位图有差，记成
  "矢量那半边一样，第 k 块位图 …"。

**这一换立刻把剩下的活说清楚了**：`位图块数 1 vs 0` 出现在 **82** 个例子里 —— 参考有一张图、
我们一张都没有（三维那一族）。而且那些参考的**矢量部分只有 70 个 token**（图之外就剩几句），
我们反倒有 354 / 5226 / 104 个（几万块 patch 投出来的路径）。也就是说：**把三维渲染成那张图
之后，这 82 个例子的两侧都会变成"一张图 + 几句"**，它们是"一样"的候选，而不是永远的
"结构不同"。

这一趟的数（干净产物缓存）：一样 68、只有数值差 9、结构不同 90、没出图 4、超 5s 18。
laserlattice 从"位图对不上"变成一条**真的矢量差**（`%%HiResBoundingBox` 35.456 vs 35.219、
token 7062 vs 6513）—— 那是标签与坐标轴那一路的账，能接着追。

<!-- ADR-0014 位图判据换成比像素-END -->

### 第十五刀：buildcycle 那**一个** token 的差 —— TeX 的 .log 每 79 列硬折一次

`buildcycle` 是"只有数值差"里最小的一个：**整份 EPS 只差 1 个数**（#2962 参考 3437、
我们 3419）。顺着它把生成的 TeX 抄出来比（真 asy 用 `-k` 留下 `ref_.tex`，我们的在
`/tmp/omni-asytex/buildcycle_.tex`），五句标签里**只有一句不同**：

```
参考   \ASYalign(64.060136,20.273036)(-0.500000,-1.000000){$S$}
我们   \ASYalign(64.060136,20.273036)(-0.500000,-0.733334){$S$}
```

位置一字不差，差的是**第二对里的 y**（对齐系数）。按 asy 的公式（drawlabel.cc:106-115）：
`Align=(0,-1)` -> `*0.5` -> `-(0.5,0.5)` = `(-0.5,-1.0)`，然后
`Depth = -depth*Align.y`、`if (Depth > 0) texAlign += (0, Depth/(height+depth))`。
参考印 -1.0 说明它那边 **depth = 0**（`$S$` 没有下伸部）；我们多加了 0.266666。

缩到三行复现（`label(f,"$S$",(0,0))` 之后看 `max(f).y`）：

```
                        真 asy                我们（改前）
$S$（数学模式）      5.58019925280199     6.47683188044832    <- 差在这里
$Q$（有下伸部）      6.74249564134496     6.74249564134496
S（不带 $）          5.58019925280199     5.58019925280199
```

再问 TeX 自己（手写一份 m.tex）：`\hbox{$S$}` 是 `wd 7.92493pt ht 8.2pt dp 0.0pt`。
而我们缓存里记的是 **`87.45552 7.33333 2.66666`** —— 宽是**两截粘起来的**，深凭空多了 2.67pt。

根因：**TeX 的 .log 每 79 列硬折一次，而且折的时候不插空格**。于是
`>dim(7.92493pt)dim` 会被折成 `>dim(7.9249` + 换行 + `3pt)dim`，`find(">dim(")` 少认一格，
后面整批错位一位（`87.45552` 就是前一个数的尾巴 `8` 与下一个数粘起来的）。改法一行：
读完 .log **先把换行去掉**（折行不加空格，所以去掉换行正好把数拼回原样）。

改后 `$S$` 的三个数与 TeX 逐字一致（`7.92493 8.2 0.0`），`max(f).y` 与真 asy 逐位相同。
这不是一个例子的事：**凡是那一行正好折在数字里的标签，量出来的宽/高/深都是错的** ——
`fjortoft`（4 处）、`mosquito`（6 处）差的数都落在同一量级（±15~18，dvips 那一路的位置）。

<!-- ADR-0014 标签盒子数学模式量高了-END -->

### 第十六刀：最后那一个"数值不同" —— `2.5E+0.1*N` 里的 `E` 是**东**，不是指数

上一刀之后"只有数值差"只剩 `fano`（2 处）。同一招（比两边生成的 TeX）一下就指到那一句：

```
参考   \ASYalign(59.214844,36.118201)(0.000000,-0.480000){6}
我们   \ASYalign(50.187500,36.659842)(-0.500000,0.000000){6}
```

`fano.asy:25` 写的是 `label("6", z6, 2.5E+0.1*N);` —— asy 那边这个对齐向量是
**`2.5*E + 0.1*N` = (2.5, 0.1)**（`E`/`N` 是 plain 里的方向常量，而 asy 有**隐式相乘**：
`2pi` 就是 `2*pi`）。按 drawlabel 的公式 `scale0 = 2.5` ->
`Align = (0.5, 0.02) - (0.5, 0.5) = (0, -0.48)`，与参考印的 `(0.000000,-0.480000)` 逐位对上。

我们印的 `(-0.5, 0)` 反推回去是 `align = (0, 0.25)` —— 把 `2.5E+0` 当成了**一个浮点字面量**
（2.5），后面 `.1*N` 再乘上去：`2.5 * 0.1 * N = 0.25*N`。

根因在**词法规格**（`frontend-asy/asy.grammar:93-94`）：那两条 LIT 规则写的是
`(? (or "e" "E") …)` —— 收了大写 E 当指数标记。而 camp.l:392 那一行的四个分支里
**写的都是小写 `e`**：

```
([0-9]*\.[0-9]+)|([0-9]+\.[0-9]*)|([0-9]*\.*[0-9]+e[-+]*[0-9]+)|([0-9]+\.[0-9]*e[-+]*[0-9]+)
```

这不是抄漏，是有用处的：大写 `E` 要留给"东"。把那两条里的 `(or "e" "E")` 收成 `"e"` 之后，
fano 那一句发出来的 TeX 与参考**逐字相同**。改的只有 asy 自己那份语法（词法规格是随语法
...
## 量：剩下那四个卡在**哪一份 libm** 上（V8 的 Math.cos 在这一点上是错的）

<!-- ADR-0014 哪份 libm-BEGIN -->

接着上一刀的线索往下量。`cardioid` 的 `t.xx` 来自 `calculateScaling`（纯 asy 的两变量单纯形），
把 `plain_scaling.asy` 抄一份到 `/tmp`、把喂给 LP 的每一条 coord 与解都印出来（17 位有效数字）：

```
                     真 asy                        我们
  MIN y user    -1.2990381472196539           -1.2990381472196542
  MAX y user     1.2990381472196537 (ts 0.25)  —— 这一条我们没有
                 1.2990381472196535 (ts 5.23)  1.2990381472196537 (ts 5.23)
  SOL  a        38.297566631496153            38.297566631496146
```

少掉的那一条不是漏了：`maxcoords(coords, operator <=)` 只留**不被支配**的那些，asy 那边
`(…537, 0.25)` 与 `(…535, 5.23)` 互不支配所以都留，我们那边两条的 user 值撞成同一个数，
一条就把另一条支配掉了。**所以根子在 user 值的最后一个 bit 上。**

再往下一层：`polargraph(f,0,2pi,operator ..)` 的 103 个结点/控制点，逐个印出来比 ——
点大多对得上，十几处控制点差 1 ulp。而 `polargraph` 里唯一的超越函数是 `expi(t)`：

```asy
for(int i=0; i < 30; ++i) { real t=2*pi*i/100; write(cos(t), sin(t), exp(t), log(t+1)); }
```

30 个样本里差 2 处：`i=13` 的 `cos`、`i=27` 的 `log`。谁错？拿第三把尺子（Python 的
`decimal`，60 位精度算同一个 double 输入的泰勒级数）：

```
  t = 0x1.a23574d659f8ap-1                    （2*pi*13/100，两边同一个 double）
  精确值        0.68454710592868866457052968…
  正确舍入      0x1.5e7cf55112013p-1  = 0.6845471059286886
  macOS libm    0x1.5e7cf55112013p-1          <- 正确舍入
  V8 Math.cos   0.68454710592868873           <- 高 1 ulp，**错的**
```

**结论**：这不是我们的降级错了，是**两份 libm 不一样**，而且在这一点上宿主（V8 的 fdlibm
移植）比 macOS libm 差 1 ulp。参考图是本机 `asy` 出的，吃的是 macOS libm；我们这条腿吃 V8。

这条差把四个例子按住了（`cardioid` / `gamma` / `lmfit1` / `laserlattice`）：路径界差 1 ulp
...
## 已落地：下标那两道检查用 undefined 当哨兵（内核 94.9ms -> 34.6ms，天花板 31.3ms）

<!-- ADR-0014 哨兵-BEGIN -->

上一节剩下的那 2.4× 就是 `$aget`/`$aset` 里的空指针与边界检查。**不能**把检查删掉 ——
那两句消息在五条腿上各有一份实现，是判据。要的是"同一句话、更便宜地算出来"。

改法：热路上用 **undefined 当哨兵**。数组是密的（`$anew` 一个个 push 出来），而方言的
每种元素类型都有零值，所以"读回来是 undefined"等价于"这个下标不在界内"：

```js
function $aget(a, i) {
  if (a === null) $rt_error("null reference");
  const v = a[i];
  return v === undefined ? $agetslow(a, i) : v;   // 慢路照老规矩算一遍
}
```

热路于是只剩一次读 + 一次比（原来是 `typeof i` 分支 + `Number(i)` + 两次范围比 +
一次 `a.length` 读）。三种情形落到慢路，消息**一个字不变**：

- 下标越界或负数 —— `a[-1]` / `a[5]` 都是 undefined
- 下标是 BigInt —— `a[5n]` 取的是属性 `"5"`，值是对的；大到印不成下标时是 undefined
- 元素真的是 undefined —— 只有 `arr<dynamic>` 装得进（JS 前端的 `js_undef`），
  慢路照样把它交出去

`$bget`/`$bset` 同一条（缓冲的元素只有 int / real，零值都是 0）。
...
```
  现状（空指针 + 边界）   94.9ms
  哨兵                   34.6ms   -> 2.7x
  手写 a[i]              31.3ms   <- 天花板，还差 10%
```

例子上（min of 3，与上一节同一台机器）：`sinc` 11.23s -> 9.68s、
`AiryDisk` 15.03s -> 10.39s。两刀合起来：`sinc` 17.34 -> 9.68、`AiryDisk` 31.86 -> 10.39。

**顺手补了三条门**：`tests/sexpr/rt/arr-oob`、`rt/arr-neg`、`rt/buf-oob`。
这三句话从前**一条门都没有** —— 只有指针与线性内存那三句有。改动把它们从快路挪到慢路，
而消息在五条腿上各有一份实现：没有门，这次改动等于把那三句话交给运气。
...
## 量：EPS 那一轴的现状 —— 一样 82，只有数值差 0，能碰的非数值差清零

<!-- ADR-0014 轴现状-BEGIN -->

```
  一样 82   只有数值差 0   结构不同 86   没出图 1   超过 3000ms 20
  （没有参考、不计分 24；摇骰子、比不出来 7；189 个全部现跑，254s）
```

上一趟（三刀性能之前、3000ms 同一条线）是 **一样 75、结构不同 45、超时 68** ——
超时那 68 个里绝大多数现在跑进了限内，于是"结构不同"那一栏从 45 涨到 86：
那不是退步，是原来**没跑完所以没算分**的那些落了地（它们几乎全是三维栅格）。
strokepath / textpath 这两刀之后是 81 -> 82，而"只有数值差"那一栏**清零**了。

结构不同的 86 个里，**83 个的差异摘要里带"位图块数 1 vs 0"** —— 参考是 dvips 嵌进去的
位图（三维那一族走 `_image`/渲染），逐字节对不上是这一轴的边界，不是缺陷。
剩下的 **3 个**（cardioid / gamma / lmfit1）是同一个签名：

```
  cardioid  #4166 参考 gsave   vs 我们 newpath
  gamma     #3040 参考 gsave   vs 我们 newpath
  lmfit1    #2496 参考 newpath vs 我们 gsave      <- 反向
```

也就是"笔上那个变换是不是精确单位"那一格（`asy__istrans`，见前面那一节的长注释）。
这一趟把它**量到根上**了。cardioid，`size(0,100)` 之后：

```
              t.xx                    inverse(t).xx           t.xx*ti.xx
  我们   36.380187647154102      0.027487488786447384      1
  真 asy 36.380187647154116      0.02748748878644737       0.99999999999999989
```

`t.xx` 本身就差 **1 ulp** —— 差在 `calculateTransform` 之前，也就是 cardioid 那条路径的
**界**（`polargraph(f,0,2pi)` 里 `f(t)=1+cos(t)`，采样点过 `cos`）。于是 asy 那边
`shiftless(t*T*tinv)` 带着 -1.11e-16 的零头、每条刻度线都套 `gsave`，我们这边正好是单位、
一个都不套。**判据不是"谁更准"，是"哪一份 t.xx"** —— 改 `asy__istrans` 的松紧没有意义
（上一轮量过：cardioid 好了、alignedaxis 坏了）。

顺手把这几个里的 cardioid 拿 **run-c 那条腿**（链真 libm）跑了一遍：前 3067 个 token 与参考
相同，第一处差是 `1.89843058e-17` vs `2.46521741e-17` —— 一个近零坐标，正是前面
「FMA」那一节记的 clang `-ffp-contract=fast` 那一档；而那 82 个 token 的长度差（gsave 块）
仍然在。所以这几个例子要的是**两件**：FMA 的合并方式，与 `cos` 这一族的最后一位
（JS 那条腿上还多一层 V8 vs libm，见「哪份 libm」那一节）。

**结论**：这一轴上"能靠读源码对上的差"已经清零了 —— 剩下的是 83 个位图、
...
**第 0 步的范围里还有一处**（顺着读出来的）：`add(frame dest, frame src)`
（asy_builtins.asy:2472）搬 `ops` 与 `labs`，但 **`has3`/`min3v`/`max3v`/`minr`/`maxr`
一格都不搬**，`prepend`（:6546）同样。也就是说 `add(f,g)` 之后 `min3(f)` 回的是
(0,0,0)。现在没炸是因为 three.asy 走到 `min3` 的那几条路上帧都是直接 `draw` 出来的；
...
**实现完之后量出来的：错的不是着色，是投影的缩放（约 2.35 倍）。**
把两侧的位图都解出来、量"非白像素的包围盒"（billboard，372x400）：

```
  参考   非白 13841 个，x 0..370   y 8..390     （铺满整幅）
  我们   非白  3160 个，x 105..260 y 130..295   （只占中间 156x166）
```

371/156 ≈ 2.38、383/166 ≈ 2.31 —— 一个**均匀的 2.35 倍左右**，而且是居中的：
说明 `xmax-xmin`（也就是 `r = H/zoom`）比该有的大一倍多，几何本身没歪。
所以下一刀先钉这一格，**不要去碰着色** —— 之前"补原语几乎不动那个数"的结论也由此解释：
ink 与参考基本不重叠，补什么都不动。三个待查的嫌疑，按可能性排：

- `zoom` 被算了两次：three.asy:2908 的 `fov=2aTan(Tan(0.5*P.angle)*P.zoom)` **已经把
  zoom 折进 fov 了**，我们又乘了一次 `zoominv`；
- `near` 取错了一头：glrender 是 `frustum(…,-Zmax,-Zmin)`，我拿 `near=-Zmax`
  当近平面，要确认 `m`/`M` 这一对在 three.asy:2889-2895 里是不是就是"近/远"；
- `H` 的角度单位（已核对过一次：度，`renderBase.cc:47` 是 `angle*radians`）。

判据是现成的：这三个改动里哪一个能把上面那个包围盒撑到 `0..370 / 8..390`。

**已经试过、都不是（省下重复试探）：**

- 把 `t` 当 modelview 乘到点上 -> 443484/446400，整幅糊掉（首处就是第 0 个像素）；
- 把 `m`/`M` 当"投影后的比"（视口 = `[m.x,M.x]x[m.y,M.y]`、点只除 z 不乘近平面）
  -> ink 只剩 **12** 个像素、缩成 4x4 的一小团。所以 `m`/`M` 确实是**视图空间的三维盒子**，
  原来那套 frustum 的形状是对的；
- `zoom`：默认 `P.zoom=1`，`zoominv` 不起作用，排除。

也就是说形状对、位置居中、只差一个均匀倍数 —— **四个数印出来了**（billboard，
`asy__r3ang/zoom/m/M` 那几格）：

```
  angle = 66.968063089442211 （度）   zoom = 1
  m = (-68.565460908155416, -56.73904390816233,  -298.47269223575057)
  M = ( 68.565647666550859,  56.739495533753256,  -64.962368243589196)
  w/h = 92.98 / 99.98
```

按这几个数手算：`tan(0.5*66.968°) = 0.6620`，`|Zmax| = 64.9624`，于是
`H = 43.00`、`rAspect = H*0.93 = 39.99`，视口是 `x ∈ [-39.99, 39.99]`。
而 ink 只占 42%，也就是我们投出来的 |x| 最大约 **16.8**。可是盒子的 x 半宽是 68.57：
若几何真在视图空间里，近脸（|z|=64.96）那些点该投到 ≈68.57，**落在视口外**，
不是内缩到 16.8。**结论：传进来的帧不在视图空间**（`three.asy:2883` 的
`f=modelview*f` 只在 `P.absolute` 时才做），几何还是用户坐标；而 `m`/`M` 是视图空间的。

所以缺的确实是那一次 modelview，只是**方向弄反了**：直接乘 `t` 会糊掉整幅
（试过），下一刀试 `inverse(t)` —— `t` 是 `tinv*inv` 那一对里的一个，
名字本身就说明它是"回去"的那一半。判据照旧：ink 的包围盒能否撑到 `0..370 / 8..390`。

**`inverse(t)` 也试过了：ink 变成 0 个像素**（全落到视口外）。两个方向都不对，
所以问题不是"正着乘还是反着乘"，而是**那个矩阵的约定**：glrender 的 `viewMat`
是 GLM 的 `dmat4`（**列主序**），而这一层的 `real[][] * triple` 是按行读的；
带平移的矩阵上"转置"不等于"求逆"，所以两次都错。下一刀该做的两件：

1. 把 `t` 那十六个数直接印出来，看平移落在最后一行还是最后一列 —— 就知道主序；
2. 顺带看 `tup` 是不是要的那一半：`viewMat` 是
   `translate(translate(dmat4(1),(cx,cy,cz)) * rotateMat, (0,0,-cz))`（renderBase.cc:175），
   离屏导出时 `rotateMat` 应当是单位，所以 `viewMat` 只剩两次平移 ——
   真正把用户坐标转到视图坐标的那一半，很可能是 `t`/`tup` 这一对里的另一个。

**十六个数印出来了**（billboard）：

```
  T0  0.013505  -0.002757   0.006164   1.5
  T1  0.006752   0.005513  -0.012328  -1.499999
  T2  0          0.013783   0.006164   1.500001
  T3  0          0          0          1
```

两件事一次看清：

- **平移在最后一列** —— 行主序，我们这一层 `real[][] * triple`
  （`t[i][0..2]·v + t[i][3]`）的约定本来就是对的，"主序搞错了"这条**排除**；
- 3x3 那一块三行的模长都是 **0.0151**（一个均匀缩放乘一个旋转），平移是 **1.5**。
  这**定量地**解释了前两次失败：乘 `t` 之后单位盒塌成 `1.5 ± 0.0075` 的一小团，
  而且 z 变成**正的** —— 我们按"相机在原点朝 −z"取 `d=-v.z`，于是 d 被夹成 1e-12、
  坐标炸掉，整幅糊满；乘 `inverse(t)` 则放大 66 倍、全部出界。

于是问题收成一个明确的问句：**`m`/`M` 的量级是 ~68，`t*v` 的量级是 ~1.5，
这两套坐标不在同一个尺度上 —— `m`/`M` 到底是谁的坐标？**
下一刀先读 three.asy:2889-2895 那几行（`m`/`M` 是怎么算出来的），把"用户坐标 ->
`m`/`M` 所在那套坐标"这一步补上。`t` 的 1/66 与 1.5 更像是"装进单位盒"的那一步，
不是 modelview。

**读完了，而且它把矛盾指到了我们自己这一侧**（three.asy:2884-2895）：

```
  m = min3(f);  M = max3(f);          // 就是那张帧自己的三维界
  zcenter = P.target.z;  r = P.distance(m,M);
  M = (M.x, M.y, zcenter+r);  m = (m.x, m.y, zcenter-r);
```

也就是说 `m`/`M` **就是帧自己的界**（z 那一格换成 `zcenter±r`）。那么 op 表里的几何
与 `m`/`M` **本该在同一套坐标里** —— 可我们量到的是：`m`/`M` 的量级 ~68，
而 op 表里的点乘上 `t` 才 ~1.5（不乘就是 ~0.5）。两者对不上，说明**不是投影的口径问题，
是我们这一侧的 op 表在搬运时丢了一次变换**：帧的界（`asy__add3`，八个角重算）跟上了，
op 表没跟上，或者跟的不是同一个。

所以下一刀不该再碰投影公式，该去核对**同一张帧上"界"与"op 表"的量级**：
印 `min3(f)`/`max3(f)` 与 `asy__ops3(f)` 头几个点，看差的是哪一次变换
（`label3`/`surface` 那条路、还是 `scale3`/`shift` 那一族有别的入口没搬 op 表）。
判据不变：ink 的包围盒撑到 `0..370 / 8..390`。

**核对过了，一张裸帧上两者是一致的**（`draw(f,unitbox)`，render=-1）：

```
  min3/max3 = (-0.250013,-0.250013,-0.250013) .. (1.250013,1.250013,1.250013)
  ops = 1076；头三格的首点 (0,0,0)、(-0.25,0,0)、(0,0.25,0)
```

也就是说 op 表与界在**同一套坐标**里，`asy__add3` 与 `asy__push3` 没有脱节。
那么 billboard 里 `m`/`M` 变成 ~68 的那 **×136** 是在这之后加上的 ——
而 op 表跟到了 shipout3（gs 确实画了东西），只是**没被同样缩放**。
所以要找的是：**哪一处把帧的三维界放大了 ~136 倍、却没经过
`real[][] * frame`**（那一处我已经补了 op 表）。嫌疑是 `picture` 那一层
按 `min3/max3` 重算界的地方（`fit3`/`add` 那一族）。找到它、让它同样搬 op 表，
量级就对齐了，投影公式本身不用动。

**找到了，而且不只一处**：第 0 步只给四个内建补了 op 表
（`_draw(path3)`、Bezier 面片、三角面片），但**同一族里还有几个仍旧只记界**：

```
  drawTube(frame, triple[] g, width, …, triple min, triple max)   :10173
      —— 粗线的管子！界是**三通过实参给的** min/max，几何一个都没留
  drawpixel(frame, triple v, …)                                   :10182
  draw(frame, triple[] v, int[][] vi, …)（三角网）                 :10187
  drawSphere / drawCylinder / drawDisk                            :10150 前后
      —— 界是拿变换阵作用在单位立方体八个角上估的（asy__addbox3）
```

`drawTube` 这一条最要紧：billboard 的粗线正是走它，而它的界来自 three.asy 传进来的
`min`/`max`（**已经是缩放后的坐标**），而我们能拿到的 op 却是没缩放的那一套 ——
一张帧里于是混了两套坐标，这正好解释了"界 ~68、op ~0.5"的 ×136。

**下一刀**：把这几个也补进 op 表（`drawTube` 存 `g`+`width`、球柱盘存 `t`、
三角网存 `v`/`vi`），并核对它们各自的坐标是哪一套。补完之后量级自然对齐，
投影公式不用动 —— 判据仍是 ink 的包围盒撑到 `0..370 / 8..390`。

**这一条推翻了：没有丢变换。** `drawTube` 补上之后 billboard 的 ink 一点没动
（它没有 kind 3 的 op），于是回头在**真 billboard 那一趟**里印 op 表的坐标：

```
  nops = 1650   op0 = (-41.332512, -33.385116, -181.717530)
                m   = (-68.565461, -56.739044, -298.472692)
```

**同一套坐标、同一量级** —— 前面"op ~0.5"是在**裸帧**（没有 `size()`）上量的，
和 `m`/`M` 不是同一次运行，是我把两个数摆在一起看错了。所以：op 表与 `m`/`M` 一致，
**投影的输入没问题，42% 是公式本身的事**。

拿真数复核一遍就能看出缺口在哪：`near = |Zmax| = 64.96`，op0 在 `|z| = 181.72`，
于是投出来 `x = -41.33 × 64.96/181.72 = -14.78`，而视口半宽是 `rAspect = 39.99`
—— 14.78/39.99 = **37%**，正是量到的那 42%。也就是说这套 frustum
（fov 66.97°、近平面 64.96）**几何上没算错**，只是 asy 的视口比它**紧**得多：
要让 ink 铺满，需要 `r ≈ 43.0 × 0.42 ≈ 18`，也就是 `tan(0.5·fov)·|Z| = 18`
要求 `|Z| ≈ 27.2`，既不是 `|Zmax|=64.96` 也不是 `|Zmin|=298.47`。

所以下一刀要读的是 glrender 在 `initDisplay` 之后**是否又调过一次视口**
（`capzoom`/`updateProjection`/`fitAspect` 那一族，renderBase.cc:170-200），
...
**再往下推一步，能用算术排掉一整族嫌疑**：屏幕上的比例是
`(x·near/|z|) / (tan(0.5·fov)·|Zmax|)`，其中 `near = |Zmax|` —— **near 在分子分母上同时出现，
约掉了**。也就是说 `near`/`Zmax` 取哪一头**根本改不了那 37%**（这是投影不变量），
只有两件事能改：

1. **`fov`**：要让 37% 变成 100%，需要 `tan(0.5·fov) = 0.662/2.38 = 0.278`，
   即 `fov ≈ 31.1°`，而我们收到的是 66.968°。也就是说要么 `angle` 不是该用的那个
   fov（`renderBase.cc:47` 的 `args.angle` 未必等于 shipout3 的 `angle` 形参），
   要么中间还有一次除以 2 之类的换算；
2. **ink 的比法本身**：参考那幅的 ink 铺满，可能是因为两个标签（Billboard/Embedded）
   在参考里画得**大得多**、顶到了边框，而我们的标签画小了 —— 那样"盒子"本身的
   尺度两边其实是一致的，问题只在标签。这一条**还没量**：下一刀应当把
   "只有 `draw(unitbox)`、不加标签"的那一版单独跑一遍对 ink 包围盒，
   一次就能分开"投影缩放"与"标签画法"这两件事。

先做第 2 条 —— 它更便宜，而且能决定第 1 条到底要不要查。

**做了，而且这是目前最干净的一把尺**：去掉标签、只留 `size(100)` + `perspective(1,-2,1)`
+ `draw(unitbox)`，两侧的图**尺寸正好一样**（396x400），于是可以直接比：

```
  参考   ink 9938   bbox 1..393  / 3..395   占宽 99.0%  占高 98.0%
  我们   ink 4390   bbox 51..319 / 86..359  占宽 67.7%  占高 68.3%
```

两件事一次分开了：

- **标签确实占了一部分**：带标签时的缺口是 2.38 倍，去掉标签只剩 **1.46 倍**
  （99.0/67.7）—— 也就是说我们的标签画得比参考小得多，那是另一条账；
- **但仍有一个真实的 ~1.46 倍投影缺口**，与标签无关。

以后就拿这一版当判据（比带标签那版干净：同尺寸、无 TeX、ink 只来自那个盒子）：
**我们的"占宽"要从 67.7% 撑到 99.0%**。下一步查 `fov`（第 1 条）时，
需要的系数是 1.46 而不是 2.38 —— `tan(0.5·fov)` 要从 0.662 降到 0.453，
即 `fov ≈ 48.6°`（收到的是 66.968°）。另一个同样值得先量的：
我们画的盒子是 48 个四边面片 + 1024 个三角面片（管子的侧面与接头），
若接头那一族没画到最外沿，ink 的包围盒也会小 —— 这一条比改公式更容易验：
把三角面片那一支临时关掉，看包围盒是否再缩。

#### 那 1.46 倍找到了：**比（minratio/maxratio）不能拿"界的八个角"算**

上面猜的两条都不是。真正的那一处在 `real[][] * frame`：它把变换后的界
按**旧包围盒的八个角**重算，`minr/maxr`（`x/z` 与 `y/z` 的比）也跟着按盒角算。
界那样算是对的（轴对齐盒子变换后取新盒子），比那样算是错的 —— `x/z` 非线性，
盒角的比与真几何的比不是一回事。asy 那边 picture.cc 的 `ratio` 是**逐个
drawelement 走**的。

在 shipout3 里开一格临时量口（把 op 表的原始界、`x·near/(-z)` 的界、
视口 `xmin/xmax`、`fov`、以及"取到 y 极值的那个点的 z"都印出来），
无标签尺子上量到的就是这个：

```
  angle=48.6747  T=tan(0.5·fov)=0.45230   zoom=1  aspect=0.99
  m=(-44.7644,-48.7333,-247.9485)  M=(44.7643,48.7331,-76.5142)
  视口   x=[-34.2615,34.2615]   y=[-34.6076,34.6076]
  投影后 x=[-25.3087,21.0875]   y=[-27.5345,19.6648]      <- 只占 0.57~0.80
  op 表 raw x=±44.714  y=±48.647   z=[-216.569,-107.894]
  取到 y 极值的那个点在 z=-189.28（**不在近面**）
```

两件事一眼就清楚了：

1. `fov` 是 48.67°，`tan(0.5·fov)=0.4523` —— **正好是上面推出来的"要求值"**。
   所以公式与 fov 都没错，先前"收到的是 66.968°"那一笔是别的例子（billboard）的数，
   记错了账；
2. 盒角 `(44.71, 48.65, -107.89)` 的比是 `48.65/107.89 = 0.451 ≈ T`。也就是说
   `angle()` 是照着这个**并不存在的点**定的 fov：真几何取到 y 极值时 z 已经退到
   -189.28，比只有 0.257。于是 autoadjust 那个循环把"盒角"摆正了、真图形却偏在一边，
   ink 只占 67.7%、而且左右不对称（-0.360 / +0.257）。

订正就是照 asy 走 op 表：变换之后逐点（path3 取结点 + 前后控制点，管子取中心折线，
面片取全部控制点）重算 `minr/maxr`。界仍按盒角 —— 还有几格内建只记界不留图
（drawpixel、三角网格 `draw`、drawSphere/Cylinder/Disk），改成走 op 表会整格丢掉。

**无标签尺子上的效果**（同一份 396×400）：

```
  参考   ink 9938   bbox 1..393 / 3..395   占宽 99.2%  占高 98.3%
  订正前 ink 4390   bbox 51..319 / 86..359 占宽 67.7%  占高 68.3%
  订正后 ink 8197   bbox 0..395 / 2..397   占宽 100.0% 占高 99.0%
```

`pj` 那一格公式一个字没动。`oW/oH` 也没动（99×100）—— 它们来自 `lambda = M-m`，
而这一刀只是把相机的那次 autoadjust 平移改对了，`M-m` 的**长度**没变
（x 89.53、y 97.47 两边都一样），所以那一层逐字节相同的封套仍然相同。

**剩下的差是"线宽"不是"位置"**：ink 8197 对 9938，包围盒还大出 1~2 px。
我们把管子当 1 px 折线描，参考那边是实心的管面（带球帽接头）——
下一刀把 kind==3 那一支从"描线"改成"按投影后的外沿填面"。

带标签那份（billboard，372×400）同时也对上了：

```
  参考   ink 13841  bbox 0..370 / 8..390   占宽 99.7%  占高 95.8%
  订正前 ink  ?      bbox 105..260 / 130..295
  订正后 ink 11544  bbox 0..371 / 7..392   占宽 100.0% 占高 96.5%
```

**注意这里的"逐字节不同"反而从 45859/446400 涨到 67378/446400** —— 这不是退步：
订正前我们那一小团画在中间、参考那一大片的位置上我们一片空白，两边都算差；
订正后我们画到了参考画的地方，但填的是**平色**、参考是 PBR 着色，于是那些
像素**值**不同。所以对 GPU 参考位图，"逐字节不同的个数"不是判据 ——
判据得按之前写下的两层来：**几何（封套、oW/oH、块数、坐标）逐字节相同，
像素按容差**。ink 的包围盒是那个容差层里第一个该看的量。

#### 相机那次 autoadjust 有现成的外部尺子：`asy -v`

这一刀不用猜有没有对齐 —— three.asy:2853 在 `settings.verbose > 0` 时会印出
`adjusting camera to …`。同一份无标签尺子：

```
  真 asy   (1.46748578691304,-1.57210972363774,1.38829476581149)
  我们     (1.46741999391348,-1.57218765289644,1.38820470029364)
```

相对差 ~7e-5。也就是说 `angle()` 那个迭代（包括它里头的 `minratio/maxratio`）
现在收敛到与 asy 同一个位置了。**这条尺子以后查三维视口先用它**：一行命令、
不用解位图。剩下那 7e-5 有个已知来处：asy 那边 `drawBezierPatch::ratio`
在 `straight` 时**只取四个角控制点**（drawsurface.cc:138），我们是十六个都取。

#### 位图的行序是自下而上：`gs` 那一份要倒着画

上面订正完，把两张位图叠起来看（黑=都有、红=只有参考、蓝=只有我们），
两个盒子还是错开的。**不是投影的事，是行序**：位图那一格 asy 是
`glReadPixels` 读回来的，OpenGL 的第 0 行在**下**；`gs -sDEVICE=ppmraw`
是**自上而下**写的。量出来的：

```
  原样     参考的 ink 只有  887/9938 落在我们的 1px 邻域内
  上下翻                  8006/9938
```

修法不是去翻那 95 万个十六进制字符，而是在 `pj` 里把图**倒着画**
（`oH - (y-ymn)/dy*oH`）—— gs 自上而下写出来的就正好是要的那个序。
叠图上**逐像素**重合从 557 涨到 5613（参考 ink 9938、我们 8195），
剩下的是沿着每条棱的 1px 红蓝边 —— 线宽/反锯齿那一层的差，下一刀的活。

#### 那 1px 边拆开了：反锯齿 + 一个 ~1.05px 的 x 偏 + 管子偏粗 1.5 倍

把同一行的灰度剖面打出来（y=200 那一行，左边那条棱）：

```
  参考       20:143  21:48  22:159            重心 20.96  宽 1.63px
  我们（原）  19:0    20:0   21:0              重心 20.50  宽 3.00px
```

三件事：

1. **参考的边是带灰阶的**（GL 那边开着多重采样），gs 默认是硬边。
   给 gs 加 `-dGraphicsAlphaBits=4 -dTextAlphaBits=4` 之后我们也有灰阶了
   （`19:14 20:0 21:100`）。注意这一格**几乎不动 ink 的重合数**
   （5613 -> 5617）—— 它动的是字节差那一层；
2. **有一个 ~1.05px 的 x 偏**，而且**与面片无关**：把面片那一支临时关掉、
   只描 path3 的中心线，重心是 19.79 / 29.45 / 10.21（三行），参考是
   20.96 / 30.5 / 11.21 —— 偏 +1.17 / +1.05 / +1.00。整体上参考 ≈ 我们 ×0.993 + 1.3，
   也就是**参考的视口比我们宽 0.68%**，中心几乎重合。
   量出来的旁证：我们的 ink 正好顶到 0 与 395（视口是"紧贴"的），参考是 1..393。
   来处已经定位到 `drawBezierPatch::ratio`（drawsurface.cc:187）：非直面片那一支
   走的是 `bound(Controls, m, xratio, …, fuzz, maxdepth)` —— 对 Bezier **细分**求
   x/z 的真极值，不是拿控制点的凸包顶。控制点凸包比真曲面**胖**（管子截面的
   控制点在半径 1.13w 上，真曲面只到 w），所以两边的 ratio 差一点；
   直面片那一支还只取四个角控制点（:143）。要对齐就得把 `bound()` 那一族搬过来；
3. **我们的管子偏粗**：宽 2.55px 对参考 1.63px（≈1.5 倍）。面片填的是四条边界三次
   曲线围出来的区域，方向对了，粗细还差一档。

顺带记一条：ink 的个数（9938 对 8195）**主要是反锯齿撑出来的** ——
...
**量了：不是均匀平移。** 给 pj 加 (+1.1px, -1.1px)（x 右移、图下移各一像素多），
叠图重合从 5504 掉到 4067 —— 退回来了。所以先前"重心偏 +1.05px"那个读数
是被抽样的那几行带偏的，不是一个整体位移。

**"管子偏粗"那一笔也要改口**：2.55px 对 1.63px 不是几何粗细的差，是**着色**的差。
管子的直径是 0.5bp，在 expand=4 下就是 2px；我们平色填满那 2px（加反锯齿边到 2.55），
参考那边是 PBR 着的色 —— 圆柱中间朝着相机、亮到 >250，按"非白"数根本不算 ink，
于是量出来只剩 1.63px 的暗芯。**几何是对的**，差的是着色。
所以像素那一层往下走就是"把 PBR 那套片元着色搬过来"，不是调线宽。

#### 下一块该动的是 `bounds`，不是像素

`bound()` 那一族现在只接在**比**上。asy 的 `drawBezierPatch::bounds`（drawsurface.cc:72）
同样是细分：非直面片对 x/y/z 三个分量各跑一遍标量版
`bound(double* P16, m, b, fuzz, maxdepth)`，`fuzz = Fuzz*norm(c,16)`（L∞ 范数）；
直面片只取四个角。我们这一侧 `asy__add3` 是控制点凸包 —— 凸包偏大，
`lambda.x` 就偏大，`ceil` 之后 `oW` 多 1。**这正是 sacylinder3D / cylinder /
...
**`shellsqrtx01` 不在这条链上**（量出来的）：它是
`currentprojection=orthographic(1,0,10,up=Y)` —— 走的是 three.asy:2830 的
`P.infinity` 那一支，`S.width/S.height` 从 `max3(S.f)-min3(S.f)` 来，
**不经过 ratio 那一格**。两侧的数：

```
  参考   封套 258..353 / 320..471   位图 380x604   即 oW=95  oH=151
  我们   封套 257..354 / 334..457   位图 388x492   即 oW=97  oH=123
```

宽多 2、**高少 28**。高少说明我们的三维界在 y 上**漏了东西**（凸包只会偏大，
方向不对）。把两张位图解出来并排看，一眼就找到了：

参考那张下方有 `$y=\sqrt{x}$` 那个标签（连着底下那支红箭头），
**我们那张只有箭头、没有标签** —— 少的正是它那 28pt。其余的都在、位置也对：
旋转面、两根轴、`$x$` / `$y$` / `$(1,1)$` / `$r$` 四个标签、红色的 `r` 箭头
（标签在正交 + `up=Y` 下是镜像的，两侧一致，那不是差）。
...
#### `oW` 那 1pt 找到了：`real[][] * frame` 的界不能拿"旧包围盒八个角"

真实路径上 `embed` 的 `P.infinity` 那一支（three.asy:2830-2841）是先
`S.f = modelview*S.f`、再用 `max3-min3` 当 lambda 去定 `S.width` 与 `oW`。
那次乘法落到我们的 `real[][] * frame`，而它的界一直是"把旧包围盒的八个角过变换"
重算的 —— 旋转之后八角盒是真界的**超集**，`lambda.x` 于是系统性偏大。

改成照 asy 逐个 drawelement 走（面片 `asy__addpatch3`、三角面片 `asy__addtri3`、
path3 与管子走控制点）。**但只有当这一帧的界完全来自 op 表里的几何时才能这么算**：
`drawTube` 的界是实参给的 min/max（管面比中心折线宽）、`drawpixel`/三角网格/NURBS
也只记界，那几种走 op 表会把界**缩小**。所以先拿同一族函数走一遍未变换的 op 表自检，
与帧记下的界一致才换，否则留着八角盒。

量出来的（两侧都重跑）：

```
  七行尺子 ar3.asy   封套 255 380 356 411、位图 400x124 —— **与参考逐字节相同**
  cylinder       8 处数值不同 -> **矢量那半边一样**，ink 404x404 盖住参考 98.3%
  shellsqrtx01   8 处数值不同 -> **矢量那半边一样**，ink 380x604 盖住参考 97.7%
  sacylinder3D   照旧"矢量那半边一样"，盖住参考 99.0%
  透视的无标签尺子 封套与叠图（5478）都没动
  九个原本"一样"的例子仍然"一样"
```

**`viewportmargin` 那笔账也清了**：不用改它。先前以为要把默认改成 `(0.5,0.5)` 才对，
是因为八角盒把 lambda 撑大了 1pt、两个错互相抵消；界改准之后 `(0,0)` 就是对的
（七行尺子逐字节相同）。settings.asy 那一行旁边的说明照实留着 —— 那个默认值
在真 asy 里确实是 `(0.5,0.5)`，但我们这一层还有别处没对齐，改它反而会破，
等哪天真需要时再连着一起改。

到这儿 **cylinder / shellsqrtx01 / sacylinder3D 三个的第一层判据（几何逐字节）都满足了**，
剩下的只有像素（平色 vs PBR 着色）。
...
- `draw(p3,blue)` 那条曲线参考是蓝的、我们是黑的 —— 面片/路径的颜色我们取
  `o.p[0]`，那一格可能不是 material 的 diffuse。
...
**又量了一趟（临时量口，已撤）**：在 `asy__istrans` 里记下真正发出去的笔，跑 cardioid：

- 走到描边那一格的笔一共 **6** 支，其中带变换（`hastrans`）的 **2** 支，
  而这 2 支的六个数**全是精确的单位**（xx=1、xy=yx=0、yy=1、x=y=0）——
  所以我们这边闸门关着；参考那边套了约 55 处。
- 顺带确认一条源码事实：drawelement.h:302 的 `transpen` 是
  `transformed(shiftless(t),pentype)` —— 笔只吃去掉平移的那一半，我们的注释与实现是对的。
  而两侧对量过的轴那一族 `t*T*inverse(t)`，**零头只落在平移那两格**、矩阵那四格两边都是
  精确单位 —— 一 `shiftless` 就没了。也就是说 asy 那边非单位的来源**不在这条式子上**。
- 还有一处对不上量级：cardioid 的 EPS 里路径成百，而这一趟 `emitop` 只被调了 6 次 ——
  带标签（走 TeX）的图，落到 EPS 里的画不是全从这个计数器过的。**下一刀先把这条路摸清**
  （哪一段 PostScript 是哪个写手出的），再谈闸门。
...
**已经改掉了**：`point(t)` 的 de Casteljau 那六句 `one_t*x + t*y` 也照 fma 算，
`min(g).y` 变成 `-1.2990381472196539`（与 asy 逐位相同），两趟 fit 的缩放因子两侧全对上，
**cardioid 从"结构不同"变成"一样"**（12 个例子里一样 7 -> 8、结构不同 4 -> 3）。

**gamma 是同一条链、再往里一层**（同一个五参 `pic.add` 量口）：

- 第一趟 fit 两侧逐位相同（`t.xx=36.86605510585305`、`t.yy=24.958333333333329`）
- **第二趟** asy 是 `35.813002677154202` / `23.962767768312805`（`t*T*inverse(t)` 非单位），
  我们是 `35.813002677154209` / `23.962767768312812`（**精确单位**）—— 闸门又是一边开一边关

与 cardioid 的差别：这一回 x 与 y **两个方向都差 1 ulp**，所以差的不像是某一条曲线的极值，
更像是第二趟喂给 LP 的 coords 里另有一格（刻度/标签那一族的 truesize 分量，或者
`internalbounds` 那一路的 padding）。下一刀从这两个数往回追。

**先排掉一条**：标签的度量不是嫌疑。两侧同一份源码量 `min/max(frame)`（一个只放标签的帧）：
`$1$` 是 `-4.422012453300125 / -5.3477310087173109 / 4.422012453300125 / 5.3477310087173091`、
...
### 十六、gamma 那个例子的根因：`gamma()` 是我们自己实现的，参考用 libm

`fma` 那一刀落地之后（`asy__pabs` 已按 `fma(y,y,x*x)` 算，五腿无回退），gamma 仍然结构不同。
顺着嫌疑清单一路量下来，**全部两侧逐位相同**：标签度量、`min/max(pen)`、
`min/max(path)`（界与求根那一族已按 fmadd 铺满）、`abs(triple)`（四个探针）、
`Sin/Cos/dir/unit/radians/degrees`。

最后量到 `gamma()` 本身 —— **六个探针全不同**（都在最后 1~3 位）：

- `gamma(0.5)`：asy `1.7724538509055161`，我们 `1.7724538509055159`
- `gamma(1.5)`：`0.88622692545275805` / `0.88622692545275861`
- `gamma(2.5)`：`1.329340388179137` / `1.3293403881791384`
- `gamma(4.5)`：`11.631728396567448` / `11.631728396567446`
- `gamma(-0.5)`：`-3.5449077018110318` / `-3.5449077018110295`
- `gamma(5.7)`：`72.527634520222946` / `72.527634520223046`

原因在明处：`frontend-asy/builtins.tab` 那一行是 `gamma 1 real nope -`（**不转手宿主**），
实现是 asy_builtins.asy:9773 的 `asy__lgam` + 反射公式；参考那边是 libm 的 `tgamma`。
gamma.asy 画的就是 Gamma 函数曲线，每个采样点的 y 值差最后一位 -> 界差 -> LP 的缩放
因子差 -> `t*T*inverse(t)` 变成精确单位 -> gsave 闸门关掉。整条链闭合。

**这一格不是 bug，是一档边界**：EPS 那一轴跑的是 JS 腿，而 `Math.*` 里没有 gamma，
所以"接宿主"只能修 C/LLVM 腿、修不了这一轴。要让 JS 腿也逐位对上，得在 JS 里复刻
macOS libm 的 `tgamma` 同一份算法（Apple/FreeBSD msun 的 `lgamma_r` + exp 那一套）——
可做但工作量与收益不成比例，而且**不可移植**（换台机器换 libm 就又不一样）。
所以 gamma.asy 在这一轴上标为"依赖超越函数最后一位、只保证容差"的一档，
与 cardioid 那种"我们算错了"分开记账。

剩下真正该继续的两个结构不同：

- **lmfit1**（方向相反：我们套 gsave、参考不套）—— 量到根了：`exp` 这一档。
  两侧同一份源码印 17 位：`exp(1.7)` 参考 `5.4739473917271999`、我们 `5.473947391727199`
  （差 1 ulp；`log`、`^` 与另外三个 `exp` 探针相同）。macOS libm 的 `exp` 与 V8 的
  `Math.exp` **只保证容差**，这一位顺着数据点 -> 界 -> LP 的缩放因子放大成闸门开关；
  差的方向取决于那一位往上还是往下，所以有时"参考套我们不套"（cardioid 那种、已修），
  有时反过来（lmfit1）。
- **laserlattice**（参考套、我们不套）—— 与 cardioid 同向，但它那张 `image/palette`
  的数据也过 `exp`/幂运算，归同一档。

**结论：这三个（gamma / lmfit1 / laserlattice）都不是"我们算错"，是超越函数最后一位
这一档的下游放大。** 要彻底消掉只能让 JS 腿的 `exp`/`log`/`gamma` 与这台机器的 libm
逐位一致（复刻 msun），不可移植且不划算。也不能反过来"把闸门放宽"——asy 的判据就是
精确比六个数，放宽会让我们在"套不套 gsave"上系统性地与参考不同，更糟。
所以这三个按此归档，EPS 那一轴的**可修项到此清空**；下一个有实质空间的是位图那一档
（AiryDisk ink 96.4%、label3 89%、billboard 78% 的余量）。

### 十七、位图那一档的下一个落点：三维标签的贴片（billboard / label3）

billboard 的 ASCII 对照（56 列）看下来：**画的东西都在**（同一个立方体框、同一批文字块），
两幅图的差别有两层 ——

- **深浅**：参考那边笔画更"实"（`#`/`+`/`=` 多），我们更"淡"（`*`/`-`/`.` 多）。
  这一层是两种光栅化器的差别：glrender.cc 里**没有 `glLineWidth`**（线宽默认 1.0，
  而 GL 的 1 像素就是输出位图的 1 像素 = 0.25pt，与我们 `setlinewidth 0.25` 同口径），
  但它常开 `GL_MULTISAMPLE`（:819/1119），覆盖是按采样点量化的，而 gs 那边是解析覆盖
  加 `-dGraphicsAlphaBits=4`（16 档）。这一层与超越函数那一档同性质，不追。
- **位置**：ink 覆盖率 78% 算的是"非白像素"，淡也算 ink —— 所以 22% 是参考有 ink、
  我们纯白的**位置**差。逐行看，文字块的列位与参考不同（字符分布相似、位置错开）。

于是位图那一档的下一刀落在**三维标签的贴片矩阵**上（`three_label.asy` 的 billboard 那一段：
标签要按视角转成贴片再投影），label3（89%）是同一族。这一族我们从前碰过一次
（"billboard 的 fov 66.968°"那笔错账就在这儿），要照 three_label.asy 逐句核
贴片的基向量与中心，而不是再去调深浅。

**上面那一段的落点是错的，已核实推翻**（又是一次不核源码的外推）：

- `Billboard` 只在 **prc 那条路**上生效 —— three_surface.asy:2007 与 :2088 都写着
  `if(prc() && interaction.type == Billboard.type)`。EPS 这一轴不出 prc，所以那一支
  **根本不走**；`three_label.asy` 这个文件在这一版里也不存在。
- 顺着标签这一族又量了两把尺子，**两侧都逐位相同**：
  - `texpath(Label("$x$"))`：1 条路径、30 段、四个界数 `-2.9039058 / -2.6937470399999999 /
    2.9039058 / 2.6937470400000003` 全同 —— 字形轮廓这一格本来就对
  - 三维标签的界（`label(pic,"$x$",(1,1,1))` 的 `min3/max3` 六个数）全同 ——
    摆放（`L.T3` + `alignshift` + `labelmargin`）也对

所以 billboard 那 22% 既不是字形、也不是摆放、也不是 Billboard 支路，只剩**光栅化那一步
的半像素约定**：同一块曲面，GL 的多重采样格与 gs 的解析覆盖在"1 像素粗的笔画"上落点
可能差一列。这与从前那把透视尺子量到的 `ink x[4..395]` 对参考 `x[3..395]`（最小边差 1px、
最大边对齐）是同一件事。

**下一刀的尺子**：一条位置已知的细线/方框（正交、正对相机、坐标恰好落在像素中心与像素
边界），两侧比 ink 落在第几列，一次定出差不差半个像素、该往哪边补。这把尺子这一轮没跑成 ——
参考 asy 在临时目录里报 "using possibly incompatible version of plain.asy"，
说明跑参考那一侧要**明确清掉 `ASYMPTOTE_DIR`** 并换一个干净目录；下一轮先修尺子再量。

### 十八、AiryDisk 少的不是量化残差，是**整条轴线**——画家算法的结构性缺陷

上面把 AiryDisk 的 3.6% 归因成"外围极淡区的 8 位量化"，是看 ASCII 图猜的，**一量就不成立**。
新尺子（/tmp/omni-shade/cmp7.mjs：把"只有参考有 ink"的像素按离白距离分档，再打掩码图）：

- 8770 个像素里 **0 个是极淡的**：离白 16~50 的 4623 个、离白 **>50** 的 4147 个
- 掩码图看：不是散点，是**几条明确的线** —— 左/中/右三条竖线加几段阶梯状横线，
  全在图的上半部

那就是**三维盒子的轴线与刻度**。我们整条线没画出来，而参考有。

根因是画家算法的结构性缺陷，不是参数没调好：我们给每个 op 一个**整体**深度键
（线是节点 z 的平均 + 朝相机的偏置），而 GL 那边是**逐像素** Z-buffer。一条长轴线
可能一半在曲面前、一半在后 —— Z-buffer 会把露出来的那半照常画出来，而我们把整条线
判成"在后"就整条被曲面盖掉。这正好解释"参考有、我们没有"且集中在曲面上方。

**照 Z-buffer 语义的修法**：把长线**按段切开**参与排序 —— 每段一个深度键，段越短越接近
逐像素。这与面片那边"按屏幕尺寸细分"是同一套思路，开销也同阶（只切长线，短线不动）。
落点是 `asy__r3hexfn` 里排序键的构建（kind 0 / kind 3 要能带段号）与发射处（按段号只画
那一段）。判据现成：那 8770 应当大幅减少，而已经"一样"的九个 3D 例子不能回退。

## 已落地：三维那一档换路 —— 光栅化搬进 C 运行时（gs 那条路量到底了）

**先说为什么换。** 用 gs 自己当尺子，同一组坐标（内容边在设备 x=1.967 处）：

```
fill   出来的边：255 227 153 153 …   （有反锯齿）
shfill 出来的边：255   1   1   1 …   （没有）
```

`/ShadingType 7` 不走 gs 的反锯齿那条路，于是每一片面片都**外溢整整一个像素**，
而参考那边是覆盖率。vD 那把尺子上剩下的 674 个差像素里 671 个贴着边、只有 3 个孤立
—— 几何早就量到底了，剩下的全是光栅口径。gs 上没有出路。

**新路**：`runtime/omni_r3.c`（准确性关键层，C 与 LLVM 两条腿共用），逐句照抄
`renderBase.cc:111 setDimensions` / `:205 ortho`、`tile.h:136 beginTile`、
`bezierpatch.h:45 normal` / `:89 Distance`、`triple.h:398 Straightness` / `:406 Flatness`、
`bezierpatch.cc:67` 的细分判据、两份 glsl 的 PBR（**全程 float**，GL 的片元着色是 float）。
asy 侧 `asy__r3hexfn` 只负责把 op 表写成一份场景清单（**原始的十六个控制点**，
细分交给 C），调新口子 `(r3render PATH)`；回空串才走 gs 那条旧路（JS 宿主那三条腿）。

**顺手挖出来的两个真 bug：**

1. **C 后端的名字空间撞车**：类型名 `c_<名字>`、变量名也是 `c_<名字>`，而 asy 里
   「变量与类型同名」是常态（`light light=currentlight`、`material material`）。形参表里
   前一个形参叫 `c_light` 之后，后一个 `c_light c_meshlight` 被 C 解析成
   "unknown type name" —— **run-c 上三维那一族一份都编不过**。类名改成 `ct_<名字>`。
2. **res2 在解析完才算，而细分是解析时就跑的**：每片面片都"不平"，递归到深度上限，
   17 片 × 4^12 ≈ 2.8 亿次。改成读到 `res` 那行就算。同一份清单 8s 跑不完 -> **0.01s**。

**耗时的账**：生成的 C 是 3.5MB，`cc -fsyntax-only` 0.62s、`cc -O0 -c` 1.13s，
全程 run-c 2.2s —— **编译不是瓶颈**，卡住的一直是光栅化里的那个递归。

**采样数是量出来的**：参考位图在斜边上的一行剖面 239 207 175 159 127（绿 1,183,1、
背景 255）换成覆盖率是 1/16、3/16、5/16、6/16、8/16 —— **十六分之一一档**，
所以是 16 个采样点，不是 4（4 采样只出 0、1/4、1/2、3/4、1）。

**窗口变换那半个像素**同样是量出来的：满幅方块在 size(40)/60/61/100 四档上，参考的
内容边一律落在 [1.5, W-2.5]，而 glViewport 的教科书公式给 [1.967, W-1.97] —— 宽度
对得上（236.07 vs 236），差一个**与尺寸无关的恒定 -0.5 像素**。补上之后满幅那把
尺子只差 8 个字节。

**现状（run-c 对真 asy，位图逐字节 / ink 重合）**：

- 满幅方块 flat：字节差 **8/172800**（最大差 1）、ink 重合 100.00%
- 斜方块 vS：97.53%（只有参考 260、只有我们 **0**）
- 轴+方块 vN：97.49%（同一族）

**剩下那 2.5% 的口径已经量清**：ink 质量 参考 10052.5、我们 10051.9，差
**0.5/10052（0.005%）** —— 形状大小与位置都对，差的只是覆盖率落进哪个十六分之一桶。
标定尺（`.omni-cache/rulers/cal.mjs`：一条竖直边按 1/16 像素扫过一个像素）显示参考的
覆盖率只取**偶数**档、每约 0.23 像素跳一档，也就是那 16 个采样点在 x 上只有 8 个互不
相同的投影、每处 2 个 —— 与 4x4 规则网格（x 上 4 档、每档 4 个）不是一套。要关掉这
2.5% 就得把点位标定出来。（尺子里两处坑：`surface(white)` 出来是 183 的灰，要
`emissive(white)` 才是真白；画布得靠一片满幅图元定住。）

**下一刀**（按价值排）：`BezierTriangle::render`（bezierpatch.cc:561，管子的接头，
现在只发三个角）与"只有一个方向平"的两个分支（:227/:319）照抄进来；然后是透明
（source-over 与深度的关系）与 kind 3 的管子；再把 EPS 那一轴整体挪到 run-c 上量。

## 已落地：C 那份光栅化器补齐三角面片、透明、曲线细分 —— 顺带三个真 bug

**补齐的三样**（都是照抄，不是自己设计）：

- **三角面片**（管子的球帽与圆盘，粗线一定走到）：bezierpatch.h:195 的 Distance、
  bezierpatch.cc:767-769 的三个角法向、:706-765 的一刀四分（left/right/up/center）。
  清单里新增 `btri <straight> <十个控制点>`，编号照 :652 那张图。
- **透明**：fragment.glsl 的 TRANSPARENT 分支是把片元写进逐像素的链表再合成，
  不透明与透明是两个缓冲。照抄：透明三角**不写深度**、光栅化分两趟、
  合成时每个采样点各自把自己那条链按深度降序 source-over。
- **曲线细分**：beziercurve.cc:62 的 `Straightness < res2` 递归搬进 C，
  清单里一段只发四个控制点（`bez`）。从前是 asy 侧按 t 均匀取 16 段 —— 判据不对、
  清单还大 16 倍。

**三个真 bug（都是量出来的，不是看出来的）**：

1. **清单别一路 `s = s + …`**：omni_str 不可变，C 那条腿上每次重新分配再整份拷贝，
   面片一多就是 O(n^2)。cylinder 在 run-c 上 100s 都写不出那份几百 KB 的清单，
   而 front-end + cc 只要 2.2s。改成攒成 `string[]` 再两两归并（O(n log n)）。
   JS 那条腿看不出来 —— V8 的 `+=` 是 rope。
2. **材质表 realloc 让图元的指针全变野指针**：图元存的是 `const r3mat *`，
   而材质数组边解析边 realloc。cylinder 有 5053 条 mat，数组搬了十几次 ——
   8896 片三角里 **2942 片读到垃圾 alpha 被当成透明**，透明片元攒了 186 万条，
   合成完整个圆柱几乎不见（ink 4.46%）。改成解析前先数 mat 的条数、一次分配好。
3. （上一节那两个：C 后端类名撞 `c_` 前缀、res2 算晚了。）

新量口 `OMNI_R3_DEBUG=1`：印三角/线段/透明片元的条数 —— 上面第 2 条就是这三个数指出来的。

**现在的账（run-c 对真 asy，位图逐字节 / ink 重合 / ink 质量）**：

- 满幅方块 flat：字节差 8/172800（最大差 1）、100.00%
- 绿方块 + 半透明红方块 tr：**100.00%**、质量差 0.0、七个采样点取值逐个相同
- 球 unitsphere：99.43%、质量差 0.26%、内部着色与参考差 0~1
- **cylinder（solids 的旋转面 + 骨架线 + 透视）：99.44%**、质量差 0.11%、5.8s
- 斜方块 / 轴+方块：97.5%（"只有我们有 ink" 恒为 0）
...
这条不是三维那一档的账，是 C/LLVM 两条腿在**长跑程序**上的账。

**量到的数**（`/usr/bin/time -l`，BezierPatch 的那份 a.out）：
`7.59 real`、**峰值内存足迹 43 GB**、最大 RSS 1.17 GB —— 也就是 7.6 秒里申请了 43GB。
顺着查了两处，排除一个、锁定一个：

- **`triple`/`pair` 不是堆对象**（frontend-asy/runtime.js:18-20：它们是 `vec`，值类型），
  所以三维几何里那些向量运算**不分配** —— 这条排除了。
- 数组的增长是**翻倍**（omni_arr.c:23/93），摊还是 2n，也不是平方 —— 这条也排除了。
- 剩下的是**记录（asy 的 struct）**：`path3` / `patch` / `surface` / `pen` 这一族每构造
  一次就在 arena 上分配一次，而 arena 永不回收。tube 那一族（three_tube.asy 的 Split
  递归 + 每片面片的控制点数组）一趟就是几百万次构造。

下一刀的选择（我倾向第 2 个，改动小且对得上 asy 的生命周期）：
1. 给记录/容器这一族加引用计数或标记清扫（最彻底，但那是 ADR-0001 里被排在 arena
   之后的那件事，要单独开一节）；
2. **作用域 arena**：给"一趟 shipout / 一次 `asy__r3hexfn`"划一块，出了作用域整块丢掉
   —— 前提是这一段里构造的记录不会被外面继续引用（要先量清）；
3. 先把 tube 那一族的临时构造减下来（治标，但能立刻让那 8 份出图）。

量口是现成的：`/usr/bin/time -l` 看峰值足迹；**新加的**是运行时自己的量口
`OMNI_MEM_DEBUG=1`（退出时印 arena 一共开了多少块/多少字节）与 `=2`
（每翻一倍印一行 —— 被 SIGKILL 打死时 atexit 不跑，只有这条看得见涨到哪一档）。

**用这两个量口把"是 tube 的账"这条猜测否掉了**（原先写在这一节里，订正如下）：

- 一条**直**粗线 `draw((0,0,0)--(1,1,1),2bp+blue)`：arena **11 MiB**
- 一条**细**线（同一条路，不走 tube）：arena **11 MiB** —— 一模一样，
  也就是说 tube 那一族**不是**大头
- 一条**曲**粗线 `draw((0,0,0)..(1,1,0)..(2,0,1),2bp+blue)`：arena **22 MiB**
- BezierPatch：arena 一路 4 GiB -> 8 -> 16 -> **32 GiB**（19643 块）然后被 SIGKILL

（前面那个"432MB"是 `/usr/bin/time -l` 量了整条 `node cli.js run-c`，里头大半是
node 自己；换成运行时的量口之后这个数就对上了 —— 11 MiB。）

所以嫌疑落在 BezierPatch 那份**退化面片**上：它的控制点里有重复点
（`{(0,0,0),(1,0,0),(1,0,0),(2,0,0)}`、`{(0,1,0),(1,0,1),(1,0,1),(2,1,0)}`），
而"够平了吗"这类判据在退化面片上很容易不收敛。

**把那份例子拆开跑，落点确定了**：只留 `draw(s,yellow)` 一句（bpA）也照样涨到
32 GiB 被杀 —— 也就是说**问题在"画一片退化面片"这条路上**，与两条粗线无关
（只留 `vequals(0.5)` 那句的 bpB 同样涨，因为它也要先建那份 surface）。

下一刀（下一节的第一件事）：把 `asy__sboundpatch`（面片的界，照 bound.cc 转写的那份）
与 `bound.cc` 的 `boundpatch` 逐句对一遍。递归上限 `asy__rmaxdepth = 53`
（bound.cc:15 的 DBL_MANT_DIG）本身与原版一致，所以差的是**判据没收敛**：
原版靠 `Fuzz*norm(...)` 在几层内就"够平"，而退化面片上那个 norm 可能是 0，
...
## 更正（2026-09-07）：上面那条"下一刀的两条路"已经过期

第 1 条（**给 arena 加 mark/release，在这一处用**）**已经做了**：`asy_builtins.asy` 的
`asy__addpatch3` / `asy__addtri3` 里那六次 `asy__sbound` / `asy__sboundtri` 已经被
`int mk = _arenamark(); … _arenarelease(mk);` 圈住（JS 那三条腿上 `_arenamark` 回 -1、
`_arenarelease` 是空操作）。所以别再按那条去改。

**三维那一档现在的时间账**（量口：`OMNI_R3_DEBUG=1` 现在多印 `进来之前 %.2fs`，
也就是 `clock()` 在 `omni_r3_render` 入口的值；`OMNI_R3_NOFMT=1` 让 `sn()` 直接回 `"0"`，
只用来算账、图是废的）：

pdb（119576 个三角、清单 80MB、804x800；真 asy `-f eps` 是 **5.6s**）
```
带格式化   进来之前 16.10s / 35.18s   解析 0.97s 光栅 0.28s   总 41.8s   清单 80MB
不格式化   进来之前 12.94s / 28.26s   解析 0.40s 光栅 0.01s   总 34.7s   清单 11.8MB
```
- **C 光栅器只占 1.25s × 2 = 2.5s（6%），不是瓶颈。**
- **格式化 double 占 3.2s/趟（全程 15%）**，却是清单 **85% 的字节**。
  所以"清单换二进制"最多值这 15%（读侧 0.97→0.40s 也在里面）。
- **大头是每趟 12.9s 的 asy 层前置**：three.asy 的 `fit3`/`deconstruct` 把 119576 个面片
  按投影变换成新帧，加上逐 op 拼行、`pp` 的百万次 push 与两两归并。与 C++ 差 8 倍
  属于"整条腿的常数"，要靠编译器侧热路径优化，不是三维这一处能修的。
- **两趟 shipout 不是 bug**：`plain_shipout.asy:103` 的 3D 分支在 `implicitshipout`
  门闩**之前**就调 `embed3`（真正渲染那一步），所以显式 `shipout(options=…)` 与退出时
  那一次各渲染一遍 —— 真 asy 也是两遍。最终份数两边都是一份（asy 覆盖文件，
  判据里我们也写文件）；只有直接看 stdout 时我们出两份，而 asy 的 stdout 从来不带图。

## 清单换了编码（2026-09-07）：数发位模式、`mat` 行不重复

上面那条"清单换二进制最多值 15%"有了落点，两刀都做了，**五个例子逐字节不变**
（sacylinder3D 18813 / splitpatch 139063 / twistedtubes 4231 / trefoilknot 58334 /
hyperboloid 6747）：

1. **数发 double 的位模式**（十六进制、`x` 前缀），不再是 `string(v, 17)`。方言里
   `realbits` 早就有（五条腿各一份，ADR-0019 路 1），所以只加了 asy 侧一格
   `_rhex(real)`（= `(sbase (realbits E) (int 16))`，frontend-asy/calls.js）与运行时
   `r3_num` 里的 `x` 分支（十进制照旧收，手写清单的单测一个字都不用改）。
   **整数值仍走十进制**：只发位模式时 `1` 要十七个字节，清单 80MB→84.7MB、
   解析 0.97→1.05s，两头都变差。`-0` 单独一条 —— 写成 `"0"` 会把符号丢掉。
2. **`mat` 行不重复发**：一次 `draw(surface, …)` 里所有面片共享同一个 material，
   而运行时那侧图元指的本来就是"最近一条 mat"（`mats + (nmat - 1)`）。判据取的是
   **上一条 mat 的输入**（三支笔按值 + opacity/shininess/metallic/fresnel0/lightOn），
   所以连 `penrgb` 与那十四次格式化都省了。踩过的坑：`pen[] == pen[]` 在 asy 里是
   **逐元素**的（回 `bool[]`），身份比较要用 `alias`；这儿用的是逐支笔的 `pen == pen`。

pdb 那把尺子（804x800、119576 个三角；真 asy `-f eps` 5.6s）：
```
之前   进来之前 8.70s / 20.52s   解析 0.97s   清单 84.7MB
之后   进来之前 7.78s / 18.25s   解析 0.85s   清单 65.2MB
```

**`cycis` 那 3.4s：查清了，也治好了 —— 但不是靠"换扫描顺序"。**
`asy__cycis_arr_pen` 93.6 万次调用、自用 3.44s，调用者**不是** `cycidx`（越界下标那一路
只 46.4 万次、含子 112ms），而是 `acopy_pen`：`material.operator init(material m)` 里那句
`p = copy(m.p)`。每个 material 都新建一份三格 `pen[]`，查的那个数组从来不在册里，
一格 memo 永不命中，每次都扫满整册（册长约七千，两趟 shipout 翻倍，正好对上第二趟慢一倍）。
- 先试的"命中就换到册尾"（自组织）：pdb 8.70→9.21s，**更慢** —— 否定回答根本走不到
  "命中"那一支。已回退，理由留在 `frontend-asy/lower.js` 的 `cycHelper` 里。
- 真正的一刀：给方言加 **`(refid E)`**（引用的身份整数），把登记册换成 **256 个散列桶**。
  五条腿各一份落点，与 `realbits` 同一条路子：C 是 omni.h 里的宏（一次强转、不付调用）、
  run-llvm 是真符号 `omni_refid`、JS 是 WeakMap 发号、解释器是 Map 发号（WeakMap 不在
  语言子集里）。桶号是 `(refid >> 4) & 255` —— **必须先右移**，arena 的块按 8/16 字节
  对齐，低几位是常数，不移就只用得上其中几个桶。用例：`tests/sexpr/cases/38-refid.sx`
  （只印"比出来的关系"，因为各条腿的号不是同一批）。

pdb 三刀累计（804x800、119576 个三角；真 asy `-f eps` 5.6s）：
```
本轮之前   进来之前 8.70s / 20.52s   解析 0.97s   清单 84.7MB
位模式+mat 进来之前 7.78s / 18.25s   解析 0.85s   清单 65.2MB
分桶之后   进来之前 6.91s / 15.04s   解析 0.87s   清单 65.2MB
矩阵摊平   进来之前 6.12s / 13.67s   解析 0.86s   清单 65.2MB
六界融合   进来之前 5.96s / 13.48s   解析 0.84s   清单 65.2MB
缓冲复用   进来之前 5.85s / 13.17s   解析 0.83s   清单 65.2MB
界并进一趟 进来之前 5.87s / 12.86s   解析 0.82s   清单 65.2MB
```

## 生成代码的优化档：量出来了（2026-09-07）

`OMNI_OPT` 这个开关早就在（cli.js 的 `optFlag()`），默认 `-O0`。**三维那一档上档位值 4 倍，
而且是逐字节中性的**：

pdb（119576 个三角）每趟 shipout 的 asy 层前置 / 解析 / 光栅：
```
-O0   5.87s / 12.86s   0.82s   0.28s
-O1   1.47s /  3.59s   0.38s   0.08s     ← 约 4 倍
-O2   1.07s /  2.90s   0.36s   0.07s
```
`-O1` 那一趟把八个例子（sacylinder3D 18813 / splitpatch 139063 / twistedtubes 4231 /
trefoilknot 58334 / hyperboloid 6747 / filesurface 17550 / triangles 10571 /
twoSpheres 435542）全跑了一遍，**位图不同字节数与 -O0 一个不差** —— `-ffp-contract=off`
一直都在，档位不动浮点结果。

代价是编译变慢（判据上一个例子 3.1s→8.7s，全是 clang 的时间）。所以"默认哪一档"其实是
按**例子的规模**分的两件事：
- 小用例（tests/asy/cases、tests/sexpr 那几百行的翻译单元）：编译主导，`-O0` 是对的。
- 三维那一档（一个例子几万到十几万个面片）：跑的时间主导，`-O1` 直接把 pdb 从约 20s 压到
  **约 6s**，与真 asy 的 5.6s 基本平了。

**这一格是 ADR 级的取向，没有替你定**：要么按例子规模自动选档（比如 op 表超过某个阈值就
`-O1`），要么让 EPS 那一轴的 run-c 腿固定 `-O1`（判据比的是字节，档位既然中性就没有风险），
要么什么都不改、只在量性能时手动开。

**算一下交叉点，我的建议是"什么都不改"。** 判据上量到的两头：
- 编译多付的：triangles 3.1s→8.7s、twoSpheres 5.5s→9.3s，也就是**每个例子多付 4~6s**
  （全是 clang 的时间，与例子的图形规模基本无关）。
- 跑省下来的：三维那一段省约 75%（pdb 每趟 shipout 前置 5.87s→1.47s）。

所以交叉点在"**跑的部分大约 6~8s**"：右边（pdb / filesurface / AiryDisk / bars3 那一族，
189 个里十来个）开 `-O1` 净赚，左边（绝大多数）反而变慢。于是：
- **默认维持 `-O0`**：它对绝大多数用例是对的，而且这条腿在测试轴上的角色是"另一份语义
  实现"，档位越低越少一层"是不是优化器改了行为"的怀疑。
- **量三维性能时手动 `OMNI_OPT=1`**（这一条已经写进 optFlag() 的注释与 memory）。
- 真要自动化再想：判据缓存（`.omni-cache/epsres/*.json`）里已经有每个例子上一趟的耗时，
  按"上次超过 8s 就开 `-O1`"是可行的，但那会让同一轴上出现**混合档位**，
  排查"为什么这个例子的字节数变了"时得先分清是档位还是改动 —— 这层代价得先认下来。

## 第八刀（已做）：清单里的**数走内存**，文本只留关键字与结构

**结论先说：这一刀不需要给方言加新 op。** 之前记的"要给方言加一格把大数组交给运行时"
已经过时了 —— blob 结构体在 03ec970 里就摊进 `omni.h` 了，C 那条腿上数组就是那个结构体的
指针，所以只要让 `_r3render` **多收一个 `real[]`**（`(r3render PATH NUMS)`），运行时那侧
直接读 `->items` 就行；口子与原来那条 `(r3render <字符串>)` 同一形状（frontend-asy/calls.js
里那十行）。

原来那条路的账（pdb，`-O0`，每趟 shipout）：
- asy 侧把清单拼出来：`anon124` 自用约 4s（含插桩，实测值更小）、`sn` 716 万次调用；
- 写 65.2MB 到 `/tmp`，再由 `r3_num`/`r3_word` 逐词读回来：解析 0.82s。

**落地的样子**：清单头一行改成 `r3 2`，`sn(real)` 不再格式化、而是把数按出现次序推进
一个 `real[]`（`sn` 回空串，于是所有调用点写法不变），随清单路径一起交给 `_r3render`；
运行时 `r3lex` 多三格（`nums`/`nn`/`ni`），`r3_num` 在 `ver >= 2` 时按游标取。
`OMNI_R3_TEXT=1` 退回 `r3 1`（数写在文本里）—— **手写的清单单测与"直接看 r3.scn"那条
调试路都靠它**，位模式（`_rhex`、`x` 前缀）与 `r3_num` 里的整词校验因此都留着。

量出来的（pdb，`-O0`，两趟 shipout）：
- 进来之前 **5.87s → 5.21s** / **12.86s → 10.94s**；解析 **0.82s → 0.14s**；
  也就是每趟省 **约 1.3s**（估的是 1.5~2s，实际略少）。
- 清单 **65.2MB → 0.59MB**（少了 110 倍：文本里只剩关键字）。
- 八个三维例子**逐字节不变**：18813 / 139063 / 4231 / 58334 / 6747 / 17550 / 10571 / 435542
  （run-c 与 run-llvm 两条腿都验过，JS 那条腿照旧走 gs 的老路）。

踩到的两个坑（都记明处）：
1. **`matcap` 的预扫描认的是 `"mat "`**（关键字后面必须是空白）—— v2 里那一行是光秃秃的
   `"mat\n"`，于是一条也数不着，材质表被写爆（图元里存的是 `const r3mat *`）。
   现象：sacylinder3D 18813 → **233985**、透明误判让 OIT 那一趟白跑（4s → 62s）。
   `pcol` 的预扫描同一个毛病（顶点色整族关掉）。改法是 `r3_kwend`（空白**或换行**）。
2. **`matline` 的二次去重（`s == lastmat[0]`）在 v2 里必须关掉**：`mline` 已经把十四个数
   推进 `nums` 了，再丢掉那一行就是文本与数组错位；而 v2 里 `s` 恒等于 `"mat\n"`，
   这一比一定成立 —— 不挡住就等于把第一条之后的 mat 全丢了。
   同类的次序坑还有一处：`tri` 那一支从前先算 `tnrm` 再算 `mat`/`pcol`，文本次序没错、
   **推数的次序错了九个**，所以现在先把 `mat`+`pcol` 算进局部再算 `tnrm`。

一条 C 的规矩顺手记下：`omni.h` 里 `omni_r3_render` 的声明在 blob 的 typedef **之前**，
参数只能写 `struct omni_arr_f64_s *`，而且**前面要单独一句 `struct omni_arr_f64_s;`** ——
在函数原型里头一次提到某个 struct 标签，那个标签只活在原型作用域里，与后面 file scope
的那一个是两个类型（报 "conflicting types"）。

JS 那三条腿不受影响（`r3render` 在那边回空串、走 gs 那条老路）。

## 第十一刀（已做）：透明改成**逐像素**的链 + GL 的填充规则

照上一节读出来的结构写了 `r3_raster_pix`（`OMNI_R3_OITPIX`，**默认开**，`=0` 退回
逐采样点那一路）：覆盖只按像素中心判、着色也在中心算一次，三趟 ——
收不透明那一格的色与深度（`pcol`/`pdep`，对应 `opaqueColor[pixel]`/`opaqueDepth[pixel]`）、
数透明片元、填片元；最后按深度降序混，底色取那一格的不透明色或背景，
`>= opaqueDepth` 的片元跳掉，混出来的**一个值**写进这一格的全部采样点
（参考那边 blend.glsl 也是一格一个值；没有透明片元的格子它 `discard`，
所以我们也只碰 `cnt > 0` 的格子 —— 那些格子照旧是不透明那一趟多重采样的结果）。

**顺带补上 GL 的填充规则（top-left）**：判据正好落在 0 上时，只有"左/上边"那一侧算覆盖。
两个相邻三角共享的那条边在各自里方向相反，于是这样的采样点只被**一个**三角认领。
原来三条都写 `>= 0`，正方形那条对角线正好穿过像素中心 —— 那些格子混了**两层**
（探针 `flat_trans`：参考 242、我们 240）。方向也量过：`dy > 0`（水平边时 `dx < 0`）
这一档比反过来那档好，1740 对 3171。

量出来的账（八个三维例子 + 两个）：
- sacylinder3D **18813 → 6449**、triangles **10574 → 183**、
  twoSpheres **435546 → 296130**、vectorfieldsphere **162572 → 123019**。
- 没有透明的那几个**一个字节不差**：splitpatch 139061、twistedtubes 4233、
  trefoilknot 58332、hyperboloid 6749、filesurface 17548。
- 探针：`edge_trans` **8754 → 0**、`flat_trans` **也是 0**（第三条判据要用**真正的**
  第三条边函数，不能用 `1-a0-a1` —— 后者的"恰好为 0"与"落在 v0->v1 那条边上"差一次舍入，
  而 tie-break 只在恰好为 0 时起作用；这一处对齐之后那条对角线的 1740 全没了）、
  `sph_trans1` 40932 → 27804、`sph_trans` 56154 → 31674。
- 于是**平面片与掠射面片那一族（五个探针）现在全逐字节相同**，球那一族还剩
...
**第十三刀：顶点先吸到子像素网格**（`OMNI_R3_SNAP`，默认 **256**，只作用在逐像素那一路）。
相邻两片三角共享的那条边，在浮点里两侧算出来的边函数不是同一个数 —— 紧贴着边的那个
像素中心可能**两片都认领**。原形（`sph_vcol` 的 (388,4)，用 `OMNI_R3_PIX` 看的）：
那一格收到 **4 个片元**（前面两片深度 0.52843684 / 0.528417289 几乎一样、背面两片也一样），
混出来 24 而参考是 72。GL 那边顶点是定点的，共享边两侧逐位相同，配上 top-left 规则
就"不漏不重"。分母量过：16 → 7155、**256 → 30**、4096 → 477、不吸 → 483（`sph_trans1`），
正好对上 GL 常见的 8 位子像素精度。
账：`sph_trans1` **483 → 30**（最大差 1）、triangles **183 → 115**、
twoSpheres 271403 → 271202、vectorfieldsphere 113056 → 112976、
sacylinder3D 6449 → 6467（略差 18 个字节）；`flat_trans`/`edge_trans` 仍是 0。

- **下一刀该查 `sph_vcol` 那一类**（0.9%、最大差 29）。**先查过一格，把"顶点色插值不对"
  这个猜法否掉了**：拿 `OMNI_R3_PIX` 看 (387,3) —— 两个片元（远的是 0.5035/α0.6、
  近的是**纯黑** 0/α0.6，也就是背面在顶上被照成 0），混出来 0.2808 → **72**，
  与参考的 72 一模一样。差出在**邻居** (388,4)：我们那一格是 24（更黑），
  于是最后那趟 2x2 盒子平均把这一格拉成 60，而参考还是 72。
  也就是说这一类还是"掠射的背面落在哪一格"那件事（与 `sph_trans1` 剩下的 483 同一个根），
  只是黑压白的反差把它放大了。twoSpheres 的 271403 与 vectorfieldsphere 的 113056
  大概也是它。
...
**twoSpheres 与 vectorfieldsphere 剩下的那两块，性质已经量清了**：
- vectorfieldsphere 67387/5368704：28713 个差的像素里 **62% 只差 1**、长尾到 96，
  位置在红箭头压着透明球的边上 —— 最后一位的量级。
- twoSpheres 209791/13390272：140332 个差的像素里 **81% 只差 1**，但有 19007 个 >16。
  大差集中在**画布顶上那条 box 边**（y=2..3、x≈1130）：参考是 0/32/111/127 的渐变，
  我们是 255 与 0 的硬边，而且我们的线在 x=1132 处还没开始、参考已经有色。
  用 `OMNI_R3_PIX` 看过那一格：**没有任何透明片元**，就是线自己的覆盖不一样 ——
  也就是**线的端点与线宽判据**（GL 是"菱形出口"规则的 1 像素宽带，我们多了一个到线段的
  圆盘判据）。这一档要单独开一刀，且会动到所有线的输出（现在很多全是线的图逐字节相同），
  所以先记着、不动。顺手试过"线的底色也放宽到任一采样点覆盖"：sacylinder3D 1488 → 1492
  （略差）、twoSpheres 一个字节不动 —— 也不是这一条。
  **又用五个探针把"线"这条路围了一圈，全都对得上**（所以那 19007 个大差不是"线宽/端点"
  的普遍问题，而是 twoSpheres 那一处的特定组合）：
  - `lw0`（默认 pen 的一条水平线，正对）：**逐字节相同**。
  - `lw`（1bp / 2bp / 4bp 三条线，正对）：24/969600（这几档在两边都走"管"）。
  - `boxln`（`draw(box(...))` 线框，正对）：**逐字节相同**。
  - `boxobl`（同上 + `orthographic(1,1,1)`）：192/1670400、只有参考 2 个像素。
  - `boxtr`（斜相机 + 线框 + 一片 `opacity(0.5)` 的板压着）：156/1670400（比不带透明的还小）。
  也就是说线宽（1 格）、圆头、斜线、以及"线被透明面压住"这四件都不是问题。
  twoSpheres 那一处**已经缩到四行复现出来了**，而且根子不是线：
  ```asy
  import three; size(20cm); currentprojection=orthographic(1,1,1);
  draw(box((-2,-2,-1),(2,2,1)));
  ```
  同一幅图换成 `size(200,200)` 只差 **120**/1670400，换成 `size(20cm)` 差
  **90228**/13390272，而且 ink 只盖住参考的 **92.6%**（只有参考 3232、只有我们 3206 ——
  **双向都有独占，是线整体错位、不是粗细**）。两种画布的 px/bp 都是 4.0，差别在
  **2267.7 → ceil 2268 那个零头**（200bp × 4 = 800 正好整除）。
  也就是说这一档与 memory 里那条"斜相机的 2.8% 是视景体长宽比的零头"同一族：
  **位图尺寸不整除时视口映射差一点**。twoSpheres（2268x1968）与 vectorfieldsphere
  （1364x1312 = 12cm × 4 = 1360.6 → 1364）都落在这一档，而 splitpatch（800x1200）整除、
  它的差 95% 是地板上的 ±1。**下一刀从 `r3_set_dimensions` 里 aspect / fullW / fullH
  与 ceil 的处理进**，判据就用上面那四行。
  **另外试过把线的覆盖判据换成纯矩形带**（去掉"到线段的欧氏距离 ≤ 0.5"那个圆盘判据，
  `OMNI_R3_LINERECT=1`）：twoSpheres 209791 → **223767**、hyperboloid 6749 → **19563**
  —— 明显更差，圆盘（圆头）反而更接近参考。
：把不透明底色改成**在不透明那一趟里顺手收**
...
调是在猜。这一刀先做**六个最小探针**（都是 `size(200,200)` + 正交，写在 `/tmp/r3probe`
里跑，`OMNI_EPS_GEN=1` 让判据自己生成参考）：

- `flat_light` / `flat_nolight`：一片正对相机的平面片（`lightgray`），打光 / `nolight`。
- `flat_trans`：同一片，`+opacity(0.5)`。
- `sph_light` / `sph_nolight`：`unitsphere`，正对，打光 / 不打光。
- `sph_obl`：`unitsphere` + `orthographic(5,4,3)`（斜相机）。
- `sph_trans1` / `sph_trans`：一颗 / 两颗相交的 `opacity(0.5)` 球。

量出来的分类（这一刀之后）：
- **平面片三档全逐字节相同**（打光、不打光、透明）—— 着色、AA、透明混色在满覆盖上都对。
- `sph_nolight` **48**/1920000（最大差 2）：球的轮廓与细分本身已经对了。
- `sph_light` 1098、`sph_obl` 1104：**斜相机不再是一格**（与正对着的数一样），
  这一档是 BRDF / 法向插值的最后一位。
- `sph_trans1` **40932**、`sph_trans` 56154：**透明是现在最大的一类**。

**修掉的那一位：float → unorm8 的乘积要按精确值算。** GL 规范 2.3.5.2 是
`round(f × 255)`，硬件那一格是定点转换 —— 而我们写的是 `(int)(v * 255.0f + 0.5f)`，
乘法先舍进 float。`nolight` 的 lightgray（0.9）正好踩上：`(float) 0.9 × 255` 的精确值是
229.49999394（该出 229），在 float 里算却被舍成正好 229.5，+0.5 就成了 230。
`flat_nolight` 因此 1896075/1920000 个字节全差 1 —— 改成 `(double) v * 255.0 + 0.5`
（新的 `r3_unorm8`，六处量化全走它，顺带把 NaN 归 0，从前那是 UB）之后**逐字节相同**。
八个三维例子上这一改是中性的（139063→139061、4231→4233、6747→6749、10571→10574…
合计 −5 个字节）—— 但它去掉了一整类"平局值上错一位"的错。

**透明那一圈：口径还没定案，两头的账都记在 `r3_raster_tri` 里那段注上。**
现象是"球的轮廓那一圈（2321 个像素）参考是纯白、我们涂了色，紧里面一圈我们又偏浅
（216 对 235，像少混了一层）"。试过"透明那一趟只看像素中心"（`OMNI_R3_TCENTER=1`）：
球那两个探针 40932→27804、56154→31674，多涂的像素 2614→1730、1484→674，
**但 `flat_trans` 从 0 涨到 5967** —— 平面片的透明边在参考里是**软**的，
所以"透明不做多重采样"这个解释是错的。默认因此仍是逐采样点。
下一刀的探针该是"一片几乎与视线平行的透明面片"：怀疑细分出来的窄条在 GL 那边被剔掉，
而我们照涂 —— 那同时能解释"多涂一圈"与"里面一圈少一层"。

**补量（同一刀里做完的，把上面那个怀疑也否掉了）**：
- `edge_opaque`（`rotate(88,Y)` 的平面片，不打光）**逐字节相同**；
  `edge_trans`（同一片 + `opacity(0.5)`）8754/67200，但**最大差只有 3**、多涂 4 个像素。
  也就是说"掠射角的窄条"这一档没有大错 —— 球那一圈的 204 对 255 不是这么来的。
- 把**墨迹集合**（非纯白的像素）四份对着数（同一颗球，打光）：
  不透明那两份 **494760 对 494760，一个像素不差**；透明那两份 497016（参考）对
  499337（我们），**参考没有一个像素是我们没涂的**，我们多涂 2321 个。
- 挑一格看透（x=381、y=1）：不透明两边都是 **239**，透明**参考是 255、我们 244**。
  同一份几何、同一套覆盖（不透明那一趟逐字节对上了），参考的透明那一趟在这一格上
  **一层都没混**。
- 第三条解释也否掉了：**"中心不在三角里就把这一片在这一格上整片丢掉"**
  （`OMNI_R3_TGATE=1`，写哪几个采样点照旧逐采样判）—— 球上 40932 → **541353**
  （最大差 116）。细分出来的小三角很多，一格的中心只落在其中一个里，其余的贡献全被丢了。

三条解释都不成立，所以先造了工具：`OMNI_R3_PIX=x,y` 把那一格的**透明片元表**
（排完序：颜色、alpha、深度）与**取平均之后、2x2 盒子模糊之前**的四个采样点字节印出来。
拿它把 (381,1) 看到底了，结论**换了一个方向**：

- 我们在 (381,1) 上其实**一个采样点都没涂**（四个都是 255）。那 244 全是最后那趟
  2x2 盒子平均从右下角 (382,2) 带过来的：`(255+255+255+214)/4 = 244.75` → 244。
- (382,2) 上只有**第 3 个采样点**被涂（91），它收到**两个片元**（球的前后两面：
  深度 0.499733657 与 0.500266314、alpha 各 0.5）。
- 同一格在**不透明**那一版：我们与参考都是 239 —— 而 239 正是
  `(255+255+255+191)/4`，也就是说那一趟**同一个采样点也被涂了**（0），
  模糊那一格的算术两边也对得上。

也就是说：**参考的透明球在这个采样点上没有任何几何，而它自己的不透明球有** ——
而我们两趟都有。差的不是合成、不是覆盖口径、也不是模糊。

**读了本机装的那两个 shader（`/opt/homebrew/share/asymptote/shaders/`），根子找到了：
参考的 OIT 是"逐像素"的，我们的是"逐采样点"的。**
- `count.glsl` 全文就一句：
  `atomicAdd(index[uint(gl_FragCoord.y)*width+uint(gl_FragCoord.x)],1u); discard;`
  —— 计数的下标是**整数像素坐标**，一格一条链，与采样点无关。
- `blend.glsl:78` 同一个下标取链；`:101`
  `outColor=OpaqueDepth != 0.0 ? opaqueColor[pixel] : push.background;`
  —— 混色的**底色**是那一格的不透明色或纯背景（也是逐像素的一个值），
  混完 `outColor` 也只写一个值。`:104-106` 还会把深度在不透明面之后的片元跳掉
  （`while(k < size && depth[listIndex+k] >= OpaqueDepth) ++k;`）。

我们这边是 `R3_NS` 个采样点各一条链、各自混、最后平均。两套结构在"一格只被部分采样点
覆盖"时必然不同 —— 而透明那一族的残差正好全长在这种格子上（球的轮廓、面片的接缝）。

**下一刀（结构性的，不是调参）**：把透明那一趟照 shader 的结构重写 ——
每格一条片元链（`gl_FragCoord` 的整数下标）、底色取那一格的不透明色/背景、
按 `opaqueDepth` 剪掉被挡住的片元、混完写一个值。做完再量那六个探针：
`flat_trans`（现在 0）不能坏，`sph_trans1`（现在 40932）该掉下来。

**两条把这个模型钉住的旁证**（都量过，别再重挖）：
- **不透明那一路没有跟着切换。** `mix_a`（全不透明的球 + 一个小球）与 `mix_b`
  （同一幅，小球换成 `opacity(0.5)`）的**参考**只在小球那 21x21 的方块里不同
  （360 个像素，包围盒 x 777..797、y 777..797），大球**逐字节相同**。
  所以参考里"有没有透明"不影响不透明那一趟 —— 我们照旧逐采样点渲染不透明，是对的。
- **参考里透明面片的"软边"是最后那趟 2x2 盒子平均做出来的，不是覆盖度做出来的。**
  `flat_trans` 参考的一行：`255 248 242 242 …`，而 248 正好是
  `(255+255+242+242)/4`；不打光那份是 `255 242 229 229 …`，242 = `(255+255+229+229)/4`。
  也就是说底下的覆盖是**硬**的（要么 0 要么满）—— 这与"逐像素、按中心判"一致，
  而 `flat_trans` 之所以在我们逐采样点那一版也逐字节对上，是因为那片方片的边
  恰好落在像素边界上（内部 242 从 x=2 起，x=1 是纯白）。

## 第九刀（已做）：下标写的长度检查、`asy__rf`、四格出参

三条一起量（pdb，`-O0`，两趟）：**5.21s/10.94s → 4.97s/10.38s**（−0.24s / −0.56s），
八个三维例子逐字节不变。三条各自的机制：

- **`a[i] = v` 只在真要长的时候才进 `asy__grow_元素`**（frontend-asy/stmts.js 的
  `asyAssignIndex`）。判据与那个 helper 体里的 while 一字相同（`(alen a) <= i`），
  所以长度与补的零值都不变。剖 pdb：`asy__grow_real` **1.09 亿次调用**、自用 5.3s 排第二,
  而它绝大多数执行就是"判据一次不成立就返回"。这一格自己只值 0.04s/0.12s ——
  **又一次印证"自用时间只能定位、不能估收益"**（插桩把这种高频叶子撑得极大）。
- **`asy__rf` 在 `asy__pboundtri2` 里手工摊成除法**（它就是 `v.x/v.z` / `v.y/v.z`）。
  剖 pdb 它被调 3838 万次，十次里九次出自那一族的十处。逐位不变：同一个除法。
- **`asy__sbound2tri` 的四格出参复用一份模块级缓冲**（`asy__ob4`）。从前是
  `real[] ob = {0,0,0,0}`，也就是每片三角一次 arena 分配（pdb 上 256 万次）。
  安全的理由与 `asy__cb16*`/`asy__cb10*` 同一条：`asy__addtri3` 不重入，三次调用
  各自读完再进下一次，退回递归的那一支不碰这一格。

顺手修掉一条**与三维无关的老账**（`tests/asy/cases/10-pairs`，全量套件里唯一的红）：
JS 与解释器那两侧手写的 `fma` 在**乘积溢出**时算的是 `inf - inf = nan`（Dekker 拆分），
而真 fma 先精确再舍一次，1e400 舍出来是 inf。于是 `abs((1e200,1e200))`
（`asy__pabs = sqrt(fma(y,y,x*x))`）在 asy 与 run-c/run-llvm 上是 `inf`、JS 腿是 `nan`。
改法：乘积或加数非有限时退回朴素式 `a*b + c`（那一档它是对的 —— 已经超出 realMax，
再加个有限的 c 还在那之上）；另外 `SPLIT*a` 自己溢出把结果污染成 nan 时也退回。
现在五条腿一致、与 asy 逐字节相同。
七个例子（含 filesurface 17550 / triangles 10571）**逐字节不变**。

**第五刀：三角面片那六个界融合成三趟**（`asy__sbound2tri`）。`asy__addtri3` 从前对
min/max × x/y/z 各调一次 `asy__sboundtri`，每次再各调一次 `asy__scornerboundtri` /
`asy__scontrolboundtri` —— 而**绝大多数面片在第 0 层就返回**，那一层的活是"三个角点取
极值 + 七个控制点取极值"，min 与 max 两侧读的是同一批数。现在一个分量一趟算出两侧，
只有"控制点跑出角界超过 fuzz"的那一侧才退回原来那份递归（参数一个字不改）。
只值 0.16s/0.19s —— 剖出来那三样合起来自用 9s，但那是**插桩把高频叶子撑大了**
（1557 万次调用，每次两回 clock_gettime）。这一条再次印证：按自用时间排也只能定位，
不能拿来估收益，收益要用不带插桩的墙上时间量。

**第六刀：求界的分量缓冲复用**（`asy__cb16*` / `asy__cb10*`）。`asy__addpatch3` /
`asy__addtri3` 从前每片新建三个 `real[]` 再逐个 push —— pdb 上两个合起来两百多万次调用，
就是六七百万次 arena 分配加增长搬家。改成模块级定长缓冲（四边 16 格、三角 10 格
**各一套** —— `asy__norminf` 与 `asy__sbound*` 读的是 `.length`，共用一套会把多出来的
格子算进去）。复用安全是因为这两个函数不重入；递归那一支里的 L/R/U/C 照旧现造。
值 0.11s/0.31s。

**第七刀：变换后的界并进同一趟**。`real[][] * frame` 从前是"先建 g 的 op 表、再走
**第二遍** op 表按变换后的几何攒界"；自检（`b3fromops`）只看未变换的 `s`，与那趟循环无关，
所以提到循环前，界就能在建 q 的同时攒（`b1`）。少的是 237 万次派发加逐 op 读字段 ——
op 的次序、种子、答案都没变（同一批 op、同一个顺序）。值 0.31s（第二趟）。

**第四刀：4x4 在循环外摊成十六格标量**（`asy__m16` / `asy__m16of` / `asy__m16mul`）。
`real[][] * triple` 每次调用要摸二十次数组 —— 4 行 + 16 格，每一格都带"越界就问登记册"的
预检与一次取长度 —— 而它在 pdb 上被调 **2578 万次**（自用 3.5s 排前三）。矩阵摊进一个
十六字段的记录之后，逐点只剩十六次字段读；表达式次序一字不动（`0 +` 起头、`* 1` 末项、
...
- 第一次 `fitAspect` 用的 `Aspect` 与最后一步用的**不是同一个**：最后一步明写着
  `(double)fullWidth/fullHeight`（renderBase.cc:992-995），位图那一对。从前把这两个
  混成一个，于是要靠 `-1e-9` 去补"数学上是整数、除法差最后一位"那类毛病 ——
  那些补丁本来就是混用逼出来的。
...
- 第一次 `fitAspect` 的 `Aspect`，**我们用的是 m/M 的比，不是 `args.width/args.height`**。
  这一条是量出来的，与源码那一句字面上不一致，所以单独记（见下）。

**`Aspect` 那一格：字面照抄反而不对（量了一轮，退回来了）。** `args.width` 就是
`S.width - defaultrender.margin`（three.asy:2905，box3 上 567.98/492.98 = 1.1521360）。
为了照抄，把这一对经清单新增的 `wh` 行传进运行时试了一轮，结论：

- box3 从 2148 变成 **91926**（比不改还差）。原因很直接：`1.1521360` 比
  `1089/945 = 1.152381` 还小一丝，第一次 fitAspect 于是给 `w=1089`，最后落在 1.152381，
  而参考要的是 1.15345。要落到 1090/945 就必须 `Aspect > 1089/945`。
- m/M 那一对（未取整的 `lambda+2*margin`，567.929/492.042 = 1.1542289）给 `w=1091`，
  最后一步再按 `fullW/fullH` 套回 **1090/945** —— 对上了。
- 退化画布上差别更清楚：`diag2`（2268x4）用 m/M 那一对算出 **aspect 正好 512**
  （= 1024/2：第一次 fitAspect 把 w 从 1134 压到 758，于是撞上 1024 那个分块下限），
  而按 ink 反推出来的就是 512（参考的墨占 1675 列，512 算出来 1677）；
  pt 那一对给 378.7，画出来的线会铺满整幅。

为什么"未取整的那一对"才对还没跟到底（`args` 在 EPS 导出那条路上怎么填的没查）。
代码里那一段注释写清了这三组对照数，**别再把它换成 `args.width/args.height`**。

走这一路的前提是"m/M 真的就是画布尺寸"，判据 `fw == 4*(int)wpt && fh == 4*(int)hpt`
（`shipout3` 那边是 `oW=(int)width`、`fw=ceil(4*oW)`）。**透视那一路对不上**（m/M 不是
画布尺寸），自动退回按位图反推的老版（连 `-1e-9` 一起留着）—— 那一档是下一刀。
...
### 顺手把 pseudosphere 那 138 万定性了：是地板被大画布放大，不是新的一类

改完之后按"画布高 > 1888 的参考"扫了一遍（只有七份），把两个没量过的例子补上：
tetra **49142**/10303200（ink 99.9%）、pseudosphere **1382926**/15431472（ink 100%）。
pseudosphere 一度看着像"整幅右移一像素"（同一行上 `我们(x+1) == 参考(x)`），**那是读错了**：

- 按量级拆开，1382926 里 **1167105（84%）只差 1**，32~127 那一档 6 万、128 以上 1262 个;
- 在平滑区做整数位移的互相关，最优位移在 0/1/2 之间摇摆，而残差只有 0.05~0.4 ——
  平滑渐变上 ±1 的花纹本来就会让"最优位移"无法确定。

画布是 2268x2268（15.4M 字节），9% 的字节差里绝大多数是那一档 ±1 的地板。真正值得追的
...
再跑十一个（同一条件，箭头右边是这一刀之后的数）：**colorpatch 51847 → 811**、
**splitpatch 139063 → 3021**、**vectorfieldsphere 162572 → 2009**、
**sacylinder3D 18813 → 1488**、**triangles 10571 → 115**、**vertexshading 4792 → 1615**、
**filesurface 17550 → 7681**、**conicurv 13215 → 8250**、twistedtubes 4233。
剩下两个大的是 **pseudosphere 58555**/15431472（最大差 123）与
**smoothelevation 50469**/5779200（最大差 128）—— 下一批的靶子。

顺带钉一个干净的新探针：**一片平三角**（`draw(surface((0,0,0)--(1,0,0)--(0.3,1,0)--cycle),
