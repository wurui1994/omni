...
3. **真正的杠杆是访存与调用约定。** 这一格量错过一次，更正记在这儿：我先量的是
   `cMir`（**线性内存**那条腿）的 MIR，而那条腿是给 wasm/js/解释器用的 —— **C 与线性内存
   无关**，原生腿上局部量的那一块是 `FRAME`（真地址）。重新按 `cMirNative` 量，
   `radiance` 过完管线 732 条指令，里头：

   - `LOAD 97 + MLOAD 89 + MSTORE 84 + STORE 57 = 327`（**45%**）是访存
   - `ARGMEM 76 + ARGSRET 45 + CALL 62 = 183`（**25%**）是**按值传 struct 的调用约定**
     （`Vec` 是 24 字节 ⇒ 进内存、指针走 x8，见 ir.js 的 `ARGMEM`/`ARGSRET`）
   - `ADD 79 / MUL 16 / DIV 5` 才是真正的活

   clang -O2 在边界上用的是同一套 ABI，差别是它把 `add`/`mul`/`norm` 那几个 `static`
   **内联**掉了 —— 边界一消失，那 25% 与大半访存一起没了。

所以顺序是：**内联 → SROA → mem2reg → regalloc**（正是第 3 节那条三步流水的次序，
Go 在 SSA 之前就内联完了，`inline.InlineDecls`）。SROA 已经落了
（`opt/sroa.js`，`decompose user` 那一格，只认 `FRAME`、地址不逃逸、偏移是编译期常数），
真产物上指令 -5%~-9%，但**时间不动** —— 因为 smallpt 的 `Vec` 是按值传进 `add` 的，
地址一进实参池就算逃逸，拆不了。**下一格是内联**。

还有一笔没还清的账：整条管线在 call-heavy 的 `fib` 上仍是 ×1.33（逐格累加量下来每一格
都 ≤1.01，差别出在"同一格跑多遍"的那几趟 deadcode/opt 上）—— 那说明这一层还有一个
"合并多了反而亏"的口子没堵住，下一轮要么按 `cost.js` 的同一条判据堵掉，要么把它记成
"等有了会溢出的分配器再说"。

`dead auto elim` 那一格现在无活可干的理由另记一笔：C 前端在**线性内存腿**上把那一块的
基址当场 `GSTORE` 进了 `$sp`，照 Go 的判据答"留着"（见 `opt/autos.js` 末尾）；
原生腿上那一块是 `FRAME`，已经由 SROA + `elim unread autos` 接走了。
