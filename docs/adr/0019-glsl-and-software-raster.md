...
11. **分层加速**：llvmpipe 的 `do_block_16`/`do_block_4`（16×16 块一次判 16 个 4×4 子块的
    全内/全外/骑边）。这是性能那一半的大头，但**先要有基线** —— 与决策一同一个道理。
12. **llvmpipe 那一头怎么取图** —— 本机是 macOS，`benchmark.py` 的 surfaceless EGL 走不通。
    这一格不解决，「与 llvmpipe 比」就是句空话；顺带也是「填充规则与 GL 同不同侧」的唯一判据。
13. **像素比对的容差的具体数字** —— 决策二定了量法，数字要等 12 解决。
