...
17. **SoA / 向量化**（决策一里那条「等基线」的路）。基线与理由现在都有了：时间 95% 以上
    在着色器里，而方言已经有 `(vec real 4)`/`splat`/`lane`。这是性能那一半唯一的大头。
18. **llvmpipe 那一头** —— docker daemon 起来就能取（`benchmark.py` 那条 surfaceless EGL 路）。
    它带来的是与**软件**光栅化器的性能对照，以及「边上归属」在 llvmpipe 上是哪一侧
    （规范没规定，要量）。
19. **512² 与 1024²**：`perf.js` 收了这两档（`node tests/glsl/perf.js 512`），但还没跑过 ——
