...
门里 `TOL = 1`，可本机量到 0。**不收成 0** 的理由写在这儿：

- 8 位量化那一步把 `round(v*255)` 的边界放大 —— `v` 差 `1e-9` 也可能让整数差 1；
- 超越函数在别的 libm / 别的 GPU 上差 1 ULP 是常态（`src/runtime/omni_math.c` 头注里
  那 51.1 万个位不同的量就是这件事）。

所以 `TOL = 1` 是「量出来的 0 + 一格已知的实现自由度」，不是拍的。门每次都把**实测的最大差**
印在消息里，于是它从 0 变成 1 的那一天会被看见。
...
13. **分层加速**：llvmpipe 的 `do_block_16`/`do_block_4`（16×16 块一次判 16 个 4×4 子块的
    全内/全外/骑边）。这是性能那一半的大头，基线已经有了（第五片那几个数）。
14. **llvmpipe 那一头** —— docker daemon 起来就能取（`benchmark.py` 那条 surfaceless EGL 路）。
    它带来的是**性能**对照、以及「边上归属」在 llvmpipe 上是哪一侧（规范没规定，要量）。
15. **更大的画布与更多用例**：现在 oracle 只比 16×16。`benchmark.py` 的五个尺寸
    （128…2048）该逐个比一遍 —— 那也是性能那条线的入口。
