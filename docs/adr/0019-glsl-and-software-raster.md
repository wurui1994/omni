...
7. ~~第二档的降级~~ —— 第六片（除了插值）。
8. **插值（varying）与顶点着色器** —— 第二档里唯一的真活：`a0`/`dadx`/`dady` 那一套
   （`lp_bld_interp.c`），而它要先有「三角形 setup」。值一片。
9. **llvmpipe 那一头怎么取图** —— 本机是 macOS，`benchmark.py` 的 surfaceless EGL 走不通。
   这一格不解决，「与 llvmpipe 比」就是句空话。
10. **像素比对的容差的具体数字** —— 决策二定了量法，数字要等 9 解决。
