...
18. **给方言加向量比较（掩码）与 `select`** —— 形状量过了（「量：SoA 要方言长出什么」那一节），
    但**理由改了**：这是**语义补全**（决策一里明着挡住的那一格），不是提速。
    动六条腿，要单独的门（`tests/sexpr/cases/03-simd.sx` 那一族的下一档）。
    做完之后把 `glsl_frag` 换成 `(vec real 4)` 只在 **LLVM 腿**上会更快 ——
    JS 腿（数组）与 C 腿（标量化结构体）反而可能更慢，那不是 bug，是刻意的取舍。
