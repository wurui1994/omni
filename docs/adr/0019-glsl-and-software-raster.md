...
14. **分层加速**：llvmpipe 的 `do_block_16`/`do_block_4`（16×16 块一次判 16 个 4×4 子块的
    全内/全外/骑边）。这是性能那一半的大头，基线已经有了（第五片那几个数）。
15. **llvmpipe 那一头** —— docker daemon 起来就能取（`benchmark.py` 那条 surfaceless EGL 路）。
    它带来的是**性能**对照、以及「边上归属」在 llvmpipe 上是哪一侧（规范没规定，要量）。
16. **`benchmark.py` 那五个尺寸的性能表**（128…2048，`Avg(ms)`/`MPix/s`/`Frame/s`）——
    要走第五片那条不印东西的路，而且得把编译摊出去。
