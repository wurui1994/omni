...
2. ~~GLSL 接到哪一层 IR~~ —— 决策一。
3. **第二片：类型检查与名字解析**。语法这一层不查的那几件事全在这儿：
   swizzle 那串字母对不对（`vec2` 没有 `.z`）、构造的元数对不对（`vec3(a,b)` 是错的）、
   混算的提升规则（向量 op 标量）、内建表（含 `pow`）、`gl_FragCoord`/`gl_VertexID`/
   `gl_Position` 这三个内建变量、`discard` 明着拒。
4. **第三片：降到 MIR**，按决策一的两条形状（一个片元一个函数、循环按 quad）。
5. **像素比对的容差的具体数字** —— 决策二定了量法，数字要等能出图。
