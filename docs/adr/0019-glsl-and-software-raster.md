...
16. **分层加速**：llvmpipe 的 `do_block_16`/`do_block_4`（16×16 块一次判 16 个 4×4 子块的
    全内/全外/骑边）。基线已经有了（第十四片那张表），优化能不能量出来现在可验。
17. **llvmpipe 那一头** —— docker daemon 起来就能取（`benchmark.py` 那条 surfaceless EGL 路）。
    它带来的是与**软件**光栅化器的性能对照（真 GL 那一档是硬件，量级差太远），
    以及「边上归属」在 llvmpipe 上是哪一侧（规范没规定，要量）。
18. **512² 与 1024²**：`perf.js` 收了这两档（`node tests/glsl/perf.js 512`），但还没跑过 ——
