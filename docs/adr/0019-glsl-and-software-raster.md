...
9. **真正的光栅化：覆盖判定**。llvmpipe 的两级掩码（`do_block_16`/`do_block_4`，
   前面量过）。现在那一段**假设三角形铺满画布**，喂一个不铺满的会把外面也画上 ——
   这是当前最大的一处「写在明处的不完整」。
10. **llvmpipe 那一头怎么取图** —— 本机是 macOS，`benchmark.py` 的 surfaceless EGL 走不通。
    这一格不解决，「与 llvmpipe 比」就是句空话。
11. **像素比对的容差的具体数字** —— 决策二定了量法，数字要等 10 解决。
