...
**第六、七、八把尺子：新降级的三条腿**（拿**旧降级的真输出**当外部尺，
`node src/cli.js emit sx x.jnc`）。三把尺子逐格/逐行/逐名对，账一笔一笔记：

- **聚合体**（`jnc-struct-emit.js` + `emit-agg.js`）：对比字段 **325 格全对（100%）**、
  整行 **229 行一模一样 229（100%）**。一路顶上去的那几刀都是尺子先印出差别再补表的：
...
- **函数头**（`jnc-fn-emit.js` + `emit-fn.js`）：**635 行一模一样 635（100%）**。
...
2. **新腿发了、旧降级没有这个名字的**：这一栏防的是"悄悄发一个不存在的符号"，
   它当场抓出过一个真错（尺子算键时漏传"在属性体里"，键算成了 `Box$m_v$op$index$set`）；
3. **旧降级发了、新腿还没试的**：**127 格，全是语句/表达式那条腿** ——
   `$newoN`×80 与 `$newcN`×7（`new` / 花括号初值的壳）、运行期助手 `jnc$…`×35、
   别的×5（局部 alias 与宿主面那几格）。声明驱动的那一整片已经清零
   （生成的取/存、生成的构造、reactor 的体、虚派发、别名、extension、跨文件那几族）。

**下一刀（已定位）**：**语句/表达式**那条腿 —— 上面第 3 栏那 127 格全从它来。
这一栏第一次量出来是 227 格，一刀一刀压到 127；每一刀都要同时看住第 2 栏（它抓出过 5 个真错：
`autoget` 误判成"要构造"、方法体里的局部量误判成"字段带初值"、尺子算键漏传"在属性体里"、
形参里的用点误判成"体外成员"、extension 的方法没挂到目标类型上）。
