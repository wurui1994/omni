...
- **函数头**（`jnc-fn-emit.js` + `emit-fn.js`）：**508 行一模一样 508（100%）**。
  规则：名字 `<东家>$<方法名>`（命名空间只是前缀，套起来是 `a$b$deep`）、方法多一格 `$this`
  （类那一族写**连通分量的根**）、形参用源码名字、形参与返回都在**存储位置**、
  重载改名 `$o<N>`、特名过一张表（`static construct` → `construct$static`）、
  算符 `op$inc` / `op$inc$post` / `op$call` / `op$bool`、属性取存 `<属性>$get`、
  裸写的 `get`/`set` **带体是下标算符**（`op$index$get`）、体外定义的点串整串用 `$` 接。
- **泛型单态化**（`jnc-generic.js` + `generic.js`）：实例名 **33/33（100%）**。
  ...
  泛型 typedef 也是模板（合成出来的是一条 typedef，没有体）。
  实例接进上面两条腿之后，字段 278 → 325、整行 197 → 229、函数头 478 → 508。

**还挂着的账（明说，不猜）**：函数头那边"返回类型解不出来×24"里最大的一族是
**实例体里的嵌套 typedef**（`123-genericname.jnc` 的 `Entry val()`）—— 那要**每格实例一份 env**
（`Box$Node` 与 `Box$int` 里的 `Entry` 是两个类型），尺子现在只有一份按裸名索引的 env。
另外"没有名字×5"是体外定义里更绕的点串、"认不出形参表×5"是形参表里还没认的那几种形状。
