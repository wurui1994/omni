...
- **函数头**（`jnc-fn-emit.js` + `emit-fn.js`）：**577 行一模一样 577（100%）**。
  ...
  **存储位置**、没写返回类型就是 void、重载改名 `$o<N>`、特名过一张表
  ...
  reactor 的 `$start`/`$stop`、虚派发 `<链的根>$$vd$<方法名>`（形参 `$aN`）。
...
3. **旧降级发了、新腿还没试的**：185 格，按族排 —— `$newoN`×80 与 `$newcN`×7（`new` / 花括号
   初值的壳）、运行期助手 `jnc$…`×35、别的×26、生成的取/存×17、生成的构造×14、
   reactor 的体 `$rN`×4 + `$eN`×2。前两族与最后一族都要等**语句/表达式**那条腿。

**下一刀（已定位）**：语句/表达式那条腿 —— 上面第 3 栏里除了"生成的构造/取存"之外全从它来。
