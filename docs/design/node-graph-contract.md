...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**275 passed / 0 failed / 1 skipped**（语言例子 64 + 手搭图 5 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 69/69 · sx 69/69 · js 69/69 · wat 68/69`。
...
现状：**41 样 × 69 份例子，41 passed / 0 failed**（节点 27 格 + 附属 14 格）。
数出来的半径（删了它，几份例子要它）：
...
**为什么默认只跑两门小的**：量过 —— 九门语法一共 2585 条产生式，一次建表平均 140ms，
整跑一遍是分钟级的。所以这条轴按语法给：默认 chez + sbcl（62 条，秒级），
`--all` 或写语言名跑别的。跑哪几门是**时间的事**，判据对每一门都一样 ——
这句话写在文件头上，不留"挑好看的跑"的空间。
...
（`fromtree.js` 的 `mapNames`，十几行）。那笔账剩下的欠款人只有两个：
