...
- `gaps('wat')` 现在真的印出**缺口**（不是"暂不支持"，是一句人话一条）。
  ...
  break/continue 补上之后**剩 2 格**：`scope-exit`（wasm 没有 unwind）/ `conv`（要 f64）。
...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**189 passed / 0 failed / 23 skipped**（语言例子 51 + 手搭图 2 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 53/53 · sx 53/53 · js 53/53 · wat 30/53（缺口 2 格）`。
