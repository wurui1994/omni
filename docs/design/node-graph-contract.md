...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**222 passed / 0 failed / 2 skipped**（语言例子 51 + 手搭图 5 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 56/56 · sx 56/56 · js 56/56 · wat 54/56`。
...
**还在跳的 2 格 —— 而且都不是"哪格节点接不住"（节点级缺口清单已经空了）：**

节点级的 `gaps('wat')` 现在是 0 格，可矩阵里还有两格在跳。这两件事都对，因为它们问的
不是同一个问题 —— 所以契约里多了一问的答案：**形状上的账**（`shapeGaps`，后端自己报，
现在 4 条：底下这两条 + 串的拼接 + 实数进内存）。不分开记的话，
"没有缺口（全接得住）"这句话就会盖住还在跳的格子，那正是这一节最怕的事。

1. **嵌套的 `func`（闭包），1 格**（chez 的 `basics`）—— 要先定"环境怎么排"：
   线性内存里的一块 + 一格函数下标（与记录同一台机器）。
2. **一格量先装串后装数，1 格**（awk 的 `tag`）—— wasm 的局部量只有一种类型，
   而图这一层没有类型。awk 没有声明，所以这是**真的答不出来**：报缺口是对的，
   印一格地址才是错答案。要接就得有类型层（附录 A.5 的 `carry`）。

四格顺手关掉的（都不是"多写一格节点"，是**把两层的约定对上**）：
