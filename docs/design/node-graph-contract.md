...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**271 passed / 0 failed / 1 skipped**（语言例子 63 + 手搭图 5 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 68/68 · sx 68/68 · js 68/68 · wat 67/68`。
...
现状：**41 样 × 68 份例子，41 passed / 0 failed**（节点 27 格 + 附属 14 格）。
数出来的半径（删了它，几份例子要它）：
...
cpp 的"声明还是表达式"、nim 的 `T(x: 1)` vs `f(x = 1)` —— 它们是真的要回问。
