...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**223 passed / 0 failed / 1 skipped**（语言例子 51 + 手搭图 5 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 56/56 · sx 56/56 · js 56/56 · wat 55/56`。
...
现状：**37 样 × 56 份例子，37 passed / 0 failed**（节点 23 格 + 附属 14 格）。
数出来的半径（删了它，几份例子要它）：
