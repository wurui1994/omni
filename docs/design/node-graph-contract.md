...
- `wat` 落一份**自足的模块**（宿主面就是那四格 `print_*` 导入）—— 这是 target.md 那句
  ...
  **还欠一条判据（明说）**：把这份 `.wat` 交给一个**真的** wasm 引擎（wat2wasm + V8 /
  wasmtime）跑一遍，输出仍要逐行相同。现在的判据是 `frontend-wat` + MIR 解释器 ——
  那是一条互不相干的实现，但仍然在这棵树里。写这一段的机器上没有 wabt / wasmtime，
  所以这一格现在是**没跑过**，不是"跑过了"：等哪台机器上有那两样再补，补的时候
  没装工具就报一句有名有姓的 skip（与矩阵里那格 awk × wat 同一条纪律）。
