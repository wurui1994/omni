...
- `gaps('wat')` 现在真的印出**缺口**（不是"暂不支持"，是一句人话一条）。
  第一版 10 格 → 记录与列表上线性内存后 4 格 → 多值与切片也上去之后**剩 3 格**：
  `scope-exit`（wasm 没有 unwind）/ `loop-exit`（墙在 OIR）/ `conv`（要 f64）。
...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**184 passed / 0 failed / 28 skipped**（语言例子 51 + 手搭图 2 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 53/53 · sx 53/53 · js 53/53 · wat 25/53（缺口 3 格）`。
