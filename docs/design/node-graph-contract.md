...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**243 passed / 0 failed / 1 skipped**（语言例子 56 + 手搭图 5 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 61/61 · sx 61/61 · js 61/61 · wat 60/61`。
...
现状：**41 样 × 61 份例子，41 passed / 0 failed**（节点 27 格 + 附属 14 格）。
数出来的半径（删了它，几份例子要它）：
