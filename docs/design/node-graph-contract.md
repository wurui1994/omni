...
**为什么默认只跑两门小的**：量过 —— 十门语法一共 3104 条产生式（cpp 一门就 519 条），
一次建表平均 140ms，整跑一遍是分钟级的（cpp 单跑一门就 130s）。所以这条轴按语法给：
...
```
go ：产生式 354 条 · 例子用到 109 条 · 备用 245 条 · 建不出表 0 条（22.3s）  3001 passed
nim：产生式 443 条 · 例子用到 110 条 · 备用 333 条 · 建不出表 0 条（159.4s）5033 passed
lua：产生式 113 条 · 例子用到  55 条 · 备用  58 条 · 建不出表 0 条（21.0s）   792 passed
```
...
lua 那一行是**另一头的参照**：同样九份例子，覆盖到 49%（go 31%、nim 25%、cpp 21%）。
差别不在"lua 的例子写得好"，在**语法本身的大小**：lua 那份 113 条产生式几乎全是"真在写
lua 就会用到"的形状，而 go / nim / cpp 的语法里装着一整门工业语言。所以这个比例不能当
"质量分"读 —— 它读出来的是"这门语言有多少东西这一批不碰"。nim 那一行还多说一件事：
**十三份例子**（这十门里最多的）也只把 443 条里的 110 条用起来，而单份例子要的是 41～72 条 ——
也就是说十三份例子之间**重得厉害**（骨架那一截人人都要），加一份新家族只往上多推几条。
