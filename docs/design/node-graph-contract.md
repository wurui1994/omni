...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**267 passed / 0 failed / 1 skipped**（语言例子 62 + 手搭图 5 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 67/67 · sx 67/67 · js 67/67 · wat 66/67`。
...
现状：**41 样 × 67 份例子，41 passed / 0 failed**（节点 27 格 + 附属 14 格）。
数出来的半径（删了它，几份例子要它）：
...
**lua 不在这一族里，理由要写下来**：lua 的 table **既是数组又是字典**，
`t = {}` 之后是当列表用还是当字典用，树上分不出来。按"键的形状"猜（串键就是 map）
对例子成立、对一般程序不成立 —— 所以这一批不收它，记成账。
