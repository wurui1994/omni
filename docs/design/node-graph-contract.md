...
- 新加一个例子家族 `intmath`（期望 `15 / 120`）—— 刻意"贫瘠"：只有整数、函数、调用、
  语句位置的 `if`、`while`、打印一格整数。**七门语言各一份**
  （lua / go / V / nim / mojo / awk / freebasic），四个后端全绿 = 28 格。
  chez 与 sbcl 进不来，理由有名有姓：Scheme / CL 的 `if` 是**表达式**（函数体最后一格
  就是返回值），而 OIR 那侧 block 不带 result；chez 那份还嵌了一格内层 `define`（闭包）。
  这两笔正是下面那张缺口清单里的两条 —— **不是"这两门语言不行"，是那两格还没做**。
...
- 矩阵里因此第一次有了 **skip** 这一档：33 格跳过，**每一格都带理由**
  ...
  现在的总账：**127 passed / 0 failed / 33 skipped**（语言例子 38 + 手搭图 2 × 后端 4）。
