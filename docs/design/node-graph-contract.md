...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**199 passed / 0 failed / 21 skipped**（语言例子 51 + 手搭图 4 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 55/55 · sx 55/55 · js 55/55 · wat 34/55（缺口 1 格）`。
  还在跳的那 21 格几乎全是**字符串**（宿主面只有 `print_i64`），不是节点接不住。
