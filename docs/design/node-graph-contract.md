...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**239 passed / 0 failed / 5 skipped**（语言例子 56 + 手搭图 5 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 61/61 · sx 61/61 · js 61/61 · wat 56/61`。
...
wat 那条腿这一族**整族跳过**，缺口清单因此从 0 又回到 4 格（有名有姓的一句：
"键怎么比、满了怎么扩、缺键报什么，三条都还没定"）。缺口清单会变短也会变长，
这就是它该有的样子。

下一批：`scope-exit` 还欠的四个提供者（lua 的 `<close>`、mojo 的 `with`、
freebasic 的 `Scope`、cpp 的 RAII）—— 这四个都要先有"方法调用 / 析构"那台机器，
不是硬凑得出来的；以及 wasm 上的 map（那三条要先定）。
