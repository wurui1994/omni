...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**461 passed / 0 failed / 3 skipped**（语言例子 102 + 手搭图 5 × 后端 4，
  再加 wat 那 5 条形状账各一份证物、五栏指纹那两组撞车、一格"出处齐不齐"、
  G1 与 G2 那八格、**19 格节点的"规格 vs 矩阵"提供者账 + 一格"账齐不齐"**），
  每条腿末尾还印一行覆盖：`interp 107/107 · sx 107/107 · js 107/107 · wat 104/107`。
...
**第二十五批之十七：cpp 的 `std::make_pair` 进矩阵 —— 多值那两格 8/8，而剩下两笔账被量清了** ——
总账 457 → **461**，例子 102 份。
