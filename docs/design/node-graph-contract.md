...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**211 passed / 0 failed / 9 skipped**（语言例子 51 + 手搭图 4 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 55/55 · sx 55/55 · js 55/55 · wat 46/55（缺口 1 格）`。
...
**还在跳的 9 格，按堆分（数是跑出来的，`node tests/graph/run.js --gaps`）：**
...
3. **闭包，1 格**（chez 的 `basics`：嵌套 `define`）—— 要先定"环境怎么排"
   （线性内存里的一块 + 一格函数下标），与记录同一台机器。
4. **既装串又装数的那格量，1 格**（awk 的 `tag`）—— awk 没有声明，所以这是**真的没类型**。
   报缺口是对的，印一格地址才是错答案。要接就得有类型层（附录 A.5 的 `carry`）。

两格顺手关掉的（都不是"多写一格节点"，是**把两层的作用域对上**）：

- **变参的内建**（CL 的 `(+ a b c)`）：折法与 arity 都**从 `prims.js` 那张表查** ——
