...
**为什么默认只跑两门小的**：量过 —— 十门语法一共 3100 条产生式（cpp 一门就 515 条），
...
**cpp 上这条轴还多量出一件别处量不到的事**（`node tests/grammar/delete.js cpp`）：
...
那 5 处就是 A.5 第 3 笔账的**大小**，而且是数出来的、不是我说的。看一处最清楚的：
删掉 `stmt -> expr ";"` 之后 `printf("%d\n", sumto(5));` 照旧解析成功，
读成了**一格声明** —— `printf` 当类型说明、`(…)` 当构造初始化式：
...
C++ 标准 [stmt.ambig] 说的就是这个：这两种读法**语法上都成立**，编译器靠"`printf`
登记成类型了吗"来判。这一批用一格 `(prefer 1)` 偏表达式挡着（偏声明量出来会给错的树），
所以基线是对的；但"另一棵树一直在那儿"这件事，只有可删除测试能把它**印出来并数清**。
`assign-op -> "="` 那三处同理（`acc = acc + i;` 也能读成"声明 `acc`，初始化式是 `acc+i`"）。
...
3. **驱动器缺一格"回问"机制（还欠，但比记账那天小了一圈）。** 原话是"cpp 判不了'声明还是
   表达式'、判不了 `<`/`>`；nim 的 `optInd` 要问'这一格的列号'"。后来一条一条量下来：
   nim 那处**树上分得开**（`T(x: 1)` 与 `f(a = 3)` 不同形，见 §A.6 那一段），map 那一族也
   不必回问（造 map 的那一步自带标记）。cpp 的三类歧义里两类是**语法自己写松了**，
   收紧就没了。**真剩下的只有一类**：`T * x;` / `f(x);` 那种"名字登记成类型了吗"。
   它的大小现在也是数出来的 —— 语法层的可删除测试在 cpp 上报"prefer 定的例子 2 份、
   换了另一棵树 5 处"（§11.1 末尾），那 5 处就是这笔账的账面。
...
（`fromtree.js` 的 `mapNames`，十几行）。那笔账剩下的欠款人只有**一个**：
cpp 的"声明还是表达式" —— 只剩它是真的要回问（nim 那处量出来是树上分得开的，见 §A.6 那一段），
而它欠多少也数出来了：语法层的可删除测试在 cpp 上报"换了另一棵树 5 处"（§11.1 末尾）。
