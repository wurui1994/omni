...
- **多值那两格的 `carry` 答案量出来了**：wasm 的函数只有一格结果，所以多值落成
  ...
  `examples/multi.*` 仍跳过，理由换成了更准的一句：**打印一格多值要运行期长度 + 拼串**。
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  （"打印一格多值要运行期长度 + 拼串" / "嵌套的函数（闭包）还没接" / "墙在 OIR 不在 wasm"）。
  跳过不是失败 —— 那正是 §6 第 2 条要的样子：待办是算出来的，不是文档里许的愿。
  现在的总账：**209 passed / 0 failed / 11 skipped**（语言例子 51 + 手搭图 4 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 55/55 · sx 55/55 · js 55/55 · wat 44/55（缺口 1 格）`。
...
**还在跳的 11 格，按堆分（数是跑出来的，`node tests/graph/run.js --gaps`）：**

1. **`conv` 的 `float`，5 格**（go / vlang / nim / mojo / freebasic）—— 最大的一堆。
   要的不是一格节点，是**两种数值类型**：f64 的值、f64 的算术、i64↔f64 的转换。
   前端那侧 `f64` 已经认（`OIR_TYPE` 里就有），所以墙在后端"值一律 i64"这条约定上。
2. **打印一格多值，2 格**（lua / go 的 `multi`）—— 要"运行期长度 + 拼串"。
   拼串的机器现在有一半了（`print_str` 那格宿主面 + data 段布局），
   缺的是**运行期造串**：把几个 i64 印成十进制字节写进内存。
3. **闭包，2 格**（chez 的 `basics` 与 `index`：嵌套 `define` / 函数当值）——
   要先定"环境怎么排"（线性内存里的一块 + 一格函数下标），与记录同一台机器。
4. **变参的 prim，1 格**（sbcl 的 `(+ a b c)`）—— 接法是折成二元链，
   但得先答一问：**prim 的 arity 是附属还是骨架**（五栏里它现在不占格）。
5. **既装串又装数的那格量，1 格**（awk 的 `tag`）—— awk 没有声明，所以这是**真的没类型**。
