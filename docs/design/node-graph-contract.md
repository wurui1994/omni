...
- 矩阵里因此第一次有了 **skip** 这一档：**每一格都带理由**
  ...
  现在的总账：**259 passed / 0 failed / 1 skipped**（语言例子 60 + 手搭图 5 × 后端 4），
  每条腿末尾还印一行覆盖：`interp 65/65 · sx 65/65 · js 65/65 · wat 64/65`。
...
现状：**41 样 × 65 份例子，41 passed / 0 failed**（节点 27 格 + 附属 14 格）。
数出来的半径（删了它，几份例子要它）：
