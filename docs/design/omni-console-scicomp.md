...
* **阶段 1 —— 会话 API**：`/api/repl`（`{ session, line }` -> `{ out, err, vars,图 }`）。
  会话 = 热工人池里**钉住的一格工人**（现在是每请求一格）。
  最小例子：`x = 10` / `x * x` -> `100`，与 `tests/repl/session.in` 逐字节一致。
  判据：tests/serve 加一格"同一会话里第二行看得见第一行的变量"。
* **阶段 2 —— 版面**：控制台那一套（命令行 + 变量区 + 绘图区 + 编辑区），
  与展示 / IDE 同一条 `data-mode` 机制。变量区的格式化照 `tos` 那一套的规矩来。
  判据：`natCompare` 那一类的纯函数判据（格式化是纯函数，能判）。
