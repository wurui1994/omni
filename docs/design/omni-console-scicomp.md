...
## 4.8 阶段 8 —— 特殊函数（L3d，**已落地**：`src/lib/spec.omni`）

手册第一个例子用的就是 `sf.gamma`，所以这一层"可缺"但很显眼。它那边 `sf.lua` 同样是
模板生成的一排绑定（`templates/sf-defs.lua.in`），算法全在 GSL 里。

**一条主线，别的都从它推**：`sfLngamma`（Lanczos，g=7 九个系数）+ 不完全 gamma 的两个
展开（级数 `gser` / 连分式 `gcf`，修正的 Lentz）。于是 `sfGamma` = exp(lngamma) 加反射公式、
`sfBeta` 三个 lngamma、**`sfErf` = P(1/2, x²)**（Numerical Recipes 那一招 —— 少写一份
erf 的近似式，也就少一份"精度只到 1e-7"的账）、`sfErfc` 大 x 上直接问 Q(1/2, x²)。
另外公开 `sfGammaP` / `sfGammaQ` / `sfFact`。

**判据当场抓到一个真 bug**（这一格值得记）：`sfGammaQ` 最初写成 `1 - sfGammaP(a, x)`。
大 x 上 P 已经是 0.999…、而 Q 只有 1e-12 那么大 —— 减完只剩四五位有效数字：
`erfc(5)` 印成 `1.53744e-12`，标准表是 `1.53746e-12`。改成两条腿各自直接给自己那一格
（`x < a+1` 走级数的补、否则连分式本身）之后对上了。**这一格是"照表逐个比"抓出来的，
两条腿的差分看不见它**（两条腿会一起错）。

判据 `tests/cases/32_spec.omni`（四条腿 + 快照）：每一格都有可查的参考值 ——
Γ(1/2)=√π、Γ(-1/2)=-2√π、lnΓ(100)=359.1342054、B(1/2,1/2)=π、erf(1)=0.8427008、
erfc(5)=1.53746e-12、P(1,1)=1-1/e、Q(3,2)=5/e²，另加三条恒等式（Γ(n)=(n-1)!、
erf+erfc=1、P+Q=1）与"级数/连分式接缝两侧接得上"。

## 4.9 阶段 9 —— 常微分方程（L3b 的 ode，**已落地**：`num` 那两格）

gsl-shell 那边这一层**也是纯 Lua 重写**（`templates/{rk4,rkf45,rk8pd}.lua.in` +
`ode-defs.lua.in`），不是 GSL 绑定。

* `numOdeRk4(f, t0, y0, t1, steps)` —— 定步长四阶。答案**只由 steps 定**，
  所以它是判据里最好用的那一格。
* `numOde(f, t0, y0, t1, h0, eps)` —— 自适应 RKF45。**步长那条律照抄它的 `hadjust`**
  （`ode-defs.lua.in:66-79`）：安全系数 0.9、误差超 1.1 倍容差按 `0.9/rmax^(1/5)` 缩
  （下限 0.2 倍）、低于 0.5 倍按 `1/(order+1)` 的指数放（1~5 倍）。抄它是因为那几个
  常数是调过的。
* 方程写成一阶系统 `f(t, y) -> y'`（`y` 是一串数）。对外**先只给"从 t0 积到 t1"那一句**；
  它那边是个状态机（`s:init` 然后 `for t,y in s:evolve(t1,tsmp)`），要轨迹自己分段调。
* 判据 `tests/cases/33_ode.omni`（四条腿 + 快照）：`y'=y` 到 t=1 是 e、`y'=t` 到 2 是 2
  （RK4 对它精确）、谐振子半圈是 (-1,0) 且 `y₁²+y₂²≡1`（**守恒量比某点的值更能说明没跑偏**）、
  四阶那一条（步数翻倍误差掉 10 倍以上）、容差收紧答案更近。
  1e-12 那一档的残渣**不照印**（那是赌两条腿 libm 的最后一位），印截过位的值与判断。

## 4.10 海龟绘图（`src/lib/turtle.omni`）—— 不在 gsl-shell 那条线上

这一格是**首页要的**（"艺术绘图例子、类似 python turtle 的 logo 那一套"），
不属于 gsl-shell 的依赖层次，所以单独记一段。

名字照 python 的 `turtle`（`forward`/`back`/`left`/`right`/`penup`/`pendown`/`goto`/
`color`/`width`），logo 的短名（`fd`/`bk`/`lt`/`rt`）当别名。与 `plot.omni` 同一条纪律：
**出一份 SVG 文本、不做窗口**，于是 IDE 的预览栏、控制台的绘图栏、首页的卡片是同一张图。

两处刻意的决定：
* **画布按画出来的范围定**（自动 viewBox + 一圈边距）。递归图形的尺度是算出来的
  （Koch 第 4 层与第 1 层差好几倍），写死画布只会裁掉一半。
* **换色 / 换宽要断开当前那一段**（各自成一条 `<path>`）—— 不断的话颜色会追溯到整段。

判据 `tests/cases/34_turtle.omni`（四条腿 + 快照）：正方形回到原点、Koch 的包围盒是
`0 -0 300 86.6`（宽 = 第 0 阶的长度、高 = 300·√3/6）、换色换宽真的出现在输出里。
例子：`ext/omni/examples/koch.omni` 与 `tree.omni`，两格都上了首页（拿 rsvg 渲出来看过）。

**顺手一格语言事实**：函数体看得见文件级的**标量**，看不见文件级的**数组**
（`list<string> PALETTE` 在函数里报 `undefined variable`）—— 所以那棵树的配色写成了函数。
另一格：类里的方法**不能写 `private`**（那个修饰符只给顶层函数）。

* **往后**：L5 等值线 / 三维、L6 数据表（gdt）—— 按需要，一格一格来。
  L3c（随机）与 L3d（特殊函数）已经在上面落地了。

**先不碰的**：`eigen` / `vegas` / `bspline` / `gdt` / `expr-*` —— 上面那一列写清了它们可缺。
（`eigen` 现在的挡路石不再是复数，而是"要不要写一份 QR / Jacobi" —— 那是另一天的活。）
