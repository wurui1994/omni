...
* **实验室模式**（任务 #114–#118，已落地）：语言实验室 —— 语法编辑器 + 实时解析 +
  AST 可视化 + 冲突报告 + 管线探针。`readGrammar` / `buildTable` / `glrParse`
  全在浏览器里跑（它们是纯函数，不碰 node API）。设计文档在 `docs/design/omni-lab.md`。
...
跑完一趟之后**哪一栏亮由 stdout 说**，与是哪门语言无关（从前这一格只问 asy）：

* `%!PS` 起头 -> EPS，页面翻成 SVG；
* 输出里有 `<svg …</svg>` 整块 -> 原样挂（一趟印好几张就竖着摆好几张）。
  **图不必在开头**：`tests/cases/27_plot.omni` 印的是"字节数 + 一整份 SVG + 字节数"
  （它是判据，顺带对账），按开头判就成了"没有图"。抠块那一格是 `render.js` 的
  `drawBlocks`，只认开头的那一格是 `drawKindOf`（控制台判"这一句的值是不是一张图"用它）。
* 两样都不是 -> 不亮那一栏（`tests/asy/cases` 底下一百多份算术例子就是这一族）。

于是 `std/plot.omni` 的 `show()`（只印一份 SVG）在 IDE 里与在控制台里看到的是同一张图。
**从首页点一格卡片进 IDE 会自动跑一趟**（asy / svg 那两类）—— 刚刚在首页看着那张图，
进来只剩一屏源码是最奇怪的一下。
