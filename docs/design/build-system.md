...
### 头怎么分：**类型一份、接口各家一份**（读了 emit.js 之后定的）

`.h` 不是"把所有东西塞一份"，也不能"每家各塞一份全量"。按 C 的真实约束分三类：

- `omni_types.h` —— **所有聚合类型的定义**，按"按值嵌套"拓扑排序（`sortAggregates` 已经在做）。
  为什么必须共用一份：C 里按值嵌套要求被嵌套者是完整类型，而这条排序**跨单元** ——
  A 的结构体字段里放着 B 的结构体时，两家的定义得在同一张序里。这不是"公用头"那种偷懒：
  它只装类型，改一个类型在任何 C 工程里都会让用到它的都重编，那是语言的语义、不是我们的债。
- `<单元>.h` —— **这一家的接口**：它的 `extern` 全局 + 它的函数原型（`protoLines` 已经按
  `f.file` 分得出来）。guard + `#include "omni_types.h"`。
- `omni_gen.h` / `omni_gen.c` —— 字面量池、容器实例化、成员派发器、JS 那一族模板：
  "名字由内容定"的那些，落成内容定址的一份（与 JS 那侧的 `gen_<哈希>` 同一条规矩）。

`<单元>.c` = `#include "<自己>.h"` + 它引到的那几家的 `.h` + 它的全局定义 + 它的函数体。

于是三种改动的代价是**标准 C 的代价**，一格不多：
  改函数体   -> 只有它那一份 `.c` 变 -> 一格 `.o` 重编
  改签名     -> 它的 `.h` 变 -> 依赖它的那几家重编
  改类型     -> `omni_types.h` 变 -> 全体重编

**实现上怎么落**：不要去切 `this.out`（那是"发完再切"，把顺序耦合进来了），而是加一个
`headers()`，**从模型直接生成**：类型用 `sortAggregates()` 的序、原型用 `proto(f)`、
全局用 `cTypeName(g.type)`，每一样都已经有现成的方法。`units()` 那四大段里只留
"函数体 + 这一家的全局定义"。
