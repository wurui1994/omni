...
`.omni-cache/asy-mods` 现在一个平目录 1131 个文件、七种后缀（415 `.js`、218 `.sx`、
218 `.stamp`、217 `.dep`、各 21 `.wk`/`.sec`/`.aif`）。除了 `.stamp`，还有两格是白留的：

- **`.dep`（217 份）没有意义**：里头记的是这个单元的 key 与它需要哪几个单元 —— 那正是
  隐式图，**从源码里读一遍就有**。它存在只是因为"跳过"那个回调要在重新解析之前先知道依赖；
  先算 `staleUnits` 再跑就不需要它。
- **`.wk`（21 份）没有意义**：那是 `omni_weak` 那种"内容由整个程序决定"的单元，
  代码里本来就写着**不复用**（键以 `t` 开头的一律重编）。不复用的东西存下来只是占地方。

于是搬迁只剩三步，每步自己能验：

1. `.stamp` → `modcache` 的键 + 一份 `index.log`；删掉 `inpSame`/`stampSame`/`inpOk`
   与 218 个 `.stamp`，同时不再写 `.dep` 与 `.wk`。
2. 产物落 `asy-mods/<配置键>/<name>.js`，文件名里不再塞哈希后缀。
3. `asyModsSkip` 那个回调式跳过换成"先用 `staleUnits` 算出要编哪些，再一趟编完"。

前两步不碰 asy 前端一行。这条轴全量 10 分钟，所以每步的便宜判据是"挑三五个例子冷热各跑
一趟，比 EPS 与重编计数"，全量只在最后跑一次。
