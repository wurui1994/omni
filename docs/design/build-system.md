...
## 10. 第一个要搬过来的真实例子：asy 那条"编到 JS 的模块缓存"

`.omni-cache/asy-mods` 是这棵树里**自己长出来的第二套构建系统**，它正好把前面九节的每条
原则都违反一次，所以拿它当第一个搬迁对象。量出来的现状（2026-09-21）：

- **一个平目录 1131 个文件、七种后缀**：415 `.js`、218 `.sx`、218 `.stamp`、217 `.dep`、
  各 21 个 `.wk` / `.sec` / `.aif`。`.js` 比 `.stamp` 多出 197 份 —— 那是旧世代的产物，
  没人回收，也没人知道它们还对不对。
- **两套键叠在一起**：文件名里已经有一格配置哈希（`01-arith__40bea4bf.js`），
  旁边还有一份 `.stamp` 记 `编译器|自己的源文件|依赖们|include们|main标记`，
  每格写成 `路径:改动时间:字节数:h内容哈希`。
- **自己写的比较器**：`inpSame` / `stampSame` / `inpOk`（`src/core/cli.js:1740` 一带）——
  "除了 mtime 与长度之外都一样就算命中"。那正是脏判定，只不过是手写的第二份。
- **没有 GC**，也不在 `omni cache` 的账里（它是"缓存"，但旧世代永不退休）。

搬过来之后这些全是引擎里现成的东西：每个单元一条边（输入 = 它的源文件 + 依赖 +
`include` 摊进来的那几个，输出 = `<name>.js`），命令哈希里掺工具指纹，判定走
`recomputeDirty`，记录走一份 `.omni_log` 而不是 218 个 `.stamp`。

**搬迁的三个缝**（按这个次序，每一步自己能验）：

1. **`.stamp` → 一格内容键 + 一份索引**。键 = `hash(工具指纹 + 自己内容哈希 +
   依赖内容哈希… + include 内容哈希… + main 标记)`，写进 `asy-mods/index.log`（`BuildLog`
   那个格式）。`inpSame`/`stampSame`/`inpOk` 与 218 个 `.stamp` 一起删掉。
   *判据*：同一批例子冷跑一趟、热跑一趟，`kept` 的计数与现在相同；EPS 逐字节不变。
2. **产物按配置分目录**：`asy-mods/<配置哈希>/<name>.js`，名字里不再塞哈希后缀。
   于是"旧世代"是一整格目录，`omni cache gc` 认得它。
   *判据*：两种配置（带/不带 `ASYMPTOTE_DIR`）交替跑，互不相冲，各自热跑都是全命中。
3. **真正接进图**：`asy.unitTexts` 那个"回调式跳过"（`asyModsSkip`）换成先造图再跑 ——
   动态依赖走 `.omni_deps` 那条路（§5 的 `deps`），于是 `include` 那一格不再需要前端
   顺手告诉我们。
   *判据*：`node tests/asy/run.js` 全绿且 EPS 逐字节不变；`-t dirty` 能说出每个单元为什么重编。

第 1 与第 2 步不动 asy 前端一行代码，第 3 步才碰。**这条轴最慢**（全量 10 分钟），
所以每一步的便宜判据都是"挑三五个例子冷热各跑一趟比 EPS 与计数"，全量只在最后跑一次。
