...
### 更正：`.dep` / `.wk` / `.aif` **不是**白留的（2026-09-21 读了消费方之后改）

这一节先前写着"`.dep` 与 `.wk` 没有意义、搬迁时直接扔"。**那句话是错的**，它是照
`.omni-cache/asy-mods` 的文件清单推出来的，没读消费方。读完之后：

- **`.dep`** 是"复用一份产物时，它引到的那几份也得跟着进来"（`frontend-asy/link.js:109`）。
  证物写在那儿：`plain_scaling.asy:204` 的 `from simplex2 access problem;` 在**函数体里**，
  而 plain_bounds 这一趟不降正文，于是 simplex2 这个单元根本不存在 —— cli 顺着 `.dep`
  把这些"只剩产物"的模块一并交进来。丢了它，reuse 就会缺模块。
- **`.wk`** 是那份产物引到的 weak 项的正文，**闭包到不动点**才够
  （`link.js:312`：`.wk` 少一条，下一趟复用它时就是"未声明"）。它不是"不复用所以白存"——
  恰恰是**被复用的那些**需要它。
- **`.aif`** 是库的**接口索引**：`lang/asy.js:350` 那句"产物齐了的库：连源码都不读，
  声明从 `.aif` 认"就是它的用途（`iface.js:144` 还量过：全存是 61MB，所以是选择性存的）。

换句话说，这四个后缀里只有 **`.stamp` 是真正可以删的**（它是手写的第二份脏判定，
`modcache` 的键 + 索引替掉它）；另外三个是**模块的编译产物接口** —— 与 Rust 的 `.rmeta`、
Go 的 export data、TS 的 `.d.ts` 是同一类东西，删了就等于每次都要读源码。

所以搬迁的正确形状是**合并而不是删除**：

1. `.stamp` → `modcache` 的键 + 一份 `index.log`（删掉 `inpSame`/`stampSame`/`inpOk`）。
2. `.dep` + `.wk` + `.sec` + `.aif` → **一份 `<name>.iface`**（一个文件、一种格式，
   里头四段：needs / weak / sigs / iface）。四种 ad-hoc 格式 + 4×218 个文件变成 1 种 + 218 个。
3. 落 `asy-mods/<配置键>/`，旧世代是一整格目录，`omni cache gc` 认得。
4. `asyModsSkip` 的回调式跳过换成"先 `staleUnits` 算要编哪些，再一趟编完"。

前三步不碰 asy 前端一行（只动 cli 这侧的读写），第 4 步才碰。每步的便宜判据：挑三五个
例子冷热各跑一趟，比 EPS 与重编计数；全量（10 分钟）只在最后跑一次。
