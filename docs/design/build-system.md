...
### asy 那四个后缀：等模块（ADR-0042）落地，不是单独去掉

先前这一节两次说错过同一件事的两头：第一次说 `.dep`/`.wk` 白留（照文件清单推的，没读消费方），
第二次说它们该留（读了消费方，但只看到"现状需要它"）。真正的账在更上一层：

- `.wk` 的存在理由是 **weak**，而 weak 的存在理由是**核心方言没有模块** ——
  共享的东西只能"谁用谁发一份"，靠链接期重复取一，于是复用产物时那些正文得有人存。
- `.dep` 是"复用时要把哪几份带进来"——**有 import 之后那就是模块图本身**。
- `.sec` / `.aif` 是模块的**导出接口**（与 TS 的 `.d.ts`、Rust 的 `.rmeta` 同类），
  该属于模块、落成 `.sx` 的一段，而不是缓存目录里的第三、第四种 ad-hoc 格式。

所以次序是：**先给 `.sx` 加模块**（ADR-0042），weak 退场，`.wk`/`.dep` 自然消失，
`.sec`/`.aif` 合成一份模块接口；缓存这一层只管"这份接口还新不新"（`modcache` 的键 + 索引）。
在那之前只动 `.stamp`（它是手写的第二份脏判定，与模块无关）。
