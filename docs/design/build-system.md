...
量出来的下一堵墙（换个程序跑，库那几格 `.o` 一个都没命中）：**所有模块都 `#include
"omni_gen.h"`**，而 gen 是"这个程序用到哪些模板 + 哪些字面量"——它随程序变，于是每个
模块的编译输入都随程序变。所以下一刀是这三件，缺一件都不成立：

1. **gen 拆细**：字面量池、每个容器实例化、每族 JS 模板各自一份内容定址的头
   （`gen/<哈希>.h`），模块只 include 它真用到的那几格。
2. **每个模块独立降级**：C 腿走 `asy.unitTexts`（JS 腿用的那一份，每个源文件一段 `.sx`），
   每段自己降到 OIR、自己发 `.c`/`.h`；别家只给**接口**（`.d.sx` 的 `decl` -> C 声明），
   与 JS 腿共用同一套 `UnitIndex` / `declRead`。
3. **mangled 名不许带"这一趟去重编的号"**（`__2` 那截）：它随程序变，同一份源文件在两个
   程序里出来的符号就不同。

*判据*（这三件做完才成立）：同一份源文件在两个程序里出来的 `.c` **逐字节相同**，
于是换个程序跑，库那几格 `.o` 全命中。`(unit "…")` 那格标记到那时就该删掉 ——
