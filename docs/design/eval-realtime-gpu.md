...
**还没记进 `REF_WRONG`**：`town textured` 我们自己这一侧还有另一格问题（见下），
得先把自己那一格查清楚，不能拿"参考错"把自己的 bug 盖住。

### 26.1 `town textured` 那一格查到哪儿了（我们一格都没画出来）

四步二分，结论是**语言那一侧的矩阵链**，不是着色器、不是批：

1. `--gfx null`（录制设备）数得出 **3528 格批**（14×14 栋 × 3 层 × 6 面），几何是发出去的；
2. 把片元着色器换成**常白**：参考铺满 74% 的屏幕，我们**一格都没有** ⇒ 与 `p` 那格
   varying（§26）无关；
3. 把 `@v`/`@f` 两段整个删掉（于是走固定管线那一档，顶点在语言侧就变成裁剪空间、
   `u_mvp` 是单位）—— **还是全黑** ⇒ 与"物体坐标 + `batchmvp`"那条路也无关；
4. 两个顺手排掉的嫌疑：`glrotate(45,1,1,0)`（**非轴对齐**那一档）单独拿出来与参考
   RMSE 0.8%（方向一致）；楼高那一句 `(j*71+i*109+k*153) % 3` 换成常数也照旧全黑。

所以下一步是照着 `glrotate(20,1,1,0)` → `glrotate(90,0,1,0)` → `gltranslate(52,-20,0)`
这条前缀逐步量我们的 `gl_mv`（那两个 `magick -format %[fx:mean]` **会把 alpha 也算进去**，
所以"mean=0.25 max=1"其实是"全黑 + alpha=1" —— 别拿它当"有东西画出来了"）。
