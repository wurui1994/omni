...
### 28.10 下一格：`gllinewidth` 现在是 no-op（`tree` 那 8.27 的来源）

`tree.pss` 画两趟：`GL_QUADS` 那趟抓进槽 0、`GL_LINE_STRIP` 那趟**先 `gllinewidth(2)`**
再抓进槽 1，最后拿着色器 1 把两张合起来。我们这一侧 `gllinewidth/1` 在名字表里直接是
`gl_nop1`（`ext/polydraw/gl-rt.js:226`）—— 线那一趟只有 1 像素宽，合成出来自然不一样：
差集是 38 个连通块、最大的三块 1810 / 921 / 327 格（**不是边缘薄片那一类**），
差最大的一格我们 `(4,0,2)`、参考 `(255,0,176)`。

这一格不能照抄 `glLineWidth`：core profile 里它的上限就是 1。按"只有一个模型"的口径，
