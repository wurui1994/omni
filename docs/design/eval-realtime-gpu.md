...
### 31.3 下一刀的靶子：这条腿的原生码根本没过优化管线

`omni build x.pss` 印出来的是 `via self -O0` —— 自带的 C 前端，而公共 MIR 优化管线
（ADR-0039）缺省**不跑**（`OMNI_MIR_OPT=0`）。也就是说 `.pss` 那一整轴的性能数字
全是未优化码量出来的，`s_eval$frame` 自用 92.7% 正是这一点的样子。
下一步先量 `OMNI_MIR_OPT=1..3` 在这几份上的收益与逐像素中性，再决定要不要给
`tests/eval/perf.js` 定一档默认值（memory 里那两个规模闸要一起看）。
