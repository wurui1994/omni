...
`--gfx null` 那一档只量语言那一半（设备认所有名字、只记账）。
    ...
    2026-09-26 **本机 OpenGL** 那一趟（`--leg c --gfx gl`，228 份）：
      148 / 228   （141 + 抓屏那七份，见 §22.3）
...
### 22.3 一个洞：**C 腿上这两格从来没接过**（2026-09-26 补）

设备那两格早就写好了，但只有 **JS 宿主**（`host/gfx-cpu.js` 的 `glcapture/1`）转过去；
`runtime/omni_fmt.c`（C 腿，也就是 `--backend c` + `--gfx gl` 那条路）里只有
`glcapture` 零参与四参的**空桩**，一参那一格压根不在表上 —— 而语言那一侧发的正是
一参那两格（边长 / 槽）。于是 C 腿上这七份直接报「这格设备上没有 'glcapture'
（1 个实参）」。补法与别的设备函数逐句相同：`gfx_gl_cap_fn` 一格 typedef、
`dlsym("omni_ev_gl_capbegin"/"_capend")`、`g_gl.on` 那一段里两句转发，
外加 CPU 备选那一侧的 `glcapture` 一参收下不管。

判据（`tests/eval/scan.js --leg c --gfx gl`）：这七份（`25_offscreen_capture` /
`texture` / `clock` / `disco blur shader +blur` / `funky` / `gears` / `tree`）
**全部从红转绿**，整份语料 **141 → 148 / 228**。

**这个洞的形状值得记住**：两个宿主（JS 与 C）各有一张自己的名字表，往设备加能力时
**只加一半**，另一半要等到有人拿那条腿跑才会发现。`--gfx gl` 上一直是 88/228 的
那个账就是这么来的 —— 不是设备缺能力，是 C 腿没接。
...
## 33. 语料里那 6 份 `timeout > 25s` 逐个查过了（2026-09-26）

口径：`node tests/eval/scan.js --leg c --gfx gl --timeout 25`（那张 177/228 的账就是它跑的，
落在 `.omni-cache/evalscan/c-gl.json`）。**6 份全是"某个循环出不来"，一份都不是"算得慢"** ——
把画布缩到 32×24 仍然不回来，所以不是像素量的问题。分成四类：

### 33.1 两层圆没夹到画布里（`conetest`：>45s -> **2ms/帧**）

采样 100% 落在 `gfx_cone -> gfx_disc -> gfx_px`。三维那一档的 `drawcone(x,y,z,r,…)`
两端各投影一次，`z` 一接近近平面（`g3_xf` 只挡 `z <= ZNEAR`）投影半径就能炸到上百万 ——
而填充圆那两层循环是 `for dy in -ri..ri / for x in -dx..dx`，**没夹**，于是转 `r²` 趟，
每一趟都在 `gfx_px` 里被"越界丢掉"。`conetest.kc` 一帧 256 个 `drawcone`，稳超时。

两刀，都是**恒等变形**（画出来的像素一个不差）：

1. `disc` 两层循环夹到 `0..w-1 × 0..h-1`：圆心先取整（对整数 `x`，`round(cx)+x` 与
   `round(cx+x)` 相等），整圆不沾画布就直接回；
2. `cone` 沿线铺圆那一趟：
   * `i` 只走**沾画布的那一段** —— 把线段按 `±(rmax+2)` 的余量做一次 1D 参数裁剪
     （`tlo`/`thi`），扔掉的那些圆一个像素都画不出来；
   * 某一格圆一旦**盖满整块画布**就停 —— 整条 cone 是同一个 `gfx_col`，后面那些圆只会
     把同样的颜色写回同一片格子。"盖满"的判据是**到画布最远那个角的距离 ≤ ri**，
     那样每一行的 `floor(sqrt(ri²-dy²))` 都 ≥ 该行要的半弦长，所以这是准的、不是估的。

**三份实现都要改**（一件事三处落点，别只改一处）：`ext/polydraw/gfx-rt.js`（`--gfx ir`
那一档生成出来的光栅器）、`src/core/host/gfx-cpu.js`（js/解释器腿的 CPU 备选）、
`src/runtime/omni_fmt.c`（C 腿的 CPU 备选 —— **`--gfx gl` 上 2D 图元也走它**，
GL 那层只画顶点批，所以扫描时踩的正是这一份）。C 那份还多一格：半径先夹到 2^30，
不然 `gfx_rnd(1e300)` 是 UB、`ri*ri` 溢出 int64。

判据：`tests/lower/run.js --only evaldraw` **35/35**（含 js/c/interp 逐字节相同）、
`correct.js --only drawsph,drawcone2,gspiral,creepers,wire_sphere` 的六个数**一个没动**
（`drawcone2` 照旧 10.13 / 2409 格红、`drawcone2_asm` 照旧 7.77 / 247 格）、
`conetest` 的 js 腿与 c 腿出图 md5 相同。

### 33.2 `klock` 钉死在帧号上，脚本自己的帧限速出不来（`magpong2` 等：>25s -> **1s**）

`magpong2.kc:38` 是这么限速的：

```
otim = tim; do { tim = klock(); dtim = tim-otim; } while (dtim < 1/60);
```

而 `render` 模式的 `klock()` 是**确定性时钟**（帧号/60 —— 离屏出图要能逐字节比，
墙上时间在那儿是噪声）。同一帧里它永远回同一个数 ⇒ `dtim` 恒为 0 ⇒ 死循环。

新规矩（两个宿主表**同时**改：`gfx-cpu.js` 的 `klockSec()` 与 `omni_fmt.c` 的
`gfx_klock_sec()`）：**一帧里头一次仍回钉死那个值，之后每一次再往前一帧的量（1/60）**，
并且保证单调。于是

* "一帧读一次 `klock`"的脚本（绝大多数）**逐字节不变** —— 头一次那个值没动；
* 上面那种限速循环**转一趟就出来**（第二次读到 `t0 + 1/60`，`dtim` 正好不小于 `1/60`）；
* 仍然是**确定性的**：一个字都不读墙上时间，所以三条腿照旧逐字节相同
  （`magpong2` 的 js 腿与 c 腿出图 md5 相同）。

这一刀顺带治好 `playnote_test` 与 `space_struct`。

### 33.3 剩下两份：都在我们之外

* **`box.kc`** —— 它是"随机摆 6~12 个 ≥32×32 的方块、摆不下就重摇"（`do {…} while(!clear(…))`）。
  扫描用的画布是 96×72，算出来的窗口只有 72×64 ⇒ **压根摆不下**，那个拒绝采样永远不成功。
  同一份脚本在 **640×480 上 0.73s 跑完**。这是"判据的画布比脚本的下限还小"，不是我们的洞。
  （所以也别去改脚本或放宽 `clear`。）
* **`lab3d.kc`** —— 光线步进的出口是 `while (pic("doubcube.png",ix,iy) != 16777215)`，
  而 `pic` 在两个 CPU 备选里都是"收下回 0"（纹理采样那一族在任务 #26），
  **并且 `doubcube.png` 压根不在语料里**（整棵树只有 `glow.kc_f30.png`）。
  缺件 + 缺能力，两头都不在我们这一侧；等 #26 把 `pic` 接上再看。

### 33.4 这一轮之后的账

重扫同一条命令：**177/228 -> 181/228 ok，`timeout` 桶 6 -> 2**（剩下的就是 §33.3 那两份）。
别的桶**一个数都没动**（`gl_settexf3` 5 / `readtouch` 3 / `fputc` 2 / `aget` 5 / `goto` 往前跳 3 …），
所以这一轮是纯增。

### 33.5 查这一类的时候省下的两步

* **先把画布缩掉**（32×24）再判"是慢还是出不来"：一秒钟就分清了这两类，
  不用等 25s 的看门狗，也不用上采样器。
* **认清扫描是哪一档**：那张 177/228 的账是 `--leg c --gfx gl`（`c-gl.json`），
  不是默认的 `--leg js --gfx null`（`js.json`，9 月 24 日的旧账、17 份超时）。
  同一份脚本在两档上的行为能完全不同（`conetest` 在 `--gfx null` 上压根不进光栅器）——
  **判退步前先确认自己跑的是同一档**。



## 34. **graphing modes**：主函数的形参表就是执行模型（2026-09-26）

用户报的是"`.kc` 生成的图压根不正常，只是纯白，没有任何绘制"（`demos/ceilflor.kc`）。
根因不在设备：那份脚本的主函数是 **`(x,y,t)`** —— `evaldraw.txt:1237` 那张表里的
**2D graphing mode**，宿主要**每个像素调一次**，回值过调色板。而我们从前只认 `()`
那一档，有形参就"当成零值的局部量"跑一趟 ⇒ 一个像素都不画，**退出码还是 0**
（扫描那张表把它记成 ok —— 正是记忆里"不许靠两边都不画白拿分"那条纪律说的洞）。

### 34.1 那张表（照 `evaldraw.txt:1237` 抄）

    ()                  每帧调一次（脚本自己画）           ← 早就有
    (x)  (x,t)          1D 画曲线                          ← **当场报**（还没接）
    (x,t,te) (a[16])    1D 声音（乐器）                    ← 语言这一半通、没有音频设备
    (x,y)               2D：每像素一次，回值过调色板       ← **这一刀接了**
    (x,y,t)             2D + 时间（秒）                    ← **这一刀接了**
    (x,y,&r,&g,&b)      2D：脚本自己给颜色（0..255）       ← **这一刀接了**
    (x,y,t,&r,&g,&b)    上面两档合起来                     ← **这一刀接了**
    (x,y,z,&r,&g,&b)    3D 体素（21 份语料）               ← **这一刀接了**（§34.5）
    (x,y,z,t,&r,&g,&b)  3D 体素 + 时间                     ← **这一刀接了**（§34.5）

语料里 37 份是 graphing modes（2D 6 / 3D 21 / 1D 10）—— 也就是说这一格的洞**不是一份脚本**。

### 34.2 形状：**循环在语言这一侧**（`ext/polydraw/graph-rt.js`）

主体落成 `ev$pix(x,y[,t][,&r,&g,&b])`，帧体换成：

    frameinit 那一趟（ev_fi = 1，调一次主体）   ← evaldraw.txt:1428 的原话
    逐行 { 逐像素 { 调主体 -> 调色 -> 攒进 ev_row } 整行交给设备 }

**整行一次**（`(gfxarr "setrow" y x0 数 0 行)`）：320×240 是 **240 次**宿主调用，
不是 76800 次。三档设备都补了这一格（`host/gfx-cpu.js` / `runtime/omni_fmt.c` /
`studio/gfx-gl.js`，后者没有平面帧缓冲 ⇒ 一格像素还是一格 1×1 四边形）。

### 34.3 两处定死的口径

1. **网格** `setgrid(-4,3,4,-3)`（`evaldraw.txt:1620` 的原话，y 是负的因为作者按屏幕
   坐标想）。像素中心取样。`setgrid(x0,y0,x1,y1)` 改那四格全局量。
2. **调色板**是**明写偏差**：说明书只钉住"`<=0` 蓝、`>=1` 红、中间平滑光谱"，
   而那条曲线写在 `EVALDRAW.INI` 里 —— **语料里没有那份 INI**，evaldraw 也没有源码。
   我们按 HSV 色相 240°→0°（蓝→青→绿→黄→红）实现，端点与说明书对得上。

### 34.4 判据与账

`tests/lower` 加一格 `evaldraw+graph2d`（`ext/evaldraw/examples/graph2d.kc`）——
回值沿横向从 0 线性到 1，按**左中右五格像素**核对：那同时钉住网格与调色板。
三条腿（js/interp/c）逐字节相同，46/46。

语料这一侧：`ceilflor` / `mandel` / `gcd` / `interference` / `driftbox` 五份从"纯白"
变成真的图（`ceilflor` 第 0 帧与第 5 帧不同 ⇒ `t` 真的在走；1019 种颜色）；
`mount.kc` 还红在另一格（`enum` 算不出常量）。

### 34.5 3D 体素那两档（2026-09-26 第二刀）

`evaldraw.txt:1311` 只钉住三件事：函数在**单位立方体 (-1,-1,-1)..(1,1,1) 里的网格**上
每格调一次、回值 `> 0` 是实心 `<= 0` 是空气、颜色从 `&r,&g,&b` 拿（0..255）。
落法是**评估 + 抽面 + 画立方体**，三步全在语言这一侧（`graph-rt.js` 的
`graph3dFrameStmts`）：

    frameinit 那一趟（与 2D 同一条口径）
    N³ 次调主体 -> ev_vox[k] = 实心 ? 打包好的颜色 : -1
    镜头（gl_framebegin + gl_enable(GL_DEPTH_TEST) + gl_lookat）
    逐格：六个方向上邻居是空气（或出了网格）的那一面 -> 一个 GL_QUADS 四边形

画那一半走的是**已经有的那台立即模式状态机**（`gl-rt.js` 的 `gl_begin(7)` /
`gl_vertex3` / `gl_color3` / `gl_end`）—— 体素这一档一行新的设备代码都没加。
攒够 3000 个四边形断一次批（`gl-rt.js` 的顶点上限是 16384 格）。

**三处明写偏差**（evaldraw 没有源码，说明书只说了上面那三件事）：

1. 网格边长固定 **32**（说明书里 `voxres` 是个菜单项，语料里一份都没读它）；
2. **镜头固定**（说明书里是鼠标拖的）—— 在 `(1.5,-1.75,1.25)` 看原点、上是 `+z`；
3. 面的明暗按法向给一格常数因子（顶 1.0、底 0.45、四侧 0.55~0.9）——
   没有光照那一摊。

### 34.6 顺手挖出来的那一格：**末尾那句带分号的表达式也是返回值**

`voxes/meatball.kc` 接上之后仍然全黑，而它的最后一句是
`a > (x*x+y*y+z*z-.9);` —— **带分号**。`eval.txt:92` 只写了"末尾那句表达式是返回值，
而且不需要分号"，于是我们从前只在语法里接了不带分号那一支（`tail` 规则），
带分号的那一档落成"算了扔掉，回 0" ⇒ 满盘空气。

拿 arm64 那棵工作树的 `eval_bench`（真 `kasm87` 编出来的机器码）量了六格：

    (x) 3+x;                      -> 8     带分号，照样是返回值
    (x) y=3+x;                    -> 8     **赋值也算**，值是赋进去的那个
    (x) {3+x;}                    -> 8     末尾是块 ⇒ 看块里最后那句
    (x) y=3+x; if(x>0) y=99;      -> 0     末尾是 if / for / while ⇒ 0
    (x) 3+x; y=1;                 -> 1     只看**最后**那一句
...
### 34.8 `.kc` 的 `glBegin` 那一族走 **EvalDraw 的相机**（2026-09-26）

用户报的是"`demos/beer.kc` 依然是白的"。挖下去是三格，一格比一格底层：

1. **素材找不到**：EvalDraw 那套语料所有素材在 `<语料根>/data/` 下，而设备只"一级一级
   往上找" —— 加一格"每级试 `data/`"，并把 CLI 递的脚本目录改成**绝对路径**
   （`omni run beer.kc` 给的是 `.`，那条梯子一步都爬不上去）。
2. **几何在镜头背后**：EvalDraw 的 `glBegin/glVertex` **不是 OpenGL**
   （`evaldraw.txt:1641` 那一节）：它跟自己那套 3D 相机 ——
   `screen = (v·右 或 v·下)/(v·前)·hz + (hx 或 hy)`，**+z 是前、y 往下**。
   语料里的几何摆在 z>0，按 OpenGL 那套（-z 是前）全在镜头背后 ⇒ 一格像素都不画。
   落法：`gl_evproj(…)` 把那套相机**压成一张 4×4 摆进投影槽**（推导写在函数头注里），
   顶点路一个字都不用改（纹理/裁剪/深度/合批照旧）。
3. **顶点色是 `setcol`**：那门语言**没有 `glColor`**，而立即模式的默认顶点色是 0 ⇒ 黑。
   设备加一格 `getcol()`（三档设备各一行），`g3_glbegin` 在 `glBegin` 那一刻把当前 2D
   颜色拆成三格喂给 `gl_color3`。深度测试也在这儿开（说明书原话：**默认开**）。

判据：`tests/gl` 第七节（红的那块 z=+4 看得见 6400 格、绿的那块 z=-4 在镜头背后 0 格）
—— **23/23**；`tests/lower --only evaldraw` 46/46。

`beer.kc` 现在几何铺满了（先前是纯白/全黑），**但纹理还没贴上** —— EvalDraw 的
