...
/**
 * litnone：**定长数组 · `none` · 三段 for 的空格子**（V 独一份）—— 三样都是"同一个标签
 * 两件事"或"类型上的性质不进图"这一类。`(none)` 既是 Option 空值又是空格子，所以 `for`
 * 那一格要先滤掉它：条件落成 nil 就是"条件为假"，`for ;; {}` 会一次都不跑。
 */
export const LITNONE = ['20', '5', '1', '3'];
/**
