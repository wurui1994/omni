...
const REF_WRONG = new Map([
  ...
  ['curvybuild.pss',
    ...
    + '（见 `heightmap` 那一条）；(2) **参考在这一份上还把纹理槽认错了**：'
    + '脚本 `glbindtexture(0)` 画一组、`glbindtexture(1)` 画另一组（`:94`/`:130`），'
    + '而正本 `qglBindTex`（`polydraw.c:1464`）是 `glBindTexture(tex[i].tar, i)`——'
    + '槽号就是纹理对象号。两张图摆在一起看：**几何逐格对得上**（轮廓、那块高光的位置'
    + '都一样），可我们那一大片是**木板纹**（`min(x%32,15)*0x040302` 那一段与噪声无关，'
    + '所以板缝一定看得见）、参考那一片是**没有板缝的灰噪点**（= 地毯那张，`0` 号槽）。'
    + '我们这一侧另有一份探针（0 号槽全红 / 1 号槽全绿，两个四边形各绑一个）：'
    + '左红右绿、逐格对；参考那一趟两个四边形都是**白的**（它连纹理都没采上）'],
  ['mipmap.pss',
