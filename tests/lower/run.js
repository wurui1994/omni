...
import {
  ...
  HOIST, LITNONE, MATCH, METHOD2, OPTRES, POINTER, POSINIT, PUSH, INHERIT, OPOVER, TMPL, CTOR, VIRT, CTMPL, LAMBDA, FMT, FORMAT, POSTEST, CTOR2, METHOV, PUREVIRT, DTORCHAIN, MIXVIRT, OUTLINE, CTMPL2, FNOVL, METHOV2, CTOR3, REFPARAM, STATICMEM, BYVALUE,
} from '../lib/cases.js';
...
const FAMILIES = {
  ...
  outline: OUTLINE, ctmpl2: CTMPL2, fnovl: FNOVL, methov2: METHOV2, ctor3: CTOR3, refparam: REFPARAM, staticmem: STATICMEM, byvalue: BYVALUE,
};
const MIGRATED = {
  ...
  /* cpp：三十个家族。`defer` 在这门语言里是 `~Say()`（RAII，与 freebasic 同一手，
     出口那一半交给公共层的 `{ kind: 'scope' }`）；`printf` 只接"一格转换 + 换行"
     （见 adapter/expr.js 的 printArgs）。`inherit` 与 `opover` 两族的期望输出是
     本机 `c++ -std=c++17` 给的，不是我们自己编的。 */
  cpp: ['basics', 'byvalue', 'conv', 'ctmpl', 'ctmpl2', 'ctor', 'ctor2', 'ctor3', 'defer', 'dict', 'dtorchain', 'fmt',
    'fnovl',
