...
#         **跑起来还没验上** —— 容器里那一趟（模拟出来的 amd64 上一个 72M 的静态
#         二进制）跑了 214s 还没出 `--help` 就被砍了（exit 137）。所以第 6 笔账里
#         「整份编译器在容器里跑起来」那一条**只对 glibc 那条腿成立**；`--libc self`
#         这一头现在能说的只有「链得出来、fib 那一级跑得对」。要验它得换一台真的
#         x86_64 机器（或者先把 UND 清零之外的那几格 —— 目录/进程/时间 —— 各写一个
#         小判据分头量）。
set -euo pipefail
