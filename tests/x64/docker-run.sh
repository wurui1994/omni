...
#         （`%f %e %g`）**现在也是**（第九格）：数字从位模式摊成精确的十进制
#         （`decExpand`，基 10^9 的大整数，收位半到偶），`tests/x64/libc-float-probe.c`
#         那 155 行两条腿都**一行不差**（arm64-osx 与 Apple 的 libc、x86_64-linux 与
#         glibc，183845 字节）。改之前是 41 行不同 —— 那一版「归一化 + 逐位取整」
#         全在 double 上算，`%f` 印 1e100 只有前 25 位是真的。
...
#      h. 探子后来多了 `pipe` 与 `setjmp`/`longjmp` 两格（`SYSCALL2` 与 arm64 的
#         `SETJMP` 那两片），所以现在是 **17 行输出**，而 `system` 那一行两条腿都真的跑
#         （原先 macOS 上是 `#ifdef __APPLE__` 跳过的）。两边各量一趟，都**逐行相同**
#         （`diff` 无输出）、两边 rc=0：
#           x86_64-linux（容器）：`./sp_lin` 与 `gcc` 那份 —— `ldd` 说 statically linked，
#             181325 字节、16 节、8 段
#           arm64-osx（本机，不进容器）：与 Apple 的 libc 那份 —— 127896 字节、13 条加载命令
#         三行新的：`system(true) 状态字: 0`、`pipe 0: 写 7 读 7 「pipe ok」`、
...
#      j. `strtod` 也**正确舍入**了（第十格）：同一个大整数（`dec.c`）反着用 —— 一串
#         十进制是两个大整数的商，挪进 [2^52, 2^53) 再做一次长除法。判据比的是**位模式**
#         （`tests/x64/libc-strtod-probe.c`，31 个串 + 21 个往返）：
#           改之前  21/31 位不同、21 个往返里 8 个回不来
#                   （`1.7976931348623157e308` 读成 inf、`5e-324` 读成 0）
#           改之后  两条腿都 0/31、往返 0 个不同（x86_64-linux 那份 204261 字节）
#         踩到的一格：**收位只许收一次** —— 先收 53 位再为非规格化右移是两次舍入，
#         `2.2250738585072011e-308`（glibc 那个著名样本）会读成最小的规格化数。
#      k. libm 那一格从「手量」变成了判据（`tests/c/libc-libm.js` + `libc-libm-probe.c`，
#         338 个采样点）。顺出两个真错：
#           * `sinCore` 的 Horner 链**跳掉了 13! 与 15!**（注释还说「在噪声以下」）——
#             `sin(0.7071067811865476)` 差 2.7e-12，而同一点上 cos 只差 1.4e-16。
#           * 大参数：`nd * PIO2_HI` 到 1e300，`sin(1e300)` 直接回 **inf**。
#         补上那两项 + π/2 拆三段 + 大参数先按 2π 的 double 折一次之后：
#           arm64-osx（本机）    最大误差 **5.4e-15**（原来 5.75e-11），252/338 逐字节相同
#           x86_64-linux（容器） 同一份 math.c，同一个数：5.4e-15，249/335 相同
#         最大的那一格是 `pow(1e300, 0.375)` —— pow 是 `exp(y·log x)`，误差在 112 个
#         数量级上放大，那是这条路本身的账。大参数三角函数没有 Payne-Hanek，明记在
#         sysroot 的 README 上（判据里那一档只判「有限且在值域里」）。
#      l. 大参数那笔欠账当场还了（第十一格）：`sin`/`cos`/`tan` 在 |x| > 2^45 上走
#         **Payne-Hanek** —— x 本来就精确（`m·2^e`），π 也自己算得出来
#         （Machin：`π = 16·atan(1/5) − 4·atan(1/239)`，每项只要「大整数除小数」），
#         所以 400 位的 π 是**算出来的、不抄常数表**，算一次存着。
#         然后 `x·2/π = 2·Dx·10^399 / (π_scaled·10^fx)`，一次二进制长除法：
#         商的低两位是象限、余数取 60 位是小数部分。量到的：
