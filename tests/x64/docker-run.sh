...
#      f. 整份编译器交叉编 + **链得出来**：76164213 字节（72.6M）、16 节、8 段、
#         入口 0x5668a8（前端 1.2s + 发射 480ms + cc 13.3s）。
#         **跑起来那一趟卡住过 214s，根因不是模拟慢 —— 是我们自己那份 malloc 死循环。**
#         第一版是「整堆线性 first-fit」，而长堆时那一大截 slack 没有块头：扫到零头
#         `bsz = 0`，既不满足 `bsz >= total`、`p += bsz` 又不动，原地转圈。
#         第二版换成「32 个箱的空闲表 + 顶上切」，全是 O(1)：判据在
#         `tests/c/libc-malloc.js`（本机 0.3s 跑完 20 万块；第一版在那儿是 timeout 124）。
#         这一格记着一条教训：**判据要摆在离病根最近的那一层**。摆在容器里那一趟，
#         看到的只有「72M 的二进制半天不出 --help」，隔着模拟器与尺寸两层猜不到 malloc。
set -euo pipefail
