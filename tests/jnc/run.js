...
//      typedef、`bitflag enum`、`sizeof`（要方言的布局先认整数宽度）、`dynamic countof`
//      （要 fat 指针带的范围），加四条 jancy 自己也拒的（命名项之后不能再写位置项、
//      `double` 上的 `&=`、int 到枚举的隐式转换、`countof` 作用在指针上）。（`? :`、不换行的 printf、条件真值化曾经在
