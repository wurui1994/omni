...
//      triple、`pair[]`、复数幂、重载、模块、隐式缩放、超越函数、给切片赋值、
//      多维数组、循环条件里的 `? :`、字符串的 `reverse`/`insert`/`split`）都在这里，
//      每条都带 ASY_NOPE 前缀 —— "还没做"和"做错了"必须能一眼分开。
//   4. **strict/ 里的是 asy 自己就不收的**（后缀 `x++`、pair 上的 `<` 和 `%`、
//      `length(int[])` 那种）：我们要拒，而且装了 asy 的话真 asy 也要拒。这一节盯的是
//      "比 asy 多接受一门语言" ——那种漏洞不会让任何用例输出不同，只会让"等价"两个字变虚。
//
