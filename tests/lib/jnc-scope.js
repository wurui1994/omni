...
/* 闸门只管甲与丙：乙那一格是明账，不许拿它当红灯，也不许假装它绿。
   ...
   **导入表接上之后（整模块那一路）**：自解 94.7%，剩 3222 处。剩的都查过来源：
     - 打不开的归档只剩 **3 处**，全是 `io_abr.jncx` —— 这个扩展**不在** jancy 这份源码树里
       （`src/jnc_ext/` 底下没有 `jnc_io_abr`），那是语料不全，不是机制缺；
     - 各插件跨**模块**引用的枚举（`SerialTapProLines` 那种，它们在别的插件模块里）；
     - 完整属性声明里的字段（`31_FullPropertyDecl.jnc` 那 18 处 `m_x`）—— 属性体那一层还没配方。 */
