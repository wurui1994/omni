 ...
 * 寄存器，初值在那张表里。我们这边没有 MIDI 那一侧 ⇒ 在入口里就地开一块、按表填初值
 * （默认 `sr=44100`、`f=60` 中央 C、`v=64`）。五行分别钉住：形参真是**一整块**、
 * 那六格非零初值（`a[3]=1/sr` 要是 0，语料里"推时间"那种循环就永远不动）、
 * 草稿寄存器是 0 且写得进读得回。语料里 `insts/` 那五份靠的就是它。
 */
export const EVINST = ['frq=60,64', 'inc=0.037275327,0.000022676',
