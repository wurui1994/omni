#!/usr/bin/env node
/* 自带 libc 的**日历**那一格与平台 libc 的逐行对账。
 *
 * 同一份探子（`tests/x64/libc-time-probe.c`）编两遍：`cc` 是尺子，我们那份走
 * `--libc self`，逐行比。量的是 `gmtime_r` 的九个字段与 `strftime` 的几种格式 ——
 * 秒数掰成年月日是纯计算（civil_from_days 那条封闭公式），所以该**一行不差**。
 * 探子头上写了哪三条不比（时区、`%Z`、认不出的转换）。
 *
 * 只在 arm64 macOS 上跑，别的机器上跳过。
 *
 *   node tests/c/libc-time.js
 */
...
const SRC = join(HERE, '..', 'x64', 'libc-time-probe.c');
const SYSROOT = join(ROOT, 'src', 'sysroot', 'arm64-osx');
const WORK = join(ROOT, '.omni-cache', 'work', 'libc-time');
const REF = join(WORK, 'time-ref');
const OBJ = join(WORK, 'time.o');
const BIN = join(WORK, 'time-self');
...
if (existsSync(REF) && existsSync(BIN)) {
  ...
  for (const d of diff.slice(0, 20)) process.stdout.write(`${d}\n`);
}
