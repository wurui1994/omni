 ...
 *
 * 同一份探子（`tests/x64/libc-stdio-probe.c`）编两遍：`cc` 是尺子，我们那份走
...
const SRC = join(HERE, '..', 'x64', 'libc-stdio-probe.c');
const SYSROOT = join(ROOT, 'src', 'sysroot', 'arm64-osx');
const WORK = join(ROOT, '.omni-cache', 'work', 'libc-stdio');
const REF = join(WORK, 'stdio-ref');
const OBJ = join(WORK, 'stdio.o');
const BIN = join(WORK, 'stdio-self');
