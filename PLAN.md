...
**定位（2026-09-21 补）**：Omni 是一个**语言平台**，不是"又一门要求你迁移的语言"。
常见做法是"你变成我"；这里是"你不必变成我，我们找一层能对接的契约"。落到工程上：
借来的语言各留各的语法，统一只发生在中间那一层（核心方言 / OIR / MIR），于是它们共享
同一批优化、同一批后端、同一套工具。**远期目标是把这层契约做成形式化引擎** ——
程序的意义用可检查的形式写下来，翻译 / 优化 / 发码都是它上面可验证的变换。
现在离那一步还远，但判据是照那个方向选的：每一格都与外面一条互不相干的实现对账，
而不是自我声明。
...
**已完成（REPL 增量化 + 前端无关，2026-08-27）**：ADR-0008 第 3 节的第一条偏差消除。
- 会话状态从"源码文本列表 + 每次重放"换成三份常驻的东西，每批输入编译出来的是一份 delta：
  模块图（`newLoadState`，`std/json.omni` 只加载一次、会话根固定是模块 0、导入边累加）、
  前端会话（`CheckSession` / `CoreSession`）、运行期会话（`InterpSession`）。
- 增量落在两个地方，不是"缓存一下"：函数符号上的 `defsDone`/`bodyDone`（旧函数体不重检），
  以及**常驻顶层作用域**——检查器的 `topScope` 与解释器的顶层 `Env` 是同一件事的两边，
  只做一边就会一边过检查一边找不到变量（`x = 10` 之后下一批还要看得见 x）。
- **驱动与语言无关**：读行/续行/回显/命令/回滚都在 `repl.js`，一门语言给出
  `getMode/setMode`、`blank`、`complete`、`echo`、`asStmt`、`snapshot/restore`、
  `add -> delta`、`full -> 整程序 OIR` 就有 REPL。`omni repl --lang omni|sx` 是同一份驱动。
  `sx` 那条是 ADR-0014 那条路的落点：语法驱动的前端印出来的就是核心方言，
  所以 asy/jancy 的 REPL 将来落在 `CoreSession` 上，增量与回滚不用各写一遍。
  交互式的核心方言不必手打 `(module …)`：声明照写，其余形式自动进这一批的入口。
- 判据：`tests/repl/session.in` 的快照**逐字节没变**（换实现不换语义）；
...
**已完成（函数值与闭包，2026-08-25）**：ADR-0010。自举链条上的下一块 —— 编译器源码里
110 个箭头函数、161 处带回调的数组方法，没有函数值 JS AST 就没法降级。
- 类型 `fn(int, string) -> bool`（只有签名，没有形参名）；lambda `fn(int x) -> int { ... }`
  **必须写全注解** —— 实参要在重载解析之前定型，靠期望类型倒推会让两者互相依赖。
- **捕获按值，且在创建闭包那一刻拷贝**。这条是核心：C 侧按引用捕获要做逃逸分析 + 装箱，
  而 JS 的原生闭包按引用，所以生成的 JS 必须显式把捕获拷进记录，不能靠宿主作用域。
  判据就是"循环里建闭包"：Omni 给 0,1,2，靠宿主会给 3,3,3（Python 的晚绑定坑）。
  给捕获的变量赋值是**编译错误**，不是"改了但外面看不见"。
- 表示是**一个指针**：闭包记录 = 函数地址 + 捕获，记录自己当第一个实参传进去，
  所以不需要胖值。每个签名生成一个类型化的调用助手 `omni_call_fn_int__int` ——
  在调用处直接展开强制转换会把被调方求值两次（`get_handler()(x)` 会调两次）。
- 具名函数当值用：包一层零捕获的薄适配器，让所有函数值共用同一套调用约定。
- lambda 提升在检查器里做（顶层函数 + 闭包记录），后端只多认三个节点：
  `MakeClosure` / `CaptureRef` / `CallFn`。捕获集合按需登记，逐层往外每层都登记一次 ——
  内层的值来自外层的捕获槽，跨层直接读外面的栈帧会读到已经返回的帧。
- 测试：`tests/cases/20_closures.omni`（差分）、`21_null_fn_call.omni`（空函数值调用在
  两个后端都是同一条诊断，C 侧不许退化成段错误）、`tests/errors/fn_value_rules.omni`
  （7 条规矩）、`tests/oracle/closures.{omni,py}`（Python 参照里那句 `lambda x, i=i:`
  正是两种捕获语义的差别本身）。
...
- 性能现状（诚实记账）：编译整个编译器，node 上 0.37s，原生 0.92s、RSS 811 MB。
  原生比 JS 宿主慢 2.5 倍 —— 在 ADR-0001"不慢一个数量级"的底线内。RSS 全是 arena
  只分配不释放的账：40.9M 次分配、872 MiB 总量，峰值就等于总量。
  已经摘掉的两个低垂果实：容器 `_from` 与 `omni_js_call3` 的实参表按最小 4 分配
  （精确分配之后 1037 -> 872 MiB、1.67 -> 0.92s）。剩下的大头是"每次回调都要造一张
  list&lt;dynamic&gt; 实参表"（127 万次）与对象字段写入时 dict 的三个数组增长（161 万次），
  两条都要靠生命周期信息才能真正去掉 —— 那是 ARC/区域分配的活，见下面第 4 条。
