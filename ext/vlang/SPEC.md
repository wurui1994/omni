...
- **我们自己量出来的**（这一门尤其重要，因为没有规格）：`vlang.grammar`
  479 条产生式 / 661 处冲突，尾巴上那条曲线。里面有三条**只能量出来**的结论：
  - **`ret` 里那三条与 `type` 重复的产生式一共 171 处冲突**（850 的两成），删掉之后
    850 → 679，语义一点没少。这是"按冲突参与度排名"那把尺子的第一次成功。
  - **区间（`..`）不该摆在表达式级联上**：摆在那儿 `a[0..3]` 既是 slice 又是 index，
    180 份文件报"两个解"。删掉之后 +187 份、冲突 −24、解析时间 60s → 47s。
  - **CDIRECTIVE 的 `not-after` 名单里不能有 `}`**（`}` 换行接着写 `#flag` 是常见排版）——
    净 −1，量过，退回来了。
- **语料与现状**：V 树里的 `.v`。量出来 **4461/6183（72%）**。
  剩下的最大一类仍是 `too many concurrent parses`，主因已定位：
  `operand` / `h-operand` 那两条受限链彼此重复（与 nim 的 551 处同形）。
...
1. **语法侧还欠 1722 份**。串插值那一类**已经不欠了**（词法层 `interp-string` 收了，
   当场试过 `'${os.join_path(out, 'index.html')}'` 读得进来）。
   现在最大的一类仍是 `too many concurrent parses`。
2. **661 处冲突**仍比 go（61）高一个数量级。已知主因是 `operand` / `h-operand`
