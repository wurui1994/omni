...
- **语料与现状**：fbc 树里的 `.bas` / `.bi`。量出来 **954/1722（55%）**。
  最大一类是 154 份 `too many concurrent parses`（`union` 里嵌 `union` 的类型上炸开）。
...
外加外部 IO 的效应。零个新节点。**
它们的量全在**语法侧**（每条一小套词序），这正是这门语言 55% 覆盖率的原因。
...
1. **语法侧还欠 768 份**，最大一类是 154 份 `too many concurrent parses`
   （`member` 那一格里 `dim`/`static`/`const`/裸字段四条路重叠）。
   还有 35 份卡在"例程头里名字后面跟裸名字"（`freebasic.grammar` 尾巴记着
   试过一手、退回来了）。`:` 的双重身份要在词法层分开。
