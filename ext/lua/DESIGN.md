...
再还一笔：**`..` 与数字的印法**。`(bin "+" str str)` 就是拼接，而数要按 Lua 的 `%.14g`
变成字符串 —— 那正是 `(tostr E (int 14))`（两边量过一模一样）。顺这一笔还查出**我自己的一个错**：
`print(<数>)` 先前直接交给核心方言的 `print`，它对 real 走 `%g`（6 位），于是 `print(1/3)`
会印 `0.333333` 而 luajit 印 `0.33333333333333`。生成的例子全是小整数，尺子没抓到 ——
这一笔是照着两边的格式化规则读出来的，不是量出来的。

量到的（`node ext/lua/tests/run.js`）：生成 136 个 + 语料 4 个，**一致 106、记账 3、分歧 0**
（另有 31 个 luajit 自己就跑不动 —— 那是"甲"那一问的事）。语料在 `tests/corpus/*.lua`：
期望由 luajit 给，那儿只放"写得像真程序"的例子（生成器造不出来的那种：几件事凑在一起）。
...
3. **两条腿**：解析/写回这条腿齐了（语料 112/112 幂等）；降级那条腿开了第一刀 ——
   同一段 Lua，`luajit` 与 `omni run` 的输出逐字相同：**106 一致 / 3 记账 / 0 分歧**。
4. **例子全生成**：`ext/lua/tests` 里没有手写的用例清单 —— 只有生成器与两把尺子。

## 10. 第二条路：`lua.grammar`（GLR），与上面那条并存

`ext/lua/lua.grammar` 是同一门语言的**另一种写法**：语法写成数据，走
`glr/table.js` + `glr/driver.js`（与 asy / jnc 那两门同一条）。

**上面那条不弃用。** 它是今天 `omni run x.lua` 真正在跑的路，而且它多担着三件这一份
语法担不了的事：降级（`lower.js`）、写回、方言增量表（luajit / gsl-shell 各一张）。
这一份只做**解析** —— 出一棵具体语法树。

两条并存的理由不是"多一个选项"，是**互为判据**：同一份语料两条路都要认；一条认一条
不认的每一处都是一笔账（要么手写的驱动器漏了一格，要么这份语法写错了）。

一条命令：

```
omni glr table ext/lua/lua.grammar --brief     # 建表 + 剩下的冲突
omni glr parse ext/lua/lua.grammar FILE...     # 解析，印树（--count 只印摘要）
```

### 量到的（M 系笔记本，node 22）

- **表**：61 终结符 / 24 非终结符 / 104 条产生式 / **211 个状态**，交给 GLR 驱动器的
  冲突 **23** 处（`var` 与 `prefixexp` 互递归那一族，SLR(1) 不够，驱动器分叉后一支自然死）。
- **建表**：冷 **80 ms**（进程总计 0.16 s）；第二遍命中内容寻址缓存 **11 ms**（0.08 s）。
- **解析**：一个进程里建一次表、连着吃 **34 个文件 / 398 KB / 76,367 记号 / 162,045 节点**，
  墙上时间 **0.35 s**（含 node 自己那 40 ms 与读表那 11 ms）—— 约 **1.2 MB/s**、
  **22 万记号/秒**。

### 覆盖：34 / 67，剩下 33 个不是这份语法的账

语料 = `reference/lua`（testes）+ `reference/luajit/src/jit` + 本目录 `tests/corpus`。
收的是 **Lua 5.1…5.4 的并集**（5.2 的 goto/标签与空语句、5.3 的位运算与 `//`、
5.4 的 `<const>` / `<close>`）。剩下那 33 个分两类，**都不是"语法写少了"**：

- **`reference/lua/testes` 那一批是 Lua 5.5-work**，不是任何已发布版本：
  `global <const> *`（5.5 才有的全局声明）、`function f(...t)`（具名变长参数）、
  第一行光一个 `#` 的注释（`#!` 我们收，光 `#` 收不了 —— `# t` 在 Lua 里是合法的取长）。
- **`reference/luajit/src/jit/*.lua` 里有非 Lua 的算符**：`a != b`、`c ? x : y`
  （`dis_arm.lua:429`、`bc.lua:72`）。那棵树不是纯 Lua 5.1。

要把这两类收进来是**加方言**，不是改这一份：与 §8 那两张增量表同一个位置。
明说的三处不足在 `lua.grammar` 文件头（长括号只收 0 级、`\z`、长串首行那个换行）。
