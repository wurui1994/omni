...
//  18. **数组形参**（`int sumArr(int xs[], int n)`，`arrmath.cpp`）：声明符上那个 `[]`
//      从前没人看 —— 形参当成一格 `int`，体里 `xs[i]` 报"aget 的实参要是数组"。收成一格
//      **列表**（方言的列表本来就是引用语义，与 C 里"数组退化成指针、改得动调用者那一片"
//      对得上）。**`int* xs` 不走这条** —— 那个写法在这条腿上专指出参（见第 3 条）；两种
//      写法在 C++ 里分不出谁是谁，所以这门定死：**要数组就写 `[]`**。
//  19. **`<math.h>`**（`arrmath.cpp`）转成公共层现成的 `(rmath …)` —— 那张表
//      （`src/core/sexpr/lower.js` 的 `RMATH`）的**名字与元数就是 C99 的**，所以这一门一行
